...
Append-only governs *normal operation* — money is never mutated in place while the platform runs. The **canonical hard reset is a full database wipe-and-restart** (delete the SQLite file; the `admin` account re-seeds via `ensureAdmin`) — an out-of-band operator action outside normal play. The admin **soft reset** (password-clear, see `ADMIN.md`) is a smaller best-effort convenience for releasing a single alias: it clears that account's `passwordHash` and **re-grants its wallet to the starting amount** (a single-account out-of-band exception), while **preserving the alias and its standings**. It is reachable only through the admin interface and refuses to run on an account with an active match or escrowed stake. **Standings live independently of the wallet ledger** (so a wallet re-grant never erases leaderboard position); the snapshot durability that lets them survive a restart is ADR-011.
