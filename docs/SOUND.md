...
- **Mute:** `isMuted()` / `toggleMute()` / `setMuted()` / `subscribe()`, persisted in `localStorage` (`rc:sound:muted`), **default = ON**. The header **`MuteToggle`** (speaker / muted-speaker) reads and writes the module directly and re-renders via `subscribe()`.
