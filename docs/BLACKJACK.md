...
- **#2 server-authoritative + redaction.** Decks, draws, timers, and outcome live on the server. A player ever sees only their own cards plus exactly one opponent card value and the opponent's hand size; all other card values, the deck seed, and stand/bust status stay hidden until reveal (`viewFor`).
...
**Decks & deal.** Two *independent* decks, one per player — so play is non-interactive (no shared shoe, no draw-order contention). Each deck is shuffled from a **provably-fair commit-reveal**: the server publishes a hash of the round's seed *before* the deal and reveals the seed *after*, so each player can verify their own card sequence. Each player is dealt 2 cards from their own deck, and sees **both of their own cards, exactly one of the opponent's, and the opponent's hand size** (rendered as a face-down back per hidden card) — symmetric, and the only opponent information ever shown.

**Actions & timer.** Hit or Stand only (no double, split, or insurance). On the deal a **10-second timer** starts for both players simultaneously; each Hit resets that player's timer; expiry → auto-stand on current total. Bust (>21) locks the hand. Ace is 1 or 11, auto-downgraded to avoid a needless bust. The opponent's hand size is surfaced — a face-down card appears when they draw — but no card value, and no stand/bust status, until the reveal.
...
**Reveal choreography (continuous, in place — never a re-render).** The reveal is one continuous scene; **no card ever leaves the screen** (no unmount, blink, or reflow of the existing cards). Every hidden opponent card is already on the table as a face-down back — each was dealt face-down the moment the opponent drew it during play (`handSize`, above) — so at the reveal they all simply **flip over in place, in sequence**; nothing is newly *dealt in* at the reveal itself. The one exception is the atomic resolving/busting hit: a hit that both extends and immediately resolves the hand in the same server step is never broadcast as a live in-play draw, so its back was never seen on the table — that one card **slides in face-down first, then flips**, exactly like every other card's entrance, never arriving already face-up. Then totals update, then the result state shows (outlines / win animation / "Push" label, below). The player should read it as a dealer turning over a hand, not a screen refresh. *(Card stacking — **OVER for everything, no exception**: each card lands **on top** of the previous cards in its hand (standard overlapping fan, newest most visible), z-order fixed before its deal animation, whether it's face-down or face-up. The old single-hole "sits underneath" exception is retired now that a fan of face-down backs, not one hole card, represents the hidden hand. Flips keep whatever z they render at.)*

**One continuous scene — never a rebuild (identity continuity).** The reveal and the push both **mutate the existing scene; they never re-render the table from the result payload.** Every card is a **persistent element with a stable key that survives in-play → reveal → push** — keyed by the **round it belongs to** (`opp-${round}-i`, etc.), which changes **only on a genuine re-deal** (a new hand), never at a win/loss (terminal — `round` is unchanged) and never when entering the push hold. The **push must reuse the just-resolved cards' identities** (key them by the *resolved* round, from `lastResult`), **not a fresh `push-*` set** — a separate set unmounts the resolved cards and mounts new ones, which is the one-frame blink. Every opponent card the reveal shows was already a face-down back sitting on the table (dealt in as the opponent drew it — see `handSize` above), so **no opponent card mounts fresh at reveal anymore**; the only motion at reveal time is each back **flipping in place**, totals updating, and outlines appearing (the one exception, the atomic resolving/busting hit, still slides in face-down before it flips — it just never arrives already open). Acceptance: watch any already-visible card (e.g. your own first card, or the opponent's) across the reveal/push — it must never blink, shift, or re-animate; the only motion is in-place flips, totals updating, and outlines appearing.
...
- **`viewFor`** — returns the player's own two-plus cards, exactly one opponent card value, and the opponent's hand size; redacts the opponent's other card values and stand/bust status until terminal reveal.
