...
### 2026-09-26#2 — D61, Coinflip's countdown ring + coin bolt, both deliberate reuses of existing RPS components. Item 1 (clock) is a clean lift-and-share, but the component it asks to reuse (`RpsCountdown`) is currently PRIVATE to `RpsHub.tsx` — needs extracting first, not just importing. Item 2 (bolt) looks like a one-line path swap but isn't: the coin is a WebGL canvas-texture cap, not an SVG, and the placeholder bolt it currently draws (`BOLT_PATH`) is a SHARED constant also used by unrelated card-game components (Blackjack/Hilo/Baccarat's card backs) — confirmed via its own doc comment that it's a known-temporary stand-in awaiting exactly this fix, which raises a real scope question the ticket doesn't address            [READY TO TICKET]
From: Designer's own report (relayed by Owner, screenshot at `D61/IMG_1319.PNG` showing the circular ring and the sharp-cornered bolt exactly as described), verified via direct reads of `RpsHub.tsx` (`RpsCountdown`'s exact Framer Motion implementation and its call site's own separate wrapper), `CoinflipHub.tsx` (`CountdownRing`'s current SVG-ring implementation and call site), `Coin.tsx` (`makeCapTexture`'s canvas/Path2D bolt-stamping math, the `ROTATE`/`scale`/`translate` derivation, the verified-numerically rotation-correction doc comment), `CardBack.tsx` (`BOLT_PATH`'s own doc comment confirming it's a deliberate placeholder), `index.css` (`--coin-heads-mark`/`--coin-tails-mark` tokens), and `Full Spec.html:90-93`/`618-622`/`629`/`3749-3752`/`3803`

## Item 1 — the clock: a clean reuse, but the source component needs extracting first

**Confirmed the RPS clock's actual implementation matches the ticket's citation precisely — just built with Framer Motion, not raw CSS keyframes.** `RpsCountdown` (`RpsHub.tsx:193-220`) is the exact digit-flip mechanism cited: a 52×26 `overflow-hidden` window, `AnimatePresence`+`motion.span` reproducing the `rcClockOutA/B`/`rcClockInA/B` keyframes exactly — `exit` matches the citation's `0%→45%→100%` color sequence (`var(--rc-text)→#B285F7→#8B45F0`) and 240ms `cubic-bezier(0.33,0,0.67,0.35)` timing; `initial`/`animate` matches the 260ms `cubic-bezier(0.3,0.9,0.32,1)` slide-in. Framer's own `key={seconds}` remount does the "A/B alternate so it replays" job the citation's own naming scheme exists for — same effect, cleaner mechanism, not a deviation worth flagging.

**The 52×52 "pop in/out" wrapper (`clockOp`/`clockScale`) is NOT inside `RpsCountdown` — it's a separate wrapper at RPS's own call site, and that part genuinely shouldn't be copied.** Confirmed at `RpsHub.tsx:544-553`: the outer `opacity`/`scale`/`transform: translate(-50%, calc(-50% - 42px))` div is RPS-specific (it centers the clock ABOVE the VS label, absolute-positioned). The ticket's own instruction — "don't use the RPS wrapper's centring transform" — is exactly right and matches what's actually in the code: Coinflip needs its OWN 52×52 wrapper (opacity/scale only, no absolute-centering transform) sitting inside Coinflip's EXISTING positioning div (`absolute left-3 top-1/2 -translate-y-1/2`, already the correct "left of the coin" placement) — not a copy of RPS's own positioning math.

**The one real gap: `RpsCountdown` is a private, unexported function — "lift it" is a literal instruction, not a figure of speech.** Recommending it move to `apps/web/src/components/hub-shared/` (matching this session's own recent precedent — `avatar.ts`'s extraction for ticket `2026-09-25#6`), exported and renamed to something game-agnostic (e.g. `DigitCountdown`), then imported by both `RpsHub.tsx` and `CoinflipHub.tsx` — one implementation, not a duplicated copy.

**Confirmed `CountdownRing`'s replacement should stay an always-mounted, opacity-gated wrapper, not the current conditional mount — a real, deliberate behavior change, not just a visual one.** Today's `{live && !revealing && <CountdownRing .../>}` (`CoinflipHub.tsx:174-178`) pops the ring in/out with no transition at all. The ticket's own check explicitly asks for a fade ("Clock pops in... when the countdown starts and fades out when it ends") — matching RPS's own always-mounted-with-opacity-transition pattern, and consistent with `CoinflipPanel`'s own stated design philosophy (its doc comment already argues for one persistent JSX subtree over conditional mounts, for the coin itself). Recommending the new wrapper stay mounted whenever the pick window COULD be live, opacity/scale-gated on the same `live && !revealing` condition already computed today.

**One existing test needs updating, not silently left to fail.** `CoinflipHub.test.tsx:282` asserts `getByTestId('coin-countdown')` — recommending the new wrapper (or the lifted component itself) keep that exact test id so this assertion doesn't need touching, or update it deliberately if the id moves.

## Item 2 — the bolt: real technical complexity the ticket's own markup instructions don't fully anticipate

**Confirmed directly: the coin is NOT an SVG — it's a WebGL 3D cylinder cap, and the bolt is stamped as a canvas texture via `Path2D`, not drawn as markup.** `Coin.tsx`'s `makeCapTexture` (`:186-228`) fills a 256×256 canvas with the face color, then paints the bolt via `ctx.fill(new Path2D(BOLT_PATH))` after a hand-derived `translate`/`rotate`/`scale` specifically tuned for the CURRENT placeholder path's own `0 0 24 24` viewBox: `scale = (256 * 0.55) / 24`, then `translate(-12, -12)` (exactly half of 24, centering that viewBox on the canvas). **Swapping in the real 351×374 path is not a one-line `d=` replacement** — the scale/translate math needs recomputing for the new (larger, NON-square) viewBox, or the bolt will render wildly mis-scaled and off-center. Recommending: `scale = (CAP_TEXTURE_SIZE * 0.74) / 351` (351 = the new viewBox WIDTH, matching the ticket's own `width="74%"` semantics exactly — an SVG's `width` attribute sets the scale factor off its own width, with height following via default `preserveAspectRatio`), `translate(-175.5, -187)` (half of 351, half of 374 — the new viewBox's own center, same pattern as the current `-12,-12`).

**The existing `ROTATE = -π/2` correction is very likely still correct — it's a property of the canvas-to-3D-geometry mapping, not the path being drawn — but this needs confirming live, not assumed, given the added complexity and the doc comment's own explicit numerical-verification history.** The doc comment's own reasoning derives `ROTATE` purely from the cylinder cap's UV formulas composed with the camera projection — nothing about it depends on which path is drawn on the canvas, so it should transfer unchanged to the new, more complex path. But that same doc comment stresses this was "verified numerically... not just assumed" for the OLD path specifically (walking its own four distinctive asymmetric points). Recommending whoever implements this do the equivalent live check against the new path once it's in (both heads and tails), rather than trusting the math carries over untested — I have no browser tooling in this environment to confirm this directly myself.

**A real scope question the ticket doesn't address, flagged rather than decided either way.** `BOLT_PATH` (`CardBack.tsx:17`) is a SHARED constant — also drawn on Blackjack's hole card, and (per that file's own doc comment) Hilo/Baccarat's deck piles. Its own doc comment states plainly: *"This is a clean GENERIC bolt — the brand mark only exists as a raster... so there is no single clean SVG path to reuse (per the spec's fallback). Swap THIS ONE constant for the designer's exact bolt path when provided — nothing else changes."* That "designer's exact bolt path" is now available (the same one this ticket cites, already in the repo via `RpsRedactedIcon`) — meaning the card-back bolt is ALSO currently the wrong placeholder, by this codebase's own prior admission, and this ticket is implicitly the trigger for that fix too, even though its own title scopes it to "Coinflip." **Recommending, for THIS ticket specifically: add a new, coin-scoped constant (do not touch `BOLT_PATH`/`CardBack.tsx`)** — keeps this ticket's blast radius to Coinflip only, matching its own stated scope exactly. Separately: recommending flagging to Owner/Designer that `CardBack.tsx`'s own card-back bolt (Blackjack/Hilo/Baccarat) is the SAME known-wrong placeholder and would benefit from the identical fix in a follow-up — not silently bundling it into this ticket, not silently leaving it as the one remaining inconsistent bolt in the app either.

**Confirmed the "darker shade of the face colour" tokens already exist correctly — nothing to add there.** `COIN_FACE_TOKENS.mark` already resolves to `--coin-heads-mark`/`--coin-tails-mark` (`#c8761f`/`#3f52d6`), both explicitly documented as "darker-orange/-blue bolt, tone-on-tone" — exactly the citation's own `#4340D8`-on-`#4F4CEA` relationship, already correctly wired into `makeCapTexture`'s own `markHex` parameter. Only the PATH needs to change, not the color logic.

---

**Ask:** item 1 is safe to implement as scoped (extract `RpsCountdown`, rebuild Coinflip's own wrapper, update the one test id). Item 2 needs two things settled before or during implementation, not guessed: (1) whoever implements should do a live visual check of the new bolt's rotation/centering on both coin faces, since the existing verified math was specific to the old path's geometry; (2) please relay to Owner/Designer whether `CardBack.tsx`'s own placeholder bolt (Blackjack/Hilo/Baccarat) should get the same fix in a follow-up — recommending a NEW coin-only constant for this ticket either way, not touching the shared one without an explicit decision.
...
**Designer's exact words: "Yes they should complete the full movement as if would be a regular result ending."**

This settles the one open item from PR #727 (D48+D49 combined implementation). PM's own implementation currently keeps `cardW`/`cardH`'s big-geometry growth gated on `terminal` only (ticket `2026-09-12#1` item 2's original decision, deliberately preserved) — an ordinary tie's reveal plays at the small 92×130 geometry today. Designer's answer means this needs to change: **an ordinary tie's reveal should ALSO grow to the big 124×176 geometry**, matching every other part of the reveal sequence that D49 already unified (the 700ms pause, the flip, the frame coloring) onto one shared timing.

**Scoped precisely, not a re-open of anything else:** this is specifically the `cardW`/`cardH`/`cardGap`/`vsWidth` ternaries in `RpsBoard` — currently `terminal ? BIG : SMALL`. Recommending these key off the SAME `revealActive` (or equivalent "a reveal — terminal OR tie — is currently active") signal `revealDone`/`revealStage` already use, rather than `terminal` alone. The clock-fade (also currently `terminal`-only per `2026-09-25#2`'s own Beat 1) is a SEPARATE citation Designer's own D48 ticket scoped narrowly to "the pick window closing" — not addressed by this specific answer, and I'm not extending this correction to it without a similarly explicit confirmation. Flagging that distinction so it isn't accidentally swept in.

**One thing worth a quick check once implemented, not a new open question:** the tie-reveal's own return-to-small-and-redacted transition (the `TIE_REVEAL_HOLD_MS`-driven reset back to the next round's window) already reverses cleanly via the existing shrink transition — growing bigger first shouldn't change that mechanism, just its start size. Worth a quick live/test check that the shrink-back-down animation still reads correctly once the tie's own reveal starts from the bigger geometry, rather than assuming it's automatically fine.

---

**Ask:** please apply this correction to PR #727's already-merged work (or fold it into whatever's next in the RPS queue) — key the big-geometry growth off `revealActive` instead of `terminal`. Let me know once it's up and I'll re-verify.

---
### 2026-09-25#6 — D52, the opponent bar: no robot/@ in the name, a real 24-face avatar pool instead of the grey default. Confirmed the name fix is genuinely two small, LOCAL string-strips (the searching and settled states are each missing a DIFFERENT half of the transform, not the same bug twice). The avatar fix is bigger than it looks: 14 new asset files ship with the ticket (verified against the existing 10 — byte-identical, a clean extension); the live match protocol has NO field at all for a real opponent's chosen avatar today (the one that exists is for a completely different screen); and satisfying Designer's own 3-way real/real-no-avatar/simulated split cleanly needs the SAME already-proven server-side lookup used elsewhere, not new client-side guessing. Also found the recent-games list doesn't render an opponent avatar AT ALL today — a real UI gap, not just unwired data            [READY TO TICKET]
From: Designer's own report (relayed by Owner, screenshot at `D52/IMG_1281.PNG` showing "🤖 @skyhook" and a grey default circle exactly as described, plus `profile picture Avatars.zip` — the full `rc-01`-`rc-24` asset set, extracted and verified: `rc-01`-`rc-10` byte-identical to what's already in the repo, `rc-11`-`rc-24` genuinely new, all 480×480), verified via direct reads of `GameHub.tsx` (`OpponentSlot`'s exact avatar/name rendering, `scanNames`'s own construction), `Avatar.tsx` (`avatarIdForName`'s current djb2 hash, `PRESETS`), `packages/shared/src/protocol.ts` (`AvatarId`, `MatchStartPayload`/`MatchStatePayload` — no opponent-avatar field on either — vs. `RecentMatchEntry`, which has one), `packages/core/src/match-history.ts` (`avatarFor`/`displayNameFor`, both wrapping injected lookups), `apps/server/src/server.ts` (`identity.getAvatarId`, the real, already-proven lookup), `ProfileHub.tsx` (`AVATAR_PRESETS`'s own separate 10-item literal; `MatchRow`'s actual render, confirming no avatar slot exists there), `GamesCarousel.tsx`/`ProfileHub.tsx`'s own bot-name-stripping precedent, and `Full Spec.html:446-448`/`3132-3137`

## Part 1 — name: two different half-fixes needed, not one bug repeated

**Confirmed exactly as reported for the SETTLED (in-match) state.** `OpponentSlot` renders `opponentName` (from `MatchStartPayload`, server-sent) completely raw, with zero transform — matches the screenshot's "🤖 @skyhook" precisely. The server never strips anything here; this string is the account's literal stored username.

**The SEARCHING state has a DIFFERENT, smaller gap — checked directly rather than assumed identical.** `scanNames` (the decorative name-scramble source) is already run through `displayHostName`, the existing ADR-010 bot-emoji stripper `GamesCarousel.tsx` uses — so the 🤖 is already gone by the time a name reaches the scan set. But `displayHostName` (correctly, for its OWN callers) ALWAYS PREPENDS one `@` — so the scrambled names shown during search already read `@name`, not `🤖 @name`, but still carry the unwanted `@`.

**Recommending one new, small, LOCAL helper inside `OpponentSlot` — not touching `displayHostName`/`ProfileHub`'s own stripper.** Those two are correct for their own callers (the carousel, the profile picker), which deliberately keep the `@` convention per the app's established bot-disclosure pattern. A bare "strip a leading 🤖, strip any leading @" function applied at `OpponentSlot`'s own two display sites (`scan` and `opponentName`) handles both cases correctly regardless of which half was already stripped upstream.

**One thing worth flagging for awareness, not blocking on.** This is a deliberate divergence from the "always disclose via 🤖 or @" convention the rest of the app uses (per `GamesCarousel.tsx`'s own doc comment citing ADR-010: "bots are ordinary 🤖-labelled clients"). Designer's own ask is explicit and precise here, so implementing it as directed — just noting the game bar becomes the one surface where that disclosure convention doesn't apply, in case that's worth a second look from Owner.

## Part 2 — avatar: the asset side is simple; the DATA side needs a real protocol addition

**The core bug, confirmed at the exact line.** `OpponentSlot`'s avatar render is `avatarId={searching && scan ? avatarIdForName(scan) : 'default'}` — once actually in-match (`searching` false), it is UNCONDITIONALLY `'default'`, regardless of who the opponent is or what they picked. This is the literal grey-circle bug.

**Assets: a clean extension, verified byte-for-byte, not assumed.** Extracted `profile picture Avatars.zip` — `rc-01` through `rc-10` are byte-identical to what's already bundled; `rc-11` through `rc-24` are genuinely new, all confirmed 480×480. `AvatarId`'s type union, `AVATAR_IDS`, and `Avatar.tsx`'s `PRESETS` map all need widening from 10 to 24 entries. Confirmed `ProfileHub.tsx`'s own account picker is structurally isolated from this — it has its OWN separately-declared `AVATAR_PRESETS: [...10 ids]` literal, not derived from `PRESETS`'s full key set — so "your own picker stays at ten" is already guaranteed by the existing design, not something that needs defending against.

**The hash: the ticket cites a SPECIFIC formula, different from what's already there — flagging the discrepancy rather than assuming today's hash is close enough.** `avatarIdForName` currently uses `hashStr` — a djb2 hash, the SAME one `discColor`/`glyphColor` already use for per-user hue — NOT the prototype's own `avForName` formula the ticket explicitly cites (`h = (h*31+charCode)|0`, `abs(h) % 24`). Recommending a NEW function implementing that exact formula, since the ticket names it specifically by name and line — and flagging clearly that this is DELIBERATELY a different hash than `discColor`/`glyphColor`'s own djb2, not an oversight to "fix" by consolidating them.

**The real gap: there is no protocol field for a real opponent's chosen avatar in a LIVE match at all — checked precisely, not assumed missing.** `AvatarId`-typed `opponentAvatarId` exists ONLY on `RecentMatchEntry` (the recent-games list) — a completely different payload. `MatchStartPayload`/`MatchStatePayload` (the live match-start and reconnect-resume payloads) carry `opponentName` but nothing avatar-related at all. Designer's "it comes from the server with the match" requires adding a sibling field to both, mirroring `opponentName`'s own exact existing pattern (required on `MatchStartPayload`, optional on `MatchStatePayload`'s resume path).

**A reusable, already-proven server-side lookup exists — recommending reuse, not a new mechanism.** `packages/core/src/match-history.ts`'s own `avatarFor` already wraps `identity.getAvatarId` (wired in `server.ts`) to populate `RecentMatchEntry.opponentAvatarId` correctly today. The same lookup, called at the match-start/resume send sites in `gateway.ts`, is the natural source for the new live-match field.

**The 3-way real/real-no-avatar/simulated split needs one real design decision — resolving it server-side, not with client-side guessing.** A bot account's own stored `avatarId` would ALSO come back `'default'` from `identity.getAvatarId` (bots never go through the avatar picker) — indistinguishable from a genuine human who simply hasn't chosen one, unless something ELSE marks the account as simulated. Confirmed there's no separate `isBot` schema flag anywhere in this codebase — the only existing signal is the same one Part 1 strips: a raw stored username starting with `🤖` (the app's own established, existing bot-disclosure convention). **Recommending the server resolve this fully before sending**, exactly mirroring how `opponentName` is already a fully-resolved, ready-to-render string: if the real stored avatar is unset AND the raw username carries the bot prefix, compute the name-hash fallback server-side and send that as the final `opponentAvatarId`; otherwise send the real stored value (`'default'` included, for a genuine avatarless human). The client then just renders whatever arrives — no bot-detection logic needed in `OpponentSlot` at all, same as today's `opponentName`.

**Same gap exists in the recent-games list's own lookup — worth fixing in the same pass, not left half-done.** `match-history.ts`'s `avatarFor` has the identical "bot with no stored avatar" hole. Recommending the same fallback logic land in both places together, per Designer's own "one source per opponent" instruction.

## Part 3 — search scramble: already wired, needs nothing beyond the Part 2 fix

**Confirmed this already works structurally — no new client logic needed.** `avatarIdForName(scan)` already drives the searching-state avatar exactly as the ticket describes; it only needs the underlying hash/pool-size change from Part 2. One continuity note: hashing the SAME bare/stripped display-name string in both the scramble and the settled state (not the raw, possibly-🤖-prefixed one) is what makes "flicker, then settle on the found opponent's actual face" cohere for a simulated opponent whose scrambled name happens to match.

## Everywhere else — the recent-games list doesn't render an avatar at all today

**Checked `ProfileHub.tsx`'s own `MatchRow` directly: it renders a game-icon tile, never an opponent avatar.** `RecentMatchEntry.opponentAvatarId` is populated server-side already but is genuinely dead client-side — nothing reads it. Designer's "same avatar in the recent-games list" needs a real, small UI addition (an `<Avatar>` in the row), not just a data-wiring fix — flagging this as real additional scope, not assuming the field being populated already covers it.

---

**Ask:** Part 1 is small and safe to ship immediately. Part 2 needs the asset files added, the type/pool widened, a new hash function, AND a real protocol change (`gateway.ts` + `protocol.ts`) — bigger than a typical client ticket, similar in shape to `2026-09-25#5`. No open questions on direction; flagging the ADR-010-disclosure divergence in Part 1 for awareness only.

---
### 2026-09-25#5 — D51, Mines draws are neither revealed nor reset. Root-caused precisely to a real gap in the existing post-round state machine (never designed for a mid-match internal draw-replay, only for a genuine match end), PLUS a deeper, confirmed finding: the server itself never preserves the drawn round's own board/score data anywhere once it redeals — meaning this needs a real server-side protocol addition (mirroring RPS's own `revealedChoices` precedent), not a client-only patch. Also found the requested opponent-bar draw ring has zero existing mechanism to build on (the one ring-capable prop is own-bar-only) and the one existing GENERIC "draw" signal is confirmed inert for Mines already (wrong field name). Confirmed the draw cap itself is already correctly implemented and should be unaffected            [READY TO TICKET]
From: Designer's own report (relayed by Owner, screenshot at `D51/IMG_1280.PNG` showing the exact reported symptom — dimmed board, converged bars, a running 25s clock), verified via direct reads of `MinesHub.tsx` (`resultPhase`'s full state machine, its `currentMatchId`-only reset effect, `myGemCount`/`oppGemCount`'s live derivation), `packages/games/mines/src/mines.ts` (`resolve()`, `redeal()`, `decide()`, confirming the redeal is fully synchronous with no server-side snapshot of the pre-redeal boards anywhere), `GameHub.tsx` (`OpponentSlot`'s complete absence of any win/lose/draw ring prop, the generic `drawBeat`/`replaysOf` mechanism), `slotReveal.tsx` (`replaysOf`'s exact field name), `RpsHub.tsx`'s own `tieReveal` (the precedent this needs to mirror), and `Full Spec.html:3376-3385`/`3787`/`3096`

## Root cause 1, confirmed precisely: the post-round state machine only ever resets on a genuinely NEW match

**`MinesHubScreen`'s own `resultPhase`/`lockReason` machine (`converge`→`reveal`→`final`) has no listener for a mid-match internal draw-replay at all.** Its only reset effect (`:676-684`) fires exclusively when `props.currentMatchId` changes to a genuinely new value — an ordinary Mines draw keeps the SAME `currentMatchId` (confirmed in `mines.ts`: `resolve()`'s draw branch calls `redeal()` and emits `new_round`, never `match.end`). So once a drawn round reaches `'final'`, NOTHING ever moves `resultPhase` back — the dimmed board, converged bars, and orange... well, no ring today either (see below) — just sit there frozen, while the server's OWN fresh round (new board, new `roundStartedAt`, ticking clock) has already arrived and is live underneath. This is EXACTLY the reported symptom: "the bars converge, the round restarts underneath them, and the clock starts counting with the board still covered" — confirmed at the code level, matching the screenshot precisely (dimmed board, both bars showing "0 gems," clock genuinely counting down at 25s).

## Root cause 2, found while checking whether a simple reset would even work — it wouldn't

**The server never preserves the drawn round's own data anywhere — `redeal()` runs synchronously, in the SAME function call, before any client ever sees the round as resolved.** Traced `resolve()` directly: `decide()` computes both scores, then — in the SAME synchronous call, before any broadcast — `redeal()` immediately overwrites `s.boards` with two fresh boards and re-stamps `roundStartedAt`. The ONE outgoing update clients ever receive for a draw already carries the NEW round's reset boards; the drawn round's own final tile states/scores are never transmitted at all, not even for one intermediate tick. Confirmed both `myGemCount`/`oppGemCount` (`:694-695`) are LIVE-derived from current `gameState` with no snapshot — so even fixing root cause 1 alone would show `0`/`0` gems (the fresh round's zeroed counts) during `reveal`/`final`, not the drawn round's real equal counts, and MY OWN tiles would already show blank/unopened instead of whatever I'd actually just revealed.

**This means a client-only fix cannot work — the data simply isn't there to show.** RPS solved the exact same class of problem for its own mid-match ties by having the SERVER snapshot both throws into the `new_round` event's own payload (`revealedChoices`) before resetting — `resolve()`'s local `b1`/`b2`/`s1`/`s2` (computed right before the redeal, still in scope) are exactly the data needed to do the same for Mines. **Recommending**: extend Mines' own `new_round` event payload with a snapshot of both players' pre-redeal board state (uncovered indices + `bustedOn`, not just the aggregate score, since Designer's own check requires the board to visibly show real revealed tiles before it "resets visibly" — a bare count wouldn't support that). No `packages/shared` protocol changes needed — `GameEvent.payload` is already loosely typed per-game, exactly how RPS's own `revealedChoices` gets away with the same thing today.

## The recommended shape: an RPS-`tieReveal`-style hold, adapted to Mines' own board visuals

With the server-side snapshot available, `MinesHubScreen` needs its own held-then-reset mechanism mirroring `RpsBoard`'s `tieReveal`: on receiving a `new_round` event whose payload's `draws` value increased, capture the snapshot into local state, and drive `resultPhase`'s existing `'final'` rendering off that FROZEN snapshot (not live `gameState`) for the 1500ms hold, then run the requested return choreography (bars home, rings fade, gem rows/captions fade, board un-dims, tiles visibly close to a fresh, differently-seeded layout, timer row expands reading 30s, clock starting 660ms after the bars arrive) before finally switching over to the live (already-fresh) `gameState` and clearing back to `resultPhase:'idle'`.

**One real implementation risk, flagged rather than assumed handled:** the server has ALREADY dealt the fresh round with open `legalMoves` the moment its update arrives — the frozen/held display must not let taps reach the real board underneath during the hold, since the server would legally accept them for the new round already in progress. Whoever implements this needs to gate `onMove`/tile interactivity on the SAME hold state, not just the visual overlay, matching Designer's own explicit check ("during the hold and the return, tapping a tile does nothing").

## The opponent-bar draw ring: no existing mechanism to build on, confirmed

**`OpponentSlot` has ZERO win/lose/draw ring capability today — only `OwnSlot` does.** Checked directly: the only ring-related thing `OpponentSlot` renders is the generic, unrelated `drawBeat && outlineClasses('draw')` (ticket `#161`'s shared "push" pulse) — and that mechanism is already confirmed INERT for Mines specifically: `replaysOf()` (`slotReveal.tsx:35-41`) reads a field literally named `replays`; Mines' own state field for this is `draws` (a different name), so `replaysOf(gameState)` returns `null` for Mines and `drawBeat` never fires. Designer's own ask (both bars ring orange on a draw, a deliberate divergence from the prototype's own own-bar-only rule) needs a genuinely new prop on `OpponentSlot` — recommending an analogous `drawRingColor`-style addition, driven off the same resultPhase/outcome-is-draw signal already computing `OwnSlot`'s existing `drawRingColor={gameId==='mines'?'#FF8A1E':undefined}`, so both bars share one signal and one color.

## Already correct, confirmed rather than assumed

- **The opponent-hold-for-reveal mechanism** ("`reveal` waits for their round to close") is already exactly right — `resultPhase`'s own effect (`:659-668`) already holds at `'converge'` until `opp?.locked` is observed true, scheduling `reveal`/`final` relative to THAT release moment. No change needed.
- **The frozen Play button** ("stays PLAYING at 0.5") should already fall out for free — `phase` stays `'in-match'` throughout an internal replay (`currentMatchId` never changes), so the shared `playFrozen` gate already holds it correctly without any Mines-specific work.
- **The draw cap itself is already correctly implemented, matching the ticket's own number exactly.** `mines.ts`'s `DRAW_CAP = 10` — confirmed a literal match. Since the cap-triggered void IS a genuine terminal outcome (`forcedOutcome:{type:'void'}`, `match_voided`, `currentMatchId` clears), it already routes through GameHub's standard, already-working terminal-result path — the SAME path every other terminal outcome uses, untouched by anything above. Recommending this get a live check once the ordinary-draw fix lands, purely to confirm the two paths don't interfere, rather than assuming.

---

**Ask:** this is a real, precise, confirmed fix — but a bigger one than a typical client-only ticket, since it needs a `packages/games/mines/src/mines.ts` payload change alongside the `MinesHub.tsx` work. No open questions on direction; happy to discuss server-side payload shape specifics if useful before implementation starts.

---
### 2026-09-25#4 — D50, REGRESSION: the header no longer stays pinned while scrolling. Root-caused precisely, and it's MY OWN error, flagged plainly — a well-documented CSS gotcha in `2026-09-24#6`'s own `overflow-x:hidden` fix on `html`/`body` (shipped in PR #714), which silently breaks `position:sticky` on any descendant via a real CSS spec rule I didn't check for at the time. Fix is a one-line, standard swap (`overflow-x:clip` instead of `overflow-x:hidden`) that keeps the original Dice horizontal-jump fix intact while restoring the header's pin. Confirmed the bottom nav is NOT affected by the same mechanism (different, unrelated positioning scheme) — Designer's own precautionary check comes back clean            [READY TO TICKET]
From: Designer's own report (relayed by Owner, no screenshots), verified via direct reads of `HubRibbon.tsx` (confirms `sticky top-0` is genuinely still applied — not lost), `App.tsx` (confirms no wrapping element was introduced between `body` and the rendered screens — rules out Designer's own candidate 1), `index.css` (`html`/`body`'s `overflow-x-hidden` rule, added in `2026-09-24#6`/PR #714), `HubToolbar.tsx` (confirms the bottom nav uses `position:fixed`, a structurally different, unaffected mechanism), `git log -S`/`git blame` (confirms the sticky header dates back to PR #144, untouched since, while the `overflow-x` change is the one, single, precisely-dated recent touch to `html`/`body`), Tailwind's own `corePlugins.js` (confirms `overflow-x-clip` is a real, available utility in the installed 3.4.17), and `Full Spec.html:159-171`/`2217`/`2723`

## Root cause, confirmed precisely — not the 3 candidates Designer listed, something more specific

**Ruled out Designer's own candidate 1 (header moved inside the scroll container) directly.** `App.tsx`'s top-level return is a bare React Fragment (`<>...</>`) — no wrapping `<div>` at all between `body`/`#root` and whatever screen renders. `HubRibbon` is still a genuine top-level sibling in the DOM, exactly matching the "body-scroll layout" the component's own doc comment describes.

**Ruled out candidate 2 (header lost its position) directly.** `HubRibbon.tsx`'s `<header>` still carries `className="sticky top-0 z-20 w-full bg-[var(--rc-bg)] ..."` — genuinely still there, unchanged since long before this regression (`git blame` traces it to PR #144, this app's own original body-scroll/sticky-header design).

**The real cause: `position: sticky` breaks the moment ANY ancestor (even `body` itself) gets a non-`visible`/non-`clip` `overflow` on EITHER axis — a real, spec-defined CSS rule, not a browser quirk.** Per the CSS Overflow spec's computed-value processing: if `overflow-x` is set to anything other than `visible`/`clip` while `overflow-y` is left `visible` (or vice versa), the browser silently promotes the OTHER axis's computed value to `auto` too — turning that element into a full scroll container on BOTH axes, even though only one was ever intentionally set. Once `body` becomes a scroll container this way, `HubRibbon`'s sticky positioning resolves against `body`'s OWN box (which is always exactly as tall as its content, since nothing constrains its height) instead of the real page/viewport scroll — so from the sticky element's own reference frame, no scrolling is ever happening, and it just scrolls away with everything else.

**This is `2026-09-24#6`'s own fix, and it's my own error — flagged plainly, not glossed over.** `index.css`'s `html { overflow-x-hidden } body { ...overflow-x-hidden }` (my own recommendation, PR #714, ticket `2026-09-24#6` — fixing Dice's horizontal-jump bug) is the ONLY change touching `html`/`body`'s overflow in this app's whole history (confirmed via `git log -S "overflow-x"` — one hit, that exact commit). I recommended applying it "at the true document root... so it defends every screen uniformly" specifically to be thorough — that choice is exactly what breaks sticky here. I didn't check for this interaction when writing that ticket.

## The fix: a one-line, standard swap — keeps the original fix intact

**`overflow-x: clip` instead of `overflow-x: hidden`, on both `html` and `body`.** `clip` is explicitly exempted from the cross-axis coercion rule (the spec only promotes the other axis when the set value is neither `visible` NOR `clip`) — it clips overflowing content identically to `hidden` visually, with zero functional difference for `2026-09-24#6`'s own original purpose (suppressing an over-wide descendant), but never makes the element a scroll container, so `position:sticky` descendants keep resolving against the real page scroll. Confirmed `overflow-x-clip` is a real, already-available Tailwind 3.4.17 utility (`corePlugins.js:1535`, `{'overflow-x':'clip'}`) — a straight `hidden`→`clip` swap on both existing `@apply` lines, no new CSS needed.

## The bottom nav: Designer's own precautionary check, confirmed clean

**`HubToolbar.tsx`'s nav bar uses `position: fixed`, not `sticky` — a structurally different mechanism, unaffected by this specific gotcha.** `position:fixed` positions against the viewport/initial containing block directly; plain `overflow` on an ancestor (without `transform`/`filter`/`perspective`/`will-change`/`contain`, none of which are present here) never changes a fixed element's containing block the way it does for sticky's "nearest scrolling ancestor" lookup. Confirmed the bottom nav's own className (`fixed bottom-0 ...`) is unrelated to and untouched by `2026-09-24#6`. No live browser access in this environment to visually confirm, but structurally this should already be fine — worth a quick look alongside the header fix regardless, exactly as Designer's own check asks.

---

**Ask:** the `overflow-x:clip` swap on `html`/`body` in `index.css` is a precise, one-line, high-confidence fix — safe to ship immediately, no open questions. Recommending whoever implements do a live check per Designer's own repro steps (scroll Games/Rewards/Account in both themes) to confirm the header now stays pinned and the bottom nav was never actually affected.

---
### 2026-09-25#3 — D49, RPS draws + adding the Dice-style bar treatment to RPS. This DEFINITIVELY ANSWERS ticket 2026-09-25#2's open question 1 — a draw needs the SAME full staged reveal as win/lose, not the current small/immediate tie-reveal — and adds a NEW bar treatment on win/lose that turns out to be free to wire via infrastructure already built for Dice. Confirmed the root cause of "the yellow rings on both bars" precisely (a pre-existing, unrelated shared mechanism RPS never suppressed), found one real implementation risk in unifying the tie-reveal (your OWN hand can vanish mid-reveal, unlike the opponent's), and flagged one number in the ticket that would introduce a real, visible bug if copied literally            [READY TO TICKET]
From: Designer's own report (relayed by Owner, no screenshots — description references a target screenshot not provided to me), verified via direct reads of `RpsHub.tsx` (`RpsBoard`'s full terminal/tie-reveal logic, the `tieReveal` snapshot mechanism, `PICK_SECONDS`), `GameHub.tsx` (`OwnSlot`'s `ownBarResult`/`gateResultOnReveal`/`onRevealComplete` machinery, the shared `drawBeat`/`suppressDrawBar` mechanism, `outlineForOutcome`), `DiceHub.tsx` (the exact `ownBarResult`+`gateResultOnReveal` wiring pattern this ticket asks RPS to copy), `BlackjackHub.tsx` (`suppressDrawBar`'s only other user), `slotReveal.tsx` (`outlineForOutcome`'s void→null behavior), `packages/games/rps/src/rps.ts` (`PICK_WINDOW_MS`, the tie-replay/`replays` mechanic, `viewFor`'s redaction), and `Full Spec.html:3087-3101`/`3300`/`3521-3533`/`3810-3811`

## This confirms and extends 2026-09-25#2's own open question, doesn't just add new scope

Ticket `2026-09-25#2`'s Part 1 flagged an open question rather than guessing: should the new 700ms pre-flip pause also apply to the tie-replay reveal, given it deliberately stays small today (ticket `2026-09-12#1` item 2). **This ticket answers that definitively: yes.** Designer's own table shows a draw going through the IDENTICAL `reveal`→`flip`→`done` beats as a win or loss, at the same timestamps, only differing in what `done` looks like. Recommending PM treat `2026-09-25#2` and this ticket as ONE combined `RpsBoard` reveal-architecture change, not two sequential patches — the `revealStage` state machine `2026-09-25#2` already recommends needs to be driven by EITHER `terminal` OR an ordinary tie event from the start, not retrofitted after `2026-09-25#2` ships alone.

## The bar treatment is a real addition (confirmed: the prototype never does this), but it's free to wire

**Confirmed Designer's own framing precisely.** `RpsHubScreen`'s own doc comment (my earlier work, ticket `2026-09-12#1` item 3) already establishes the prototype's `mOutcome` is gated on `view === 'mines' || isDice` — never RPS — so the file genuinely never touches RPS's bars. This ticket is explicit that we're deliberately diverging from the file here, which resolves what would otherwise be a real conflict with my own prior "RPS structurally cannot use this mechanism" note — it's not that RPS can't; the prototype just never asked it to, until now.

**The exact mechanism already exists, built for Dice — this needs zero new bar-rendering code.** `GameHub.tsx`'s `OwnSlot` already implements everything Designer describes: a colored ring (`winRingColor`/`lossRingColor`), a fill overlay (`winFillColor`) running the same `WIN_FILL_IN_MS`/hold/fade-out timing, and the exact `you won` text (lowercase, Arial bold 16px, letter-spacing 0.6px, `right:18px`, z-index/positioning all already byte-matching this citation, per ticket `2026-09-17#6`/D28). `DiceHub.tsx` already wires it via `ownBarResult` + `gateResultOnReveal` + calling `onRevealComplete()` from inside its own board when its own custom reveal timing considers itself done. **Recommending `RpsHubScreen` copy this exact pattern**: `ownBarResult`, `gateResultOnReveal`, `winRingColor='#16A34A'`, `winFillColor='#16A34A'`, `lossRingColor='var(--rc-loss)'` (same values `2026-09-25#2` Part 2 already asks for on the card frames) — then have `RpsBoard` call the existing `onRevealComplete` prop the moment its own new `revealStage` reaches `'done'`.

**Confirmed "nothing on a draw" is already the mechanism's default behavior, not something to build.** `outlineForOutcome` (`slotReveal.tsx:17-22`) returns `null` for a `void` outcome — and `phase` never even reaches `'result'` during an ordinary mid-match tie in the first place (`GameHub.tsx`'s own `phase` formula only reaches `'result'` once the match has genuinely ended). So the bar mechanism structurally can't fire on an ordinary tie at all, and produces no verdict on a terminal void either — Designer's "draw: nothing on the bar" is a free consequence of wiring the SAME mechanism Dice already uses, not a separate case to special-case.

## The yellow rings: root cause confirmed, and it's unrelated to anything above

**Not the `barVerdict==='draw'` path — that one's actually inert for RPS today.** Traced it down: `OwnSlot`'s `barVerdict==='draw'` branch (which WOULD default to `ring-amber-400` with no `drawRingColor`) can never fire for RPS today, since `ownBarVerdict` is unconditionally `null` whenever `ownBarResult` is unset (confirmed RPS never sets it, until this ticket).

**The real source: a separate, pre-existing, generic mechanism RPS was never opted out of.** `GameHub.tsx`'s own `drawBeat` (`:762-788`, ticket `#161`) fires a shared orange "draw→rematch" pulse on BOTH bars, universally, off a rise in the public `replays` counter — the ONE generic, game-agnostic tell that "a push just happened," built for every pick-based tie-replay game (Coinflip, RPS, Keno, Limbo, Roulette). `BlackjackHub.tsx` already suppresses this via `suppressDrawBar` for the identical reason Designer now wants for RPS — Blackjack carries its own push treatment on the cards/label instead. **Recommending RPS get the same `suppressDrawBar` flag**, now that draws are shown via the card frames — closes this cleanly, exactly matching an existing, proven precedent rather than inventing a new suppression path.

## The "auto-continue on a draw" needs one real distinction the prototype's own model doesn't have

**The prototype has no replay cap; our server does.** An ORDINARY mid-match tie in `rps.ts` auto-replays server-side with the SAME opponent/escrow — the client never re-matches, matching Designer's "no matchmaking, no Play tap" exactly, since the server already did the work; the client's 1500ms hold is purely a COSMETIC delay before it stops showing the held reveal. But at `REPLAY_CAP`, `rps.ts` sends a genuine terminal `void` outcome — `currentMatchId` clears, `phase` becomes `'result'`, and there is no live match left to auto-continue into. Designer's own citation has no equivalent of this cap (the mock's draws loop forever). **Recommending**: apply Designer's exact `done`-state coloring (both frames `#F79009`) to BOTH cases uniformly, but only auto-revert-and-continue for the ordinary, non-terminal tie — the cap's terminal void should behave like the win/lose case's own "no automatic return," staying expanded until Play is pressed for a genuinely new match via normal matchmaking. This is a real, structural difference the ticket doesn't address (since it can't, from the prototype's own simpler source) — flagging rather than guessing which way Designer would want the (extremely rare, cap-triggered) case handled.

## One real implementation risk found while checking this is buildable, not assumed safe

**Your OWN hand can currently vanish mid-tie-reveal — the opponent's side already solved this, yours doesn't.** The tie-reveal's existing `revealedChoices` payload snapshots BOTH players' throws for the round that just ended (confirmed in `rps.ts`'s `resolve()`), but the current code only reads the opponent's half (`revealed?.[opponentId]`) for `tieReveal.choice` — never your own. Your own card's hand icon is driven by live `myChoice` (`optimisticPick ?? serverChoice`), and the round-bump reset effect nulls `optimisticPick` the instant the tie fires, with the fresh round's own `choices` already cleared server-side — so `myChoice` goes blank immediately, before any of the new staged-reveal delays even begin. Designer's own spec requires "your hand shows on your card at 1.6 scale" sustained through the WHOLE reveal — recommending `RpsBoard` also snapshot `revealed?.[playerId]` (already present in the same payload) the same way the opponent's side already does, rather than continuing to read live `myChoice` during a tie's own reveal window.

## One number that would introduce a real, visible bug if copied literally

**The clock citation is a mock value, not the real server window — flagging before anyone copies it.** Designer's own note: "The clock is showing 10. It's 7 (line 3300)." Checked `rps.ts` directly: the REAL server-enforced pick window is a genuine `PICK_WINDOW_MS = 10_000` — a live, authoritative 10-second deadline the server actually locks on. The prototype's "7" is a scripted mock value with no real backend behind it. If the client's own `PICK_SECONDS` display constant changed to 7 while the server still locks at 10, the countdown would hit 0 a full 3 real seconds before the round actually resolves — a new, visible, confusing bug (clock says zero, nothing happens). **Recommending: do NOT change `PICK_SECONDS` to 7** unless Owner/Designer also wants to shorten the REAL server-side `PICK_WINDOW_MS` to match — that's a gameplay-pacing decision, not a cosmetic display fix, and needs an explicit answer rather than a silent copy of the prototype's number.

## Already covered

**"You (Demo88)" and "Forfeit" in the screenshot** — already ticketed and shipped (not yet deployed) as `2026-09-25#1`/D47. Once that deploys, this specific screenshot detail resolves on its own; no new action here.

---

**Ask:** the bar-treatment wiring, `suppressDrawBar`, and the your-own-hand snapshot fix are all safe to implement as scoped. Two things need an explicit answer before implementing, not a guess: (1) should the rare terminal void-at-replay-cap get the same orange `done` coloring but WITHOUT auto-continuing (my recommendation), or should Designer's exact loop somehow still apply; (2) should `PICK_SECONDS`/the real `PICK_WINDOW_MS` actually shorten to 7s, or should the display stay matched to the real 10s window as it is today. Recommending this ticket and `2026-09-25#2` ship together as one `RpsBoard` change, given how intertwined the reveal-stage timing now is between them.

---
### 2026-09-25#2 — D48, Designer's RPS reveal-sequence package, plus a green-and-red unification rule for RPS/Mines/Dice. Part 1: the terminal reveal currently does everything in one step (grow, choices-collapse, flip, frame-color) instead of the design's three staggered beats — confirmed most of Beat 1 is ALREADY correct, pinned down exactly what's missing (the 700ms pre-flip pause, the flip card's own translateX nudge, the deferred frame-color, a clock fade). Part 2: one win-green (`#16A34A`) and one loss-red (`var(--rc-loss)`) across all three games — Mines and Dice are ALREADY fully compliant (checked directly), only RPS still needs the update, plus one real ambiguity flagged rather than guessed            [READY TO TICKET]
From: Designer's own report (relayed by Owner, no screenshots), verified via direct reads of `RpsHub.tsx` (`RpsBoard`'s full terminal/tie-reveal logic, `RpsRevealFlipCard`, `FRAME_WIN`/`FRAME_LOSE`, the choice-tile ring), `GameHub.tsx` (`OwnSlot`'s `winRingColor`/`winFillColor`/`lossRingColor` gating, RPS's own doc comment on why it opts out of the bar-level mechanism entirely), `DiceHub.tsx` (`DICE_WIN_GREEN`/`DICE_FILL_GREEN`), `MinesHub.tsx`/`MinesPlay.tsx` (grepped clean), `index.css` (`--rc-green`/`--rc-loss` token definitions), and `Full Spec.html:606-642`/`3083-3100`/`3263-3270`/`3803-3813`

## Part 1 — the reveal sequence

**Most of Beat 1 is already correct — checked precisely rather than assumed broken.** `terminal` (`outcome != null`) already drives, correctly: card width/height 92→124 / 130→176 over 620ms (`cardW`/`cardH`, ticket `2026-09-12#1` item 2), the gap 36→12 and VS column 56→38 (same transition), the choices wrapper's opacity/max-height collapse (`:532-539`, exact transition list match), and the hand icon's scale 1→1.6 (`:451`). None of that needs to change.

**What's actually missing or wrong, confirmed by reading `RpsBoard`'s exact terminal-swap logic:**

- **No 700ms pause before the flip.** `terminal` alone decides BOTH "grow the cards" (Beat 1) AND "which component renders" — the moment `outcome` arrives, the ternary at `:476-514` swaps straight to `<RpsRevealFlipCard key="terminal">`, which starts its own 820ms `rotateY` animation on that same render. Beats 1 and 2 fire simultaneously today — this is precisely "the choices vanish and the cards flip in one step" the ticket opens with.
- **The translateX 16px nudge-and-return doesn't exist at all.** Checked `RpsRevealFlipCard` (`:640-674`) end to end — no `translateX`, no second transform of any kind on the outer box. This beat is entirely absent, not just mistimed.
- **Frame color changes the same instant as everything else, not ~900ms after the flip starts.** `myFrame`/`oppFrame` (`:352-356`) derive directly from `outcome`, unconditionally — there's no staging that defers the color swap to Designer's Beat 3 (~1600ms from clock-zero, i.e. ~80ms after the 820ms flip's own animation completes).
- **The clock digit never fades.** No opacity/scale transition tied to `terminal` anywhere in the VS column (`:458-474`) — the design calls for opacity 1→0, scale 1→0.55 alongside everything else in Beat 1.

**Recommended shape, reusing this file's own existing clean-timer patterns (the `tieReveal` effect already does exactly this kind of staged, torn-down-on-unmount timing):** replace the single `terminal` gate for the reveal's OWN internal staging with a small state machine — e.g. `revealStage: 'grow' | 'flip' | 'done'` — driven by a `useEffect` on `terminal` that arms two chained `setTimeout`s (700ms → `'flip'`, then +900ms more from `'flip'` → `'done'`; reset to nothing when `terminal` goes false, e.g. a new match starts). Keep `terminal` itself driving Beat 1 (size/gap/choices/hand-scale — unchanged, already correct). Gate the `RpsRevealFlipCard` MOUNT on `revealStage !== 'grow'` instead of `terminal` directly (so the redacted `RpsFrame` stays put through the 700ms pause even though the cards have already grown). Gate `myFrame`/`oppFrame` on `revealStage === 'done'` instead of `outcome` directly (staying `FRAME_NEUTRAL` through `'grow'`/`'flip'`). Add the clock's own opacity/scale transition keyed on `terminal` directly (Beat 1, no staging needed there).

**Add the translateX nudge inside `RpsRevealFlipCard` itself** (a local `useState`+`useEffect`, `slide: true` on mount → `translateX(16px)`, `transition: transform 410ms ease-in-out`, flipping to `false`/`0px` at exactly +410ms) — since this component is shared by both the terminal reveal AND the mid-match tie-reveal, this one change benefits both call sites for free, matching Designer's own description of the nudge as intrinsic to the flip mechanic itself.

**One open question, flagged rather than decided unilaterally:** should the NEW 700ms pre-flip pause also apply to the tie-replay reveal (`tieReveal`, `:493-506`)? That path deliberately does NOT grow the cards — a separate, still-valid, well-reasoned decision from ticket `2026-09-12#1` item 2 (avoiding a fight between the terminal-only growth and the tie-reveal's own 1.5s hold/reset timer) that this ticket doesn't ask to revisit. Since there's no "grow" beat to pause after in the tie case, I'm recommending its flip keep starting immediately, as today — only the shared translateX nudge applies there automatically. Flagging in case Designer intends the pause universally.

**Minor, low-risk note:** the citation's `height:266px` fixed card-container isn't literally present in this port — the existing flex-column layout should reproduce the same vertical-settling effect as the choices row collapses, without a hard height, but I have no browser tooling in this environment to confirm visually; worth a quick live look once implemented.

## Part 2 — one win-green, one loss-red, across RPS/Mines/Dice

**Mines and Dice are ALREADY fully compliant — checked directly, not assumed.** `DiceHub.tsx`'s `DICE_WIN_GREEN` (`#16A34A`) already drives the winning cube number and the winning history pill (ticket `2026-09-16#4` item 4). `GameHub.tsx`'s `OwnSlot` already gates `winRingColor`/`winFillColor` to `#16A34A` and `lossRingColor` to `var(--rc-loss)` (`#FF3E5E`) for `gameId === 'dice' || gameId === 'mines'` (tickets `2026-09-16#4`/`2026-09-16#7`/`2026-09-17#6`/D28). Grepped `MinesHub.tsx`/`MinesPlay.tsx` for `34D399`/`22C55E`/`0B8F5A` directly — zero hits. Nothing to change on either game.

**RPS is the only game that actually needs this ticket, and it's three precise, scoped edits:**

- `RpsHub.tsx:64`, `FRAME_WIN`: `#34D399` → `#16A34A`.
- `RpsHub.tsx:65`, `FRAME_LOSE`: `#F04438` → `var(--rc-loss)` (`#FF3E5E`) — Designer's own explicit correction ("the file has ... `#F04438` for the loss here — both replaced"), matching the SAME token Dice/Mines already use for their own bar loss ring.
- `RpsHub.tsx:563`, the selected choice tile's ring: `var(--rc-green)` → literal `#16A34A` in both themes. Carry the SAME zero-alpha-at-the-same-hue technique ticket `2026-09-22#4`/D41 deliberately used for the unselected state (`rgba(52,211,153,0)` is `#34D399`'s RGB at zero alpha, specifically to avoid a hue-shift during the box-shadow transition) — update it to `rgba(22,163,74,0)` (`#16A34A`'s RGB), not just the selected value alone.

**Confirmed RPS's player bar needs NO change — this is a real "already satisfied," not an oversight.** `RpsHubScreen`'s own doc comment (`:687-693`, my own earlier work, ticket `2026-09-12#1` item 3) already establishes RPS structurally never uses the bar-level win-fill mechanism at all — the prototype's own `mOutcome` is gated on `view === 'mines' || isDice`, never RPS. RPS's real win signal is the card frame (covered above); there is no `winRingColor`/`winFillColor` wiring to add for RPS.

**`--rc-green` itself is correctly out of scope, confirmed rather than assumed.** Its only other consumers in these three files are doc-comment prose, not live styling — Designer's own carve-out (stake amounts, LIVE dot, gem captions, XP, rakeback) holds; the token definition in `index.css` should NOT change.

**One real ambiguity, flagged rather than guessed either way:** `DiceHub.tsx:49`, `DICE_FILL_GREEN = '#22C55E'` — the sliding progress-fill on BOTH players' tracks during a live roll (not itself a win/loss signal; it's the same color on both bars regardless of who eventually wins, more a "counting in progress" indicator). Designer's own itemized Dice list names only two mechanisms as needing `#16A34A` ("the higher cube number, the win pill in the history belt") and doesn't mention this fill — but the ticket's own literal grep check (`grep -rn "34D399|22C55E|0B8F5A"`) would still flag it, since it's a real, matching literal in one of the three game components. I can't tell from the ticket which reading is intended — recommend confirming with Designer before touching it either way, rather than guessing.

---

**Ask:** Part 1's staged-timer restructure and the RPS-only color edits in Part 2 are both safe to implement as scoped above. Two open items need a decision, not a guess: (1) should the tie-replay reveal also get the new 700ms pre-flip pause, or stay immediate as today; (2) should Dice's progress-fill green (`#22C55E`) also move to `#16A34A`, or stay as its own separate, non-win-state color. Happy to relay either answer back into the ticket once you have it.

---
### 2026-09-25#1 — D47, Designer's RPS package, three fixes. (1) delete every caption + the Forfeit button/handler — none of that text exists in the prototype; (2) a correction to Designer's own earlier RPS-tiles note — the choice icons are ALWAYS purple, never white/grey, confirmed directly against the prototype's hardcoded SVG fills; (3) an opponent card-back bolt reported off-centre on a second round — confirmed this build's architecture has NO equivalent of the prototype's persistent, resettable transform state the citation describes, but found a real, different, adjacent bug in the same area worth checking against instead            [READY TO TICKET]
From: Designer's own report (relayed by Owner), screenshots at `design-ref/D47/`, verified via direct reads of `RpsHub.tsx` (`RpsBoard`, `RpsHandIcon`, `RpsFrame`, `RpsRevealFlipCard`, `RpsPanel`), `GameHub.tsx` (the shared `phase` formula and its match-boundary bridging), `packages/games/rps/src/rps.ts` (`round`, `forfeit`, `scheduledDeadlines`/`timeoutMove`), and `Full Spec.html:605-658`/`3294`/`3805-3809`

## Item 1 — remove every caption + Forfeit, confirmed exactly as reported

All three strings are real and exactly where Designer says: `RpsHub.tsx:518` (`You (username)`), `:573` (`Picked {myChoice} — tap another to change, or wait for the timer`), `:585-587` (the `Forfeit` button, `onClick={onForfeit}`). None of them exist in the prototype's own `isRps` markup — confirmed directly.

**This does reverse an explicit prior Owner decision, flagged rather than silently overridden.** The `Forfeit` button's own doc comment (`:575-583`) records ticket `2026-09-12#1` item 4/3: Owner's own call was to keep the FUNCTION ("a stuck player must still be able to leave a bad match") while restyling it down from a first-class action, precisely because the prototype itself never renders a visible forfeit control either. Designer's new ask is stronger — delete the code path, not just the visual weight.

**Checked whether RPS specifically still needs that safety valve, and it doesn't.** `rps.ts`'s `scheduledDeadlines`/`timeoutMove` (:249-270) is a generic, engine-level per-round deadline: at window close, the core auto-injects a deterministic seeded auto-throw (`autoThrowFor`) for ANY still-unlocked player, connected or not. Every round resolves on the clock regardless of human input — RPS structurally cannot hang waiting on a player the way Chess/Mines/Blackjack can (none of which have a per-move auto-substitution). A player who just walks away doesn't strand their own escrow or the opponent's; the match keeps auto-resolving to a decisive winner (or void at the tie-replay cap) with zero manual intervention. Owner's original rationale was sound in general, but RPS's own clock-driven design was seemingly swept into the same styling call without this per-game distinction being drawn out at the time — recommending we now honor Designer's fuller ask for RPS specifically, on this basis.

**Scoped narrowly, checked directly:** `onForfeit` is shared `GameAreaArgs`/`GameHubScreenProps` plumbing used by Chess/Mines/Blackjack/`Play.tsx` too — only `RpsBoard`'s own local button + destructuring should go, not the shared prop/interface or `App.tsx`'s `handleForfeit`/`ws.forfeit()`.

## Item 2 — icons are always purple; correcting Designer's own earlier note

Confirmed exactly as Designer's correction states. `RpsHandIcon` (`:85-125`) bakes every fill as a hardcoded literal (`#B285F7`/`#7B3BE0`/`#8B45F0`, etc, ported byte-for-byte from `Full Spec.html:611-654`) — no `currentColor`, no state-driven binding on the icon itself in the prototype's own markup. The ONLY thing this build got wrong: ticket `2026-09-22#4`/D41 added an optional `color` param and wired it at the live picker's call site (`:566`, `color={myChoice === id ? '#FFFFFF' : 'var(--rc-muted)'}`) — implementing Designer's OWN prior citation at the time (the prototype's `rpsChoice()` computes that exact white/muted pair, `Full Spec.html:3244-3256` — genuinely present in the prototype's source, just never consumed by its own markup, which D41's own doc comment already flagged as "dead code in the prototype's own source... applied here as a deliberate ask" per Designer's citation then). Since Designer is now correcting that earlier instruction, not finding a fresh bug: drop the `color` prop from the `:566` call site (back to plain `<RpsHandIcon choice={id} size={54} />`, matching every other call site in this file already). The tile's own ring (`:563`, `inset 0 0 0 3px var(--rc-green)` / zero-alpha) is untouched — Designer confirms that part was always correct.

## Item 3 — the reported bolt-centering bug: the exact prototype mechanism doesn't exist here, but a real, different bug in the same area might explain the symptom

**Checked precisely, not assumed: this port has no equivalent of `rpsFlipX`/`rpsFlipRot` at all.** The prototype's bug class (one persistent flip-card element, two pieces of state that must reset each round) doesn't map onto this codebase's architecture. Here, each of the three opponent-card states — redacted (`RpsFrame` + `RpsRedactedIcon`, no perspective/flip, `:508-513`), tie-reveal (`RpsRevealFlipCard key={tieReveal.seq}`, `:498-506`), and terminal-reveal (`RpsRevealFlipCard key="terminal"`, `:484-492`) — is a STRUCTURALLY DIFFERENT React element at the same JSX position, swapped via ternary. A transition between any two of them is a full unmount-then-mount, never a live re-render of retained transform state; `RpsRevealFlipCard`'s own `rotateY` is a hardcoded `initial={0}`/`animate={180}` literal on `motion.div` (`:652-654`), fresh every mount by construction, and the bolt's own parent boxes are plain `flex items-center justify-center` wrappers with no transform of their own at all. I can't find a code path that reproduces "one of two transforms didn't reset" here — there's nothing stateful to fail to reset.

**A real, different, adjacent bug found while checking whether `RpsBoard` can even persist across two matches — worth checking against instead.** `RpsPanel` (`:610-620`) mounts `RpsBoard` with no `key`, and `renderGameArea(areaArgs)` (`GameHub.tsx:872`) is a plain function call, not a keyed element — so `RpsBoard` is the SAME React instance across two consecutive matches whenever `GameHub`'s own `phase` formula never dips through `'idle'`/`'waiting'` in between (its own comment at `:602-613` documents exactly this kind of one-render bridging elsewhere). RPS is one of only two games passing `searchFloorMs={0}` (`:537-539`, "Coinflip/RPS... #387") — no artificial search delay — so an instant rematch against an already-resting bot is a real, plausible way to hit this. When it happens: `rps.ts`'s own `round` field (`:169`) starts at a fixed `0` for every new match, incrementing only on a tie-replay (`:123`) — so two consecutive ties-free matches (the common case) both sit at `round: 0`. `RpsBoard`'s `optimisticPick` reset effect is keyed on `[round]` (`:379-381`) — if `round` never changes across the match boundary, that effect never re-fires, and the PREVIOUS match's pick stays selected (ring-highlighted) into the new match until the player taps again. Not what Designer described, but the same general area, genuinely real, and would survive item 1's caption removal (the ring would still be wrong, just silently).

**Recommending, in order:** ship items 1 and 2 as described (both fully confirmed, zero risk). For item 3, key `RpsBoard`'s mount off the match itself (e.g. `currentMatchId ?? 'idle'`) rather than patching `round`'s dependency array alone — this forces a fresh instance per match unconditionally, closing the `optimisticPick` staleness bug AND making the (currently-unreproducible) bolt concern structurally impossible going forward, in one change. Given I have no browser tooling in this environment, ask whoever implements to run Designer's own frame-capture-and-diff check on the current build first — it's possible the original report was against a stale cached PWA bundle (the exact class of bug ticket `2026-09-22#5` already found and fixed once this project) rather than anything in `main` today.

---

**Ask:** Items 1 and 2 are safe to ship as-is — no open questions. Item 3: please confirm live whether the reported symptom still reproduces on the current build before assuming a fix is needed; if it does, the `RpsBoard` per-match key above is the recommended fix regardless of which exact mechanism turns out to be responsible.
...
## Fix 1 — Designer's own diagnosis, confirmed exactly, unconditionally worth doing regardless of root cause

**Checked directly: there is no horizontal clip anywhere on the app shell.** `apps/web/src/index.css`'s own `@layer base` rules for `*`/`body` set only `border-border`/`outline-ring` and `bg-background`/`text-foreground` — no `overflow-x` anywhere on `html`, `body`, or any root wrapper. Designer's own structural read is exactly right: the prototype's fixed 390px `overflow:hidden` phone-frame (`Full Spec.html:159`) has no equivalent in this build (correctly so — this build is a real fluid-width responsive app, not a fixed frame), so nothing stops an over-wide descendant from widening the whole document and forcing a horizontal scroll.

**Recommending Designer's own fix exactly:** `overflow-x: hidden` on the app's own top-level scroll container. Zero-risk, purely defensive — this can only ever prevent an existing bug's visible symptom, never change anything for a layout that's already correctly sized.

## Fix 2 — Designer's two suspects, checked against this build's ACTUAL code (not the prototype's)

**Suspect 1 (the history belt) doesn't match as literally described, but pointed to a real, different, confirmed hazard in the exact same component.** This build's `DiceHistoryBelt` (`DiceHub.tsx:248-294`) doesn't use the prototype's `translateX`-shift trick at all — its own doc comment explains it was replaced with Framer Motion's `layout`/`AnimatePresence` for the same visual result. Its wrapper already has `overflow-hidden` (`:250`) — Designer's literal suspicion (a missing clip on the belt's own wrapper) doesn't apply here.

**What I found instead, checked precisely:** each pill is `flex: '0 0 calc((100% - 32px) / 5)'` (`:268`) — `flex-shrink: 0`, meaning a pill never shrinks below its "1-of-5" width. `history` is capped at 6, not 5 (`:532`, `.slice(0, 6)`) — specifically so a 6th (evicted) pill can still render its exit animation under the wrapper's `overflow-hidden` rather than vanishing abruptly. This is a well-known CSS interaction: `overflow: hidden` clips *painting*, not *layout sizing* — a flex item with no explicit `width`/`min-width` (the outer `dice-history-belt` div is `mt-auto`, implying it's a flex item in a column, not a plain block) can still be pulled wider during its own shrink-to-fit sizing pass by non-shrinking descendants, even though the excess is visually clipped once that size is settled. When a 7th pill needs evicting (the array exceeds 6 and AnimatePresence keeps the exiting one mounted alongside the new 6), 7 non-shrinking "1-of-5" pills briefly demand more width than the wrapper's own intended size — pulling the wrapper's actual layout width wider, not just its paint. `BELT_DURATION_S = 0.46` (`:96`) — 460ms — matches Designer's own observed timing exactly, not approximately.

**The honest gap, flagged rather than resolved by assumption:** per this exact mechanism, the earliest point 7 pills can ever coexist is round 7 (history needs to already hold 6 before a 7th eviction fires), not round 2 as Designer reports. I don't have live browser access in this environment to reproduce and confirm which round it actually starts at, or use Designer's own suggested DOM-inspector/outline technique to nail the exact overflowing element. Flagging this precisely rather than either (a) silently accepting "round 2" and mis-describing this mechanism as its cause, or (b) silently substituting "round 7" for Designer's own reported "round 2" without saying so.

**Suspect 2 (the Play-button shake) is definitively ruled out — confirmed via direct code read, not assumption.** `playShake` (`GameHub.tsx:1437-1460`) is only ever set from `guideToBet()`, which is only ever called when `armedStake == null` (`:1463`) — i.e., only on a REJECTED Play press (no bet armed), never on a successful one that actually starts a round. By the time a round genuinely starts, this code path has never run. Not the cause.

## Recommended shape

Ship Fix 1 (global `overflow-x: hidden`) regardless — it's correct, needed, and safe on its own even if it doesn't turn out to be the full story. For Fix 2, add `min-width: 0` to the belt's outer flex-item wrapper (the standard fix for this exact class of flex/overflow interaction) since it's a real, confirmed bug worth fixing either way — but ask whoever implements this to use Designer's own suggested DOM-inspector check on an ACTUAL round 2 (not round 7) to confirm whether the belt is really what's crossing the edge at that specific, earliest-reported moment, or whether something else entirely is responsible for the round-2 case specifically. If it turns out to be something else, Fix 1 alone (already being shipped) should still fully suppress the visible symptom while that separate root cause gets its own follow-up.

---

**Ask:** Fix 1 is safe to ship immediately, no further investigation needed. For Fix 2, flagging the live-reproduction gap explicitly — whoever implements this has real access to reproduce the actual bug in a browser, which I don't have here; worth 5 minutes with Designer's own outline-trick before assuming the belt explains the round-2 case specifically, even though the belt bug itself is real and worth fixing regardless.
...
Owner asked for a way to bound the DB size further, beyond 2026-09-24#1's already-shipped meaningless-pair pruning — floated two candidate rules (a per-account transaction count cap, or a global time window covering everything) and asked me to propose the actual mechanism.

## The correctness trap, checked directly before proposing anything

**`getBalance(accountId)` is `SELECT COALESCE(SUM(amount), 0) AS total FROM ledger_entry WHERE account_id = ?` — confirmed directly in `ledger.ts`.** There is no separate stored balance anywhere; every account's current balance is derived, on every read, from the complete sum of its own history. **This means a real transaction (a `GRANT`, `SETTLE_WIN`, `RAKE`, `ADMIN_CREDIT`, `REWARD_CLAIM`) can never simply be deleted** the way 2026-09-24#1 deletes meaningless `BET_ESCROW`/`SETTLE_REFUND` pairs — those sum to exactly zero, so removing them is money-neutral by construction; deleting a real `+1000 GRANT` from weeks ago would permanently understate that account's balance by 1000, with no way to recover the true number. This is exactly why 2026-09-24#1 was scoped as narrowly as it was.

## The safe mechanism — a standard ledger "checkpoint" pattern, not a novel technique

**Before deleting a batch of an account's own real rows older than the retention cutoff, first write one new synthetic entry whose `amount` equals the exact sum of everything about to be deleted.** New `LedgerEntryType`: `'OPENING_BALANCE'` (NULL `matchId`, same shape as `ADMIN_CREDIT`/`REWARD_CLAIM` — not tied to one settlement). The account's own computed balance is mathematically unchanged: `SUM(everything)` before compaction equals `SUM(the new compaction entry) + SUM(everything still kept)` after. This is the standard technique any ledger/accounting system uses to bound its own history without losing correctness — not something invented for this ticket.

**The compaction entry is itself eligible for the NEXT compaction round, for free, with no special-casing.** Stamp its own `created_at` at the moment of compaction (now), not backdated to the cutoff — so it correctly reads as "fresh" and isn't immediately swept again in the SAME pass. A second compaction round N days later naturally sums the PRIOR compaction entry together with whatever's newly aged out since, replacing both with one updated entry — an account only ever holds at most one `OPENING_BALANCE` row at a time, self-maintaining, no accumulation of its own.

## Time window over per-account count — a concrete reason, not just simplicity

**A global time cutoff reuses 2026-09-24#1's own exact shape** (the same hourly-timer/batching/VACUUM machinery already shipped) — "compact every account with any row older than N days" is one simple, uniform rule. **A per-account count cap ("keep each player's last 100") needs a window-function query and immediately raises a real ambiguity Owner's own phrasing doesn't resolve**: does "100 transactions" mean 100 ledger rows, or 100 matches played (one match produces 2-4 rows)? The time-window version has no equivalent ambiguity. Recommending the time window; Owner's own number (10 days) as the default, env-var tunable matching the codebase's existing convention (`LEDGER_COMPACTION_RETENTION_DAYS`, alongside `LEDGER_CLEANUP_RETENTION_DAYS`).

## One real, checked consequence — flagged plainly, not silently decided either way

**`match_history.ts`'s own `matchNetStmt` (`SUM(amount) WHERE match_id = ? AND account_id = ?`) computes the win/loss delta shown for a match in `GET /matches/recent`.** Once a match's own `ledger_entry` rows are compacted away, this returns `NULL` → the existing `?? 0` fallback (`match-history.ts:421`) — so a sufficiently old match would show a `net` of 0 instead of its true historical amount. **Checked whether this is actually reachable, not just theoretical:** `getRecentMatches`'s own `limit`/`offset` pagination caps `limit` at 50 per page, but `offset` itself has no ceiling — a client paging back far enough into an account's own full match history (still intact forever in the never-pruned `match_results` table) WOULD reach a compacted match and see `net: 0` there, even though `match_results` itself still correctly shows that match's real `outcome`/`stake`/`winner_id`. Only the profit/loss delta on a very old match would go stale — matches Owner's own explicit framing ("I do not imagine we could be digging into any longer history"), but flagging the precise, verified shape of the tradeoff rather than assuming it's fine.

## Scope

`packages/shared/src/protocol.ts` (`LedgerEntryType` gains `'OPENING_BALANCE'`), `packages/core/src/ledger.ts` (new compaction function, `EphemeralLedger` gets the same real-vs-stub treatment 2026-09-24#2 gave `getOpenEscrowMatchIds` — guest mode is a much smaller, session-bounded world, PM's call on whether it needs this at all or can stay a documented no-op), `apps/server/src/ws/gateway.ts` (new timer, same `setInterval(...).unref()` idiom, batched the same way `cleanupSettled` already is — one account compacted at a time or in small batches, yielding between them, for the same reason 2026-09-24#1's own DELETE is chunked). Should trigger `snapshotter.trigger()` the same way any other real write does.

**Given this now touches every account's REAL transaction history, not just meaningless bot noise, this needs the same rigor 2026-09-24#1 got before it's trusted:** a test proving `getBalance()` is byte-identical before and after compaction for real win/rake/grant data specifically (not just the already-proven escrow-pair case), and a test proving a second, later compaction round correctly folds a prior `OPENING_BALANCE` entry into the next one rather than accumulating multiple.

---

**Ask:** no urgency flagged by Owner on this one either — treat like 2026-09-24#2, queue normally. Worth a live-data sanity check once shipped: pick a handful of real accounts, compare `getBalance()` before and after the first compaction pass actually runs, byte-for-byte.
...
Owner's own re-test after the service-worker fix went live is exactly what surfaced this — sound was STILL broken (ruling out staleness as the explanation this time), and the specific detail of sound "catching up" right at a screen-unlock moment is precise enough to trace to an exact mechanism, not just a hunch.

## The mechanism, confirmed directly in the code

**`unlock()`/`context.resume()` — the only thing that ever revives a suspended `AudioContext` — is wired to fire EXACTLY ONCE per page load, ever.** `installUnlockOnFirstGesture()` (`sound.ts:175-184`) installs a global `pointerdown`/`keydown` listener; the handler calls `unlock()`, then immediately **removes both listeners** (`:180-181`). This is by design for the ORIGINAL problem it solves (the very first mobile-autoplay unlock) — but it means nothing in the app EVER calls `resume()` again for the rest of that page's lifetime, no matter how many later gestures happen.

**`play(name)` itself has no fallback — it silently no-ops if the context isn't `'running'`** (`:157`, `if (!context || context.state !== 'running') return;`), by design (never force autoplay) — but combined with the above, this means: the FIRST time a mobile browser suspends the AudioContext after that initial unlock — which is ROUTINE, EXPECTED behavior on the browser's own part (locking the screen, backgrounding the tab, some OS-level audio interruptions all commonly trigger this) — sound goes silently, permanently dead for the rest of that session. Every subsequent tap, PLAY press, tile reveal, is a normal gesture that does nothing to revive it, because the ONE listener that could have was already removed.

## This precisely matches every detail of the re-test, not just loosely

- **Chess and Mines sound both still silent after the SW fix** — expected: this is a genuinely different bug, the SW fix was correct for what it fixed, this is a separate gap the re-test correctly surfaced.
- **"Play a Friend" (a universal, purely client-side `play('reject')` trigger, no server round-trip needed at all) was ALSO silent** — strong confirmation this is an `AudioContext`-level issue, not anything specific to Mines/Chess's own sound-triggering logic; the SAME shared `play()` function is used everywhere.
- **Sound "caught up" the instant the phone was unlocked from its screensaver** — consistent with the browser's own visibility-return behavior (re-focusing a backgrounded page is one of the few paths that can implicitly interact with a suspended context, and/or with the batch of already-queued game-state updates the client couldn't process while backgrounded finally running through and each correctly firing its own sound now that some other event happened to revive the context around the same moment) — the PRECISE trigger for that one instance is less important than the underlying gap it reveals: this app has no INTENTIONAL mechanism to recover from a suspension at all.

## A second tester's report ("some Mines sounds present, but not the explosion") — checked for a separate root cause, found none; consistent with the same bug

**Checked precisely, not assumed, that this isn't a repeat of the D31 server bug (a terminal-branch field silently missing).** `viewFor`'s terminal branch (`mines.ts:307-311`) spreads the RAW board object directly (`{...s.boards[p], score: ...}`), and `bustedOn` is a native field on that raw object (`:29`) — it survives into the terminal view for free, unlike `score`, which needed the explicit D31 fix. The NON-terminal branch's own `myView` construction (`:317-322`) also correctly includes `bustedOn` the instant it's set server-side. No analogous gap found for `bustedOn` specifically.

**Checked the sound-trigger condition itself (`MinesHub.tsx`'s D39 diff effect) — correct on direct re-read.** `if (me?.bustedOn !== undefined && seen.bustedOn === undefined) play('mines-mine');` fires exactly once per genuine bust, correctly scoped. **Checked the asset file itself for a possible codec/corruption issue** — `mines-mine.mp3` is a standard MPEG Layer III file, same format/bitrate/sample-rate as every other sound asset already confirmed working (`play.mp3` etc.) — ruled out a bad-file explanation.

**Given both of those come back clean, the most likely explanation is that this is the SAME AudioContext-suspension bug, not a separate one** — which sound happens to be silently dropped depends entirely on TIMING (whichever `play()` call happens to fire right after a suspension event), so "gems worked, the explosion specifically didn't" for one particular round is exactly the kind of INCONSISTENT-per-sound symptom this bug would produce, not evidence of a code-level gap specific to the mine sound. Flagging this as the most likely shared explanation rather than a confirmed second finding — if it recurs even with the fix below in place, worth a fresh look.

## Fix

**Standard, well-established correction for this exact class of Web Audio gap: listen for the page regaining visibility/focus, and call `unlock()` again.** Concretely, add a `document.visibilitychange` listener (checking `document.visibilityState === 'visible'`) inside `sound.ts`, calling `unlock()` — which is ALREADY idempotent and safe to call repeatedly per its own existing doc comment. Browsers' autoplay-gesture requirement applies to the FIRST-ever `resume()` call on a context; once a context has been genuinely unlocked by a real user gesture even once (which the existing one-time listener already guarantees), subsequent `resume()` calls from non-gesture contexts like a visibility-change handler are standardly permitted — this is a widely-documented pattern, not a novel workaround.

**Scope, precisely:** this is a `lib/sound.ts`-only change — no changes needed to Mines' own D39 sound logic, Chess's existing sound wiring, or anything server-side. A single new listener, installed once (alongside where `installUnlockOnFirstGesture()` is already called), covers every sound in the app uniformly.

---

**Ask:** small, well-precedented fix, scoped to one file. Given how central `AudioContext` suspension is to mobile browser behavior generally, worth a live-device re-test specifically involving a screen-lock/unlock cycle mid-session, not just a fresh page load — that's the scenario this bug (and this fix) is actually about.

---
### 2026-09-22#5 — service worker never actually auto-updates: Owner's own report (Mines/Chess sound intermittently missing, "closing the browser fixed it temporarily") traced to a real, confirmed configuration gap — the app's own PWA update code claims "automatic skip-waiting + reload, no user prompt needed" but, verified by rebuilding the app and reading the ACTUAL generated service worker plus the exact client library source three layers deep, nothing in the current setup ever sends the signal that would make that true. A newly-deployed version sits fully downloaded but inert until every tab is fully closed — explaining the intermittent stale-code symptom precisely, including Designer's own independent "older-looking code" reports from yesterday            [READY TO TICKET]
From: Advisor   Re: Owner's own direct report (relayed live: intermittent missing sound in Mines and Chess, temporarily fixed by fully closing and reopening the browser), verified by rebuilding `apps/web` fresh and reading the literal generated `dist/sw.js`, `apps/web/src/main.tsx`'s registration call, `vite-plugin-pwa@0.21.2`'s own `registerSW()` client template (`dist/client/build/register.js`), and `workbox-window@7.4.1`'s own `Workbox` class source — traced the exact control-flow gap through all three layers, not inferred from behavior alone

Owner asked me to analyze what could be going wrong after noticing intermittent missing sound (Mines and Chess both), self-correctly hypothesizing "some code fallback version" and independently correlating it with Designer's own separate report of occasionally seeing older-looking code. Investigated and confirmed the mechanism precisely — this is a real, verifiable configuration bug, not a guess.

## The app is a PWA whose service worker precaches the ENTIRE app shell — including `index.html` itself

**Rebuilt `apps/web` fresh (`npx vite build`) and read the literal generated `dist/sw.js`.** Confirmed: `s.precacheAndRoute([{url:"index.html", revision:"..."}, ..., {url:"assets/index-[hash].js", revision:null}, ...])`, followed by `s.registerRoute(new s.NavigationRoute(s.createHandlerBoundToURL("index.html")))`. This means **every page load is answered directly from the service worker's own cache, never touching the network at all** once the service worker is installed — the entire app shell (HTML + JS + CSS) is served from a frozen snapshot taken at whatever moment that service worker was built.

**This is why `2026-09-21#11`'s own cache-header fix doesn't help here — it was never the relevant layer.** That fix correctly keeps `index.html`/`sw.js` at `Cache-Control: max-age=0` (always revalidate) at the HTTP layer — but a `NavigationRoute` bound to a precached response means the request is intercepted and answered by the service worker's `fetch` handler BEFORE it would ever reach the network or consult that header. Two entirely separate caching layers; the HTTP one was already correctly fixed days ago, the service-worker one was never touched.

## The real gap: nothing in the current setup ever tells a newly-downloaded version to take over

**`main.tsx`'s own comment (`:24-27`) claims: "hourly polling for a new service worker, and — per autoUpdate's default behavior — automatic skip-waiting + reload when one is found, no user prompt needed."** Checked whether this is actually true by tracing the literal control flow through all 3 layers involved, not assuming the comment is correct:

1. **`main.tsx:28-33`** calls `registerSW({ immediate: true, onRegisteredSW })` with no other options — no `onNeedRefresh`, and the function's own return value (`updateServiceWorker`) is discarded, never called.
2. **`vite-plugin-pwa`'s own generated client code** (`dist/client/build/register.js`, confirmed via direct read of the installed `0.21.2` package) — for `registerType: 'autoUpdate'` (`auto === true`), registers listeners ONLY for `"activated"` (triggers `window.location.reload()`) and `"installed"` (fires `onOfflineReady`). **It never registers a listener for the `"waiting"` event, and never calls `sendSkipWaitingMessage()`/`wb.messageSkipWaiting()` anywhere in that branch** — that call only happens in the NON-auto (`prompt` mode) branch, gated behind the app itself calling the returned `updateServiceWorker()` function (which, per point 1, this app never does).
3. **`workbox-window@7.4.1`'s own `Workbox` class** (confirmed via direct read of the installed package source) dispatches a `"waiting"` event when a new service worker finishes installing but can't yet activate — it does NOT auto-skip-waiting internally; that's an explicit, opt-in method (`messageSkipWaiting()`) the caller must invoke.

**Net result, confirmed by tracing all 3 links: nothing in this app's current configuration ever calls skip-waiting.** A new service worker downloads, installs, and sits in the `waiting` state indefinitely. The `"activated"` event this app's own reload-on-update logic is listening for **never fires** under normal operation — the ONLY thing that ever activates a waiting service worker is the browser's own native fallback: once every tab/client controlled by the OLD service worker closes, the waiting one activates on the next fresh navigation.

## This precisely explains every symptom reported, not just loosely

- **"Closing the browser fixed it (temporarily)"** — exactly the browser's own native fallback mechanism described above; the only path that has ever actually worked here.
- **"Broke again later"** — another deploy landed in between, leaving a fresh version stuck `waiting` again behind the (now-doubly-stale) active one.
- **Mines sound specifically missing** — `2026-09-22#2`/D39 (today's batch) is what added Mines sound in the first place; a phone still running a pre-D39 service worker simply doesn't have that code at all — not a bug in the new feature, a version mismatch.
- **Chess sound inconsistently missing too** — consistent with the same mechanism if the device happened to be pinned to different stale versions across different visits (this app has deployed several times today alone).
- **Designer's own independent "older-looking code" reports** — the same mechanism, corroborating from a second, unrelated source.

## Fix

**Standard, well-known correction for exactly this class of gap: `skipWaiting: true, clientsClaim: true` in the `workbox` options passed to `VitePWA()`** (`vite.config.ts:52-61`). This makes the GENERATED service worker itself unconditionally call `self.skipWaiting()` in its own `install` event and `self.clients.claim()` in `activate` — removing the dependency on the message-based handshake entirely, so a newly-downloaded version activates as soon as it finishes installing (on the next `registration.update()` check, currently hourly — see below) rather than only when every tab closes.

**Confirmed this correctly completes the reload flow the code already has, not something needing new wiring.** `workbox-window`'s `"activated"` event fires off the registration's own state transitions (`installing→installed→activating→activated`) regardless of WHETHER skip-waiting was message-triggered or unconditionally baked into `install` — so `main.tsx`'s EXISTING `wb.addEventListener("activated", ...)` → `window.location.reload()` logic should correctly fire once this one config change ships. This is a genuine bug fix completing the app's own already-stated intent, not new scope.

**One real, worth-flagging tradeoff — not mine to silently decide, Owner already aware and testing in parallel.** With this fix, an already-open tab can reload automatically once a new version is detected — including, in principle, mid-match. `docs/DEPLOY.md`'s own documented reasoning for the WS 60-minute timeout cites `match.resume` as already handling exactly this class of reconnect-and-resume disruption, so this should recover gracefully rather than losing a match — but it IS a real behavior change (an unprompted reload during active play, where today there's none at all) worth Owner's/Designer's own eyes before or shortly after shipping, not something to bury silently in this fix.

**Secondary, smaller consideration, not blocking:** `main.tsx:31`'s hourly `registration.update()` polling interval means even AFTER this fix, a new version might not be detected for up to 60 minutes on an already-open tab (a FRESH navigation always checks natively regardless of this interval, so this only affects tabs left open across a deploy). Given how frequently this app is currently being deployed (several times some days this week), a shorter interval (e.g., 10-15 minutes) may be worth considering — flagging as a secondary, optional tightening, not bundling into the same fix without Owner's own call on it.

---

**Ask:** the core fix (`skipWaiting`/`clientsClaim`) is a small, well-precedented config change — the standard fix for this exact class of PWA-update gap. Flagging the mid-match-reload behavior change explicitly since Owner is testing in parallel and should know what to expect; the polling-interval question is separate and optional.

---
### 2026-09-22#4 — D41, RPS pick tiles have unwanted text labels and the selection-green treatment is incomplete: confirmed the labels exactly as reported (both usages — idle preview AND live picker), confirmed the ring/icon-colour gap too, but found the "icon colour" instruction cites a prototype computed value that's genuinely never consumed anywhere in the prototype's OWN markup — dead code in the prototype itself, not a rendering behavior that exists on screen there. Flagging precisely and recommending implementing it anyway, since Designer's own citation is specific and deliberate, not an accidental reference            [READY TO TICKET]
From: Advisor   Re: Designer's D41 report (design-ref/D41, screenshots showing ROCK/PAPER/SCISSORS text under the icons), verified against `apps/web/src/screens/RpsHub.tsx`'s `RpsIdle` (`:230-273`) and the live picker (`:512-543`), `RpsHandIcon` (`:75`), and the prototype's own exact source at both the markup (`Full Spec.html:641-656`) and the JS computed-values function it's built from (`:3244-3256`, `rpsChoice()`)

Confirmed both parts of the report, plus 2 additional precise discrepancies in the selection-ring mechanics beyond what D41 itself lists, and one genuine citation subtlety worth flagging rather than silently porting.

## Part 1 — the labels: confirmed exactly as reported, in BOTH of the two places this renders

**`RPS_CHOICES`'s own `label` field is rendered in two separate spots — both need the label removed.** The idle preview (`RpsHub.tsx:252-256`, shown before a match starts) and the live in-match picker (`:522-539`, the actual selectable tiles) each render `<span className="text-[11px] font-bold uppercase tracking-wide text-muted-foreground">{label}</span>` directly under the icon. Confirmed via direct grep of the prototype's own markup (`Full Spec.html:641-656`) — the tile block is genuinely just one `<svg>`, no text element at all, at either usage. Delete both `<span>` elements; `RPS_CHOICES`'s own `label` field can stay (still used for `aria-label`/`aria-pressed` accessibility text on the live picker's `<button>`, which the prototype's own source doesn't need since it's not a real button element there — not a citation match, just sound accessibility practice to keep).

**The idle preview's own `gap-1` class (icon-to-label spacing) becomes dead weight once the label is gone** — harmless with one child, but worth dropping in the same pass for cleanliness, not a functional requirement.

**One additional citation-match gap, not called out in D41's own report but visible in the same source block:** our tiles are `className="... py-4 ..."` (16px top/bottom padding, matching D41's own cited `padding:16px 6px`'s vertical half exactly) but have NO horizontal padding class at all — the prototype's own citation is `padding:16px 6px` (6px LEFT/RIGHT too). Doesn't affect the 86px height claim (that's purely vertical), but worth adding `px-1.5` (6px) while touching this code, matching the citation fully rather than partially.

## Part 2 — the selection green: confirmed the reported gaps, plus 2 more precise ones found by reading the actual CSS values side by side, not just checking presence/absence

**Already correct, confirmed:** the selected-state ring already uses `var(--rc-green)` (`RpsHub.tsx:535`) — checked the token's own definition (`index.css:87`/`:263`) and it resolves to exactly `#34d399` dark / `#0b8f5a` light, matching D41's own cited values precisely. This part needs no change.

**Gap 1 (matches D41's own report): unselected ring uses a color-mismatched zero-alpha value.** Currently `'inset 0 0 0 3px transparent'` — CSS `transparent` is `rgba(0,0,0,0)` (transparent BLACK), not `rgba(52,211,153,0)` (transparent GREEN, D41's own cited value). For a `box-shadow` color transition to animate as a clean fade rather than a subtle hue-shift-then-fade, both endpoints need matching RGB channels, differing only in alpha — exactly what D41's own wording flags ("so the transition animates cleanly in and out"). Fix: `'inset 0 0 0 3px rgba(52,211,153,0)'`.

**Gap 2 (found independently, not in D41's own listed items — checked the actual easing curve, not just "is there a transition"):** the current className is `transition-[background,box-shadow] duration-200 ease-out` — D41 cites plain `ease`, a genuinely different curve (`ease-out` and `ease` are NOT the same easing function). Small, but since D41 is this precise about every other value, worth matching exactly: `duration-200 ease` (drop `-out`).

**Gap 3 (the ticket's own "icon colour" item) — confirmed NOT currently implemented at all, AND worth flagging a real subtlety in its own source before recommending the fix.** `RpsHandIcon` has no color/selection prop today — every hand icon always renders its full multi-tone illustration (`#B285F7`/`#7B3BE0`/`#8B45F0` etc., several distinct literal fills per icon), identically regardless of pick state. Checked the prototype's OWN source precisely for this: `rpsChoice(k, light)` (`Full Spec.html:3244-3256`) DOES compute a `color: on ? '#FFFFFF' : 'var(--rc-muted)'` field, matching D41's own cited values exactly — but a full, direct read of the ENTIRE picker-tile markup block (`:641-656`) shows this `.color` field is **never referenced anywhere** — every `<path fill="...">` in the prototype's own SVGs is a hardcoded literal, with no `{{ ... .color }}` binding at all, in either the picker tiles or the reveal-card usage of the same `rpsRock`/`rpsPaper`/`rpsScissors` state object. **This is dead code in the prototype's own source — a computed value the prototype's own author never actually wired up, not a behavior that exists anywhere on screen there.**

**Recommending implementing it anyway, not skipping it — this reads as a deliberate ask, not an accidental citation.** D41 states the icon-colour values specifically, with their own transition timing (`220ms ease`), as one of 4 explicit bullet points under "The selection green" — too precise and too deliberately grouped with the (genuinely-rendered) ring values to be an accidental reference to unused code. Treating this the same way earlier tickets have treated a deliberate departure from the prototype's own literal rendering (applying as directed, with the discrepancy noted for the record) rather than either silently skipping it or silently assuming it must already be correct because "the prototype does it" — it doesn't, but Designer's own intent here reads clearly enough to implement.

**Implementation note, since our icon is structurally different from a simple monochrome swap:** `RpsHandIcon`'s SVGs use MULTIPLE distinct literal fills per icon (not `currentColor`), same as the prototype's own — implementing the color swap means each `<path>`'s `fill` needs to become the passed color when selected/unselected, turning the icon into a flat single-tone silhouette in that state (muted-gray unselected, white selected) rather than its current always-multi-tone look. This is a real, visible change to how the picker specifically looks — worth being upfront about, not just a quiet prop addition — though it only affects the picker tile usage (`RpsHub.tsx:538`), not the 3 OTHER call sites of the same shared `RpsHandIcon` component (the idle preview at `:254`, and the two reveal-card usages at `:434`/`:642`, which keep their full multi-tone look unchanged — the prototype's own `.color` field was only ever computed for the picker-tile context, never referenced for the reveal cards either).

---

**Ask:** label removal is unambiguous and safe. The 2 additional ring-mechanics discrepancies (color-matched zero-alpha, easing curve) are small, precise value corrections. The icon-colour item needs a judgment call flagged plainly above — recommending implementing it as a deliberate Designer ask despite the prototype's own dead-code status, but flagging it precisely rather than treating it as a simple, unambiguous port.

---
### 2026-09-22#3 — D40, opponent's gem strip wraps 3 rows instead of ~2, squeezed to roughly a third of the bar's width: confirmed exactly as reported, root cause found, and it's a direct regression from my OWN D37 fix from yesterday — the "Playing…" label went from a conditionally-UNMOUNTED element to an always-mounted, in-flow flex sibling, and even at opacity 0 it still reserves its own real width, squeezing the gem strip's `flex-1`. I flagged this exact risk in the D37 ticket itself ("worth a live-page look... rather than assuming") and assessed it as "likely negligible" — that assessment was wrong. Also: 2 of Designer's own 3 listed causes are actually ALREADY correctly implemented in our code — only the width issue is real, the other two symptoms are downstream consequences of it            [READY TO TICKET]
From: Advisor   Re: Designer's D40 report (design-ref/D40, a phone screenshot showing the opponent's 12 gems wrapping 5-5-2 across 3 rows while the player's own 6 gems fit cleanly on one), verified against `apps/web/src/screens/GameHub.tsx`'s `OpponentSlot` (`:1085-1182`) and `apps/web/src/screens/MinesHub.tsx`'s `GemCountRow` (`:558-580`), cross-checked against the prototype's own exact source (`Full Spec.html:461`, confirmed the full rule — not just the `position:absolute; right:14px` D40 itself quotes)

Confirmed exactly as reported. This one's a direct regression from my own work yesterday (D37/`2026-09-21#10`) — flagging that plainly, not as an unattributed new bug — plus a genuine correction to two of Designer's own three listed causes, which I traced precisely rather than assuming all three apply equally.

## Root cause, confirmed: my own D37 change turned the "Playing…" label into an in-flow flex sibling that reserves width even when invisible

**D40's own point 1 names the likely culprit exactly right — and it's traceable to a specific commit.** `GameHub.tsx:1176-1181` (the shared "Playing…" label, EVERY hub using `OpponentSlot` renders through this) is currently:
```
...
```
`shrink-0` is an ordinary, in-flow flex class — NOT `position:absolute`. At the exact reveal moment in D40's screenshot, `oppLocked` is `true`, so this element IS correctly invisible (`opacity:0`) — but being in-flow, it still reserves its own natural width in the row regardless of opacity, squeezing whatever's left for the gem strip's own `flex-1` sibling (`GameHub.tsx:1150`). Confirmed no other max-width or stray constraint anywhere in either file (`grep` for `max-width`/`maxWidth` across both — only comment prose, no actual CSS) — this is the sole cause.

**This is a direct regression from my own `2026-09-21#10`/D37 ticket, shipped yesterday (PR #673) — flagging plainly.** Before D37, this same label was a plain conditional MOUNT (`phase === 'in-match' && <span>...</span>`) — when invisible, it was entirely ABSENT from the DOM, reserving nothing. D37 deliberately changed it to always-mounted+opacity-toggled specifically to make the 260ms fade actually work (the mount/unmount was popping instantly, not fading) — a correct fix for THAT problem, but it introduced exactly this width-reservation side effect. **I flagged this exact risk in the D37 ticket itself** ("switching from conditional-mount to always-mounted+opacity means the label's own box now reserves its natural width even while invisible... likely negligible... but worth a live-page look once shipped rather than assuming") — my own stated confidence there was wrong; it's not negligible, it's the entire bug D40 reports. Correcting the record precisely rather than treating this as a fresh, unattributed issue.

## A precise correction to Designer's own diagnosis: only 1 of the 3 listed causes is actually missing from our code — checked each directly, not assumed

**Point 2 (height cap) is ALREADY correctly implemented.** `GemCountRow` (`MinesHub.tsx:558-580`) already has `overflow-hidden` (className) and `maxHeight: 38` (inline style) — both present, confirmed by direct read. The partial third row visible in the screenshot isn't evidence the cap is missing or `overflow` isn't hidden — it's the natural, downstream visual consequence of point 1's own width squeeze: forced into 3 rows by insufficient width, the (already-correctly-capped) 38px box clips the 3rd row exactly at its edge, producing the "partially visible" row D40 itself describes. Once the width is fixed and 12/22 gems fit in ≤2 rows (matching D40's own math), this cap has nothing left to clip — becomes the safety net it was always meant to be, exactly as D40's own closing line anticipates ("the cap is a safety, not the layout").

**Point 3 (alignment) is ALREADY correctly implemented too.** `content-center` (→ `align-content:center`) is present in `GemCountRow`'s own className; `justify-content:flex-start` needs no explicit class at all — it's flexbox's own browser-default initial value, and nothing in either file overrides it. Both already match the prototype's own cited values exactly.

**So the fix is narrowly scoped to point 1 alone** — no changes needed to `GemCountRow` itself, which was already correctly built to the prototype's own citation from the start.

## Fix — reuses an existing, already-correct sibling pattern in the SAME file, one element above

**Checked the prototype's own full source at line 461 (D40 quotes only the `position:absolute; right:14px` half) — and it reveals the exact right answer.** The prototype stacks "Searching…" and "Playing…" in the literal SAME absolutely-positioned slot (`grid-area:1 / 1`, one shared wrapper `position:absolute; right:14px; top:50%; transform:translateY(-50%)`), each independently opacity-toggled — mutually exclusive states, same visual real estate by design.

**Our own code ALREADY implements exactly this treatment for "Searching…", one element above the buggy "Playing…" span** (`GameHub.tsx:1138-1142`): `className="absolute right-3.5 top-1/2 -translate-y-1/2 animate-pulse text-sm font-bold text-muted-foreground"`. **Fix: give "Playing…" the identical positioning classes** (`absolute right-3.5 top-1/2 -translate-y-1/2`, replacing `shrink-0`), keeping its own existing font/opacity/transition styling untouched. The parent container is already `relative` (`GameHub.tsx:1097`, confirmed), so no new positioning-context setup is needed — this is a genuinely minimal, well-precedented change reusing an idiom already proven correct in the exact same component.

**Confirmed this fix is safe and correct for every hub sharing this label, not just Mines** — "Playing…"/"Searching…" are already temporally mutually exclusive with any gem-row content (gemRow is `undefined` entirely until the reveal sequence begins, and D37 already established a 560ms+ margin between "Playing…" fully fading and the gem strip becoming visible), so taking it out of flow creates no new overlap risk anywhere; for hubs without a gem row, it's a straightforwardly more-correct change too (the name span's own `flex-1` currently gets slightly squeezed by "Playing…"'s reserved width in every hub today, not just Mines — this fixes that everywhere).

---

**Ask:** small, one-class-list change, reusing an existing pattern already proven correct one line above it. Given this affects the shared `OpponentSlot` component (every hub), worth a live-page re-check post-deploy — not because the diagnosis is uncertain (this is standard, unambiguous flexbox behavior, not the kind of CSS-resolution ambiguity D24/D25 needed empirical verification for), but because it's shared code touching every game's opponent bar.

---
### 2026-09-22#2 — D39, two new Mines tile-reveal sounds: confirmed straightforward to add, but our OWN sound architecture is genuinely different from the prototype's `sfx()` the ticket describes — Web Audio buffer-sources, not one reused `<audio>` element per name — which makes 2 of the ticket's own "Two implementation details" (the overlap pool, the explicit Play-press preload) ALREADY satisfied for free by our existing system, nothing extra to build for either. The one thing that DOES need real design care, not present in the prototype's own simpler client-simulated reveal: firing honestly off SERVER-CONFIRMED tile state (not the optimistic tap), correctly scoped per-ROUND given Mines' own internal draw-replay mechanic            [READY TO TICKET]
From: Advisor   Re: Designer's D39 attachment (`mines-gem.mp3` 0.94s, `mines-mine.mp3` 2.04s), verified against `apps/web/src/lib/sound.ts`'s actual implementation, `apps/web/src/screens/GameHub.tsx`'s existing unlock/preload wiring, `apps/web/src/screens/MinesHub.tsx`'s `MinesBoard`/`cellKind()`, and `packages/games/mines/src/mines.ts`'s own round-replay mechanic

Straightforward feature addition, confirmed real and buildable — but worth being precise about where our architecture already differs from the prototype's own `sfx()` mechanism the ticket describes, since porting its literal implementation notes verbatim would mean solving two problems we don't actually have, while under-specifying the one real subtlety we do.

## Our sound system is architecturally different from the prototype's `sfx()` — checked directly, not assumed

**Ours (`lib/sound.ts`) uses the Web Audio API's `AudioBufferSourceNode`, not a single reused `<audio>` element per name.** `play(name)` (`:142-157`) looks up an already-DECODED `AudioBuffer` (from `preloadSounds()`, which fetches+decodes every `MANIFEST` entry up front) and creates a BRAND NEW, one-shot buffer-source node on every single call, starting it immediately. This is a meaningfully different mechanism from the prototype's own `sfx()` (one `Audio` object per name, `currentTime = 0` + `.play()` on every call — genuinely rewindable/cuttable, which is WHY the prototype needs the pooling trick).

## This makes 2 of the ticket's own "Two implementation details" already true for free — confirmed, not assumed

**"Overlap on fast gem taps" — no pool needed.** Since every `play('mines-gem')` call spins up its own independent buffer-source from the SAME shared decoded buffer, calling it 5 times in quick succession already produces 5 fully independent, fully-overlapping playbacks with zero risk of one cutting another short — there's no shared, rewindable object to collide over in the first place. The prototype's own round-robin-pool workaround exists specifically to compensate for a limitation our architecture doesn't have.

**"Preload on Play" — already happens, earlier and more broadly.** `preloadSounds()` decodes EVERY `MANIFEST` sound uniformly (not selectively), and is triggered by `unlock()`, which fires from a GLOBAL first-gesture listener (`GameHub.tsx:453`, `installUnlockOnFirstGesture()`) the instant the user taps ANYTHING on the page — not specifically the Play button. By the time a visitor has navigated to Mines and pressed Play, they've already tapped through several screens; the new sounds (once added to `MANIFEST`) will already be decoded and ready well before that point, not needing a Play-press-specific preload trigger at all.

**Mute — already correctly wired, exactly as the ticket asks.** `play()`'s own first line is `if (muted) return;` — the SAME check every other sound in the app already goes through. No separate toggle needed; this is a non-issue, confirmed.

## The one real subtlety, genuinely more involved than the prototype's own version — needs care, not a literal port

**The ticket's own framing ("hook into the tile `reveal` handler... at the moment the tile actually opens") describes the PROTOTYPE's client-SIMULATED reveal — there's no equivalent synchronous handler in our build to hook into.** Our tile tap (`MinesBoard`'s `onClick`, `MinesHub.tsx:395`) only ever SUBMITS the move (`moveIdx(i)`, fire-and-forget, `void`-returning) — it does NOT know the outcome. The tile only genuinely "opens" once the server's broadcast updates `gameState` on a LATER render, matching the same honest-data discipline already established throughout this engagement for Mines/Dice ("never fabricate an event that hasn't happened yet"). Firing the sound directly in the click handler would mean playing it even if the move never actually resolves as expected (a stale-`legalMoves` race, a network hiccup) — the wrong signal to key off.

**Correct hook: detect the SERVER-CONFIRMED transition, not the tap.** `MinesBoard` already computes `myUncovered` (a `Set` from `me?.uncovered`) and `bustedOn` (`me?.bustedOn`) — exactly the two fields that transition the instant, and ONLY the instant, a tile genuinely, confirmedly opens. An effect comparing these against a "previously seen" snapshot — firing `play('mines-gem')` for each newly-added `uncovered` index, `play('mines-mine')` once when `bustedOn` transitions from `undefined` to set — correctly and honestly satisfies all 3 of the ticket's own gating conditions as a side effect of the diff itself: a tile that doesn't newly appear in the diff never fires (covers "already open," "wrong phase" — nothing changes server-side during search/idle — and "after bust," since no more taps are legal once locked).

**Confirmed this correctly, automatically excludes the `autoSafe` bulk reveal too — by construction, not by an extra condition.** `cellKind()`'s own `autoSafe` branch (the "reveal all 24 remaining tiles at lock" mechanic, `2026-09-16#6` item 2) is a pure RENDERING fallback — it never adds anything to `me.uncovered` itself. Since the sound effect watches `me.uncovered` directly (not `cellKind()`'s own derived render output), the 24 auto-revealed tiles never appear in the diff at all — "the auto-reveal is silent" falls out for free, no special-casing needed.

**One genuine design subtlety the prototype's own simpler single-reveal-per-match model doesn't have to solve: this needs to be scoped per-ROUND, not just per-match.** `packages/games/mines/src/mines.ts` confirms Mines has its own internal draw-replay mechanic (`round`, bumped on a tied result, same match/escrow) — a mid-match round replay resets `me.uncovered` back to `[]` server-side. A naive "seen" set that only resets on `currentMatchId` changing (the existing pattern this file already uses for `resultPhase`/`lockReason`, since THOSE are correctly one-shot-per-match concepts) would, on a replay, silently SKIP the gem sound for any tile index that happens to repeat between the two rounds — the same index would already be marked "seen" from the earlier round. **The reset needs to be keyed off round too** (e.g., `` `${currentMatchId}:${view?.round}` ``, re-establishing the baseline from whatever's already present — never retroactively sound-triggering for a reconnect mid-round where some tiles are already revealed).

## Files and placement — one correction from the ticket's own path

**`apps/web/src/assets/sounds/`, not `assets/audio/`.** The ticket's own path follows the prototype's literal convention; our build's own existing convention (confirmed via `ls`: `play.mp3`, `dice-roll.mp3`, `dice-win.mp3`, `reject.wav` all already live in `assets/sounds/`) is the one to match. Straightforward: copy both provided files in as-is (no generation script needed — these are Designer-provided, not self-synthesized, same precedent as `play.mp3`/`dice-roll.mp3`/`dice-win.mp3`), import their URLs, add two `MANIFEST` entries in `lib/sound.ts`.

---

**Ask:** small feature addition overall — 2 asset files, 2 `MANIFEST` lines, and one new effect in `MinesBoard` (the round-scoped diff-and-play logic, the one part needing real care). No pooling code, no explicit preload-on-Play code — both already handled by the existing system.

---
### 2026-09-22#1 — D38, Dice's roll "sometimes" starts mid-track instead of at 0: confirmed, empirically reproduced with exact numbers, and traced to a genuine regression from my own D35 change — the reveal clock (armed off server data, arriving at unpredictable network-latency-dependent speed) races the cube's own fixed 660ms reveal delay, and on a fast round-trip (very plausible vs. bots) the count-up is already well underway by the time the cube/fill first become visible. Designer's own two-reason prototype diagnosis doesn't literally describe our architecture (no interpolate-from-current bug, no two-step setState reset) — but the SAME symptom, confirmed via a different, precisely-identified mechanism. Fix verified in a scratch copy: 2-line change, 29/29 existing tests unaffected            [READY TO TICKET]
From: Advisor   Re: Designer's D38 report ("the roll sometimes starts from the middle of the tracks"), verified via direct code review of `apps/web/src/screens/DiceHub.tsx`'s `DicePanel` reveal-clock effect and `packages/games/dice/src/dice.ts`'s own reveal-gating (`bothRevealed`), PLUS an empirical repro rendering the real component with controlled timing (both the failing case and the correctly-working case reproduced with real numbers), PLUS a scratch-applied fix confirmed to resolve both and pass the full existing `DiceHub.test.tsx` suite unchanged

Confirmed real, and did the full empirical workup rather than reasoning from the prototype's own citations alone — those citations describe a mechanism our architecture doesn't share, so I traced OUR OWN actual mechanism from scratch and reproduced both the bug and the fix against the real component.

## Neither of the prototype's own 2 stated reasons literally applies to our code — checked, not assumed

**Reason 1 (interpolating from a stale `current` value instead of `target × e`) — does not apply.** `revealedRoll()` (`DiceHub.tsx:440-445`) is already exactly `Math.round(finalRoll * revealEase(p))` with `p` starting at 0 — no `from` value anywhere, confirmed by direct read. This was never the bug here.

**Reason 2 (the prototype's own two-step `setState` reset, both landing before the cube is ever visible) — doesn't describe our architecture, since we don't drive this off local component state the prototype's own way.** Our reveal timing is real-clock-driven (`Date.now()`-based, off actual server data), not scripted via nested nested-`setTimeout` `setState` calls the way the prototype's `startDice()` is. There's no analogous "double reset" in our code to be missing.

## The real, empirically-confirmed mechanism: a race between two independently-timed clocks, introduced by my own D35 change

**Traced it to a specific architectural gap: the reveal clock's own START is completely decoupled from `cubeActive`'s 660ms artificial delay (added in `2026-09-21#6`/D35, my own earlier ticket, to fix the DIFFERENT "cube pops in mid bar-split" bug).** `DiceHubScreen`'s own `armedSig`-setting effect (`:500-512`) reads `props.gameState` directly, computing a signature and arming the reveal clock the INSTANT the server delivers a result — completely independent of whether the cube is actually visible yet. Inside `DicePanel`, the reveal clock (`revealElapsed`, `:397-409`) starts its own `Date.now()`-based timer the moment `armedSig` changes, with ZERO knowledge of `cubeActive`'s own separate 660ms delay.

**Before D35, this race didn't exist — the two were always synchronized for free.** Pre-D35, the old `DiceBoard` component only ever MOUNTED once `live` became true, with `active` unconditionally `true` from that same render (no artificial delay of any kind) — so the reveal clock and cube visibility always started in the exact same instant, no possible gap. D35 deliberately introduced `CUBE_REVEAL_DELAY_MS` (660ms) as a NEW gap between "cube could theoretically be shown" and "cube is actually shown" — correctly fixing D35's own target bug — but nothing re-examined whether the (unrelated, pre-existing) reveal clock still needed to start in lockstep with that visibility gate. It didn't get updated, and this is the result. **Flagging this plainly: this is a regression traced to my own D35 ticket, not a fresh, unattributed bug** — the same "correction owed to my own earlier work" standard applied to D30's root cause a day earlier.

## Empirically reproduced BOTH the bug and its "sometimes" nature, with exact numbers — not asserted from reasoning alone

Rendered the real `DiceHubScreen` component with controlled timing via two scenarios:
- **Fast round-trip** (server result already present in `gameState` the instant `live` flips true — realistic against bots, who resolve near-instantly): at the exact moment the cube becomes visible (opacity 1, +700ms), the label already reads **"46.50"** and the fill is already at **46.5% width** — nowhere near 0. Confirmed reproducing exactly what Designer describes.
- **Slow round-trip** (result arrives ~600ms after `live` flips true, simulating network/opponent latency): at the same +700ms mark, the label correctly reads **"0.00"**, fill at the exact 10px nub. Confirmed the SAME code path works correctly when the server is slower than the 660ms window — this is precisely why the bug reads as "sometimes," not "always": it depends entirely on how fast the round-trip happens to be relative to the fixed 660ms delay, which varies run to run.

## Fix — small, and its correctness confirmed empirically in a scratch copy (not committed)

**Gate the reveal clock's own start on `active` (the same prop `DiceTrack` already reads for cube opacity/scale), not just `armedSig` alone.** Concretely, in `DicePanel`'s reveal-clock effect (`:397-409`):
```
useEffect(() => {
...
```
A 2-line change (the guard condition, the dependency array). This guarantees the clock's `start = Date.now()` reference is always set to whichever comes LATER — `armedSig` being set, or `active` becoming true — so the count-up can never begin before the cube is actually visible, regardless of how fast or slow the server round-trip happens to be. Pleasingly, this also reproduces the prototype's OWN 460ms gap between `diceCubeIn:true` and `runDiceRoll` actually starting (`Full Spec.html:3421`→`:3423`, the `mnR1` `setTimeout(..., 460)`) — once gated this way, `REVEAL_SOUND_MS` (460ms) becomes the delay between "cube visible" and "count-up begins producing nonzero values," matching the prototype's own citation exactly, for the reason the prototype's OWN nested-timeout structure gets it for free and ours needed an explicit gate to replicate.

**Verified, not just reasoned through:** applied this exact 2-line change to a scratch copy, re-ran the same 2 empirical repro scenarios — the fast-round-trip case now correctly shows "0.00"/10px at the same +700ms mark (previously 46.50%/46.5%-width). Re-ran the FULL existing `DiceHub.test.tsx` suite against the scratch fix: **29/29, unchanged** — every existing test mounts the component ALREADY live (`active` true from render 1, the documented reload/reconnect case), so none of them exercise the now-fixed race at all; this fix is additive, not disruptive. Reverted the scratch change before writing this ticket — PM's own implementation is the one that ships.

## A second, related, lower-priority finding — flagged precisely, not silently folded in or silently dropped

**Designer's own "Check" section also describes: "Play again immediately after the result — fills slide from the last values to the nub during the search, same start."** Checked this specifically — our build does NOT do this. `DiceTrack`'s `fillWidth` (`:173-175`) uses a flat `!active ? '50%' : ...` — during EVERY 'waiting' phase (searching), regardless of whether this is truly the first-ever round or a replay after a prior result, the fill always shows 50% (the "true cold idle" value). Traced the prototype's own source precisely: `diceMy`/`diceOpp` are `null` ONLY before the very first round of a session — once any round resolves, they hold a REAL NUMBER (the last roll) continuously, through the ENTIRE next search, until the scripted `+680ms` reset explicitly zeroes them — so the prototype's own 50% fill is a ONE-TIME, first-round-only visual, never recurring on a replay. **This is a real, pre-existing gap — confirmed NOT introduced by D35 (the old, separate `DiceIdle` component hardcoded `active={false}` unconditionally too, before D35 ever existed) — but it does directly contradict this ticket's own explicit check.**

**Not recommending folding this into the same fix**, for three reasons worth being upfront about: (1) it's architecturally more involved — replicating it needs new persisted state (the last-resolved round's own values, not currently kept anywhere past the moment `history` is pushed) plus a design call on the intermediate reset timing the prototype's own scripted `+680ms` doesn't map onto our async, server-driven data flow cleanly; (2) it's the LEAST visible of the two findings — the board is dimmed to 0.28 opacity during the whole search, and the discrepancy (50% vs. the true last value) is easy to miss at a glance; (3) the primary, empirically-confirmed bug above is almost certainly what's actually being reported, given the ticket's own repeated emphasis on the CUBE's own first-visible position/label as the core invariant. Flagging this precisely so it isn't lost, not silently deciding it's out of scope — Designer/PM's call whether it's worth a fast follow-up.

---

**Ask:** the primary fix (reveal-clock gating) is small, empirically verified, and safe — 2 lines, no regressions in the existing suite. The secondary finding (search-phase fill value) is real but more involved — recommend a separate follow-up ticket rather than bundling, unless Designer wants it addressed now.
...
Owner sent a Cloud Run metrics screenshot and 3 phone screenshots (broken-image icons on the games grid, the hero banner carousel, and the "Originals" tile section) and asked for help finding the monitoring console to check load. Traced it all the way to root cause with hard data, not just the dashboard.

## The evidence, precisely

**1,129 of 8,213 requests (13.7%) in the last 24h got HTTP 429** (`gcloud logging read ... httpRequest.status=429`) — image assets (`keno.webp`, `hero-1.webp`, `hilo.webp`, etc.), a font file, and plain API routes (`/games`, `/wallet`, `/games/popularity`) all rejected outright. This is Cloud Run's OWN infrastructure-level rejection (distinct from `@fastify/rate-limit`, which only ever applies to `/auth/guest` and `/rewards/claim` — confirmed via `server.ts`). Still happening in the current hour when checked (21 in the last hour), not purely historical.

**Why: the single mandatory instance has a concurrency ceiling that was never explicitly set, so it defaults to 80.** `gcloud run services describe` confirms `autoscaling.knative.dev/maxScale: '1'` (deliberate, documented, ADR-009 — match state is in-memory and the DB is a local SQLite file, neither survives scale-out) with `containerConcurrency: 80`. `docs/DEPLOY.md`'s own canonical deploy command (`:108-119`) never passes `--concurrency` at all — 80 is Cloud Run's bare default, not a deliberate choice anyone made.

**What's consuming it: the always-on bot-crowd holds 32 WebSocket connections continuously.** SSH'd into the demo-taker VM (`rapidclash-demotaker` project) and read its live config: `TAKER_ONLY_GAMES=coinflip:3,blackjack:3,chess:3,rps:3,mines,crash,roulette,dice,baccarat,keno,limbo,hilo` — per `tools/bot-crowd/README.md`'s own documented formula (1 taker + N resters per listed game), that's 4 games × 4 bots + 8 games × 2 bots = 32 bots, each holding its own persistent WS connection, running 24/7. That alone is ~40% of the 80-slot budget before any real visitor arrives.

**Precisely WHEN the 429s cluster — pulled the raw Cloud Monitoring concurrency metric via the REST API rather than eyeballing the dashboard, and it changes the picture in an important way.** During calm periods, real concurrency peaked at only ~50 (never reached 80) over the last 24h. But 429s aren't spread evenly across the day — 759 of the 1,129 landed in a single hour (09:00 UTC), which is EXACTLY when `rapidclash-00123-rt9` (today's D30-D35 batch) deployed; another 297 clustered around 13:00 UTC. This is a **deploy/restart-time reconnect burst**, not sustained all-day overload: when the bot-crowd's 32 connections all reconnect within seconds of each other (plus any real visitor mid-load at that exact moment), it briefly spikes past 80 before settling back to ~50. Given deploys have happened nearly every ticket cycle this week (each followed by a bot-crowd restart per standing practice), a tester loading the site in that few-minute window would see exactly the reported broken-tile symptom — repeatedly, across "several days," which is exactly what was reported.

**Compounding factor found in the code: no WebSocket heartbeat.** `apps/server/src/ws/gateway.ts` has a `close` handler (`:638`) that does correct, thorough cleanup — but only fires on an actual close event. There's no `ping`/`pong`/`isAlive` mechanism anywhere in the file (confirmed via grep) to proactively detect a connection that died WITHOUT a clean close (extremely common on mobile — losing signal, backgrounding, switching networks). Without one, a dead mobile connection can sit occupying a concurrency slot for up to Cloud Run's own 3600s (1-hour) hard cap before being forcibly reaped — reducing the real margin further over time, on top of the deploy-burst problem above.

## Two items, both Owner-approved after discussion (final number: 300, not the 250 I first floated)

**Item 1 — raise `containerConcurrency` from 80 to 300.** This is a Cloud Run SERVICE CONFIG change (`gcloud run services update rapidclash --region us-central1 --concurrency 300`), not an application code change — doesn't touch `--max-instances` or the single-writer architecture at all (still exactly one instance; this just lets that one instance accept more simultaneous requests before Cloud Run starts rejecting). 300 comfortably absorbs the worst-case burst (32 bots reconnecting at once + several real testers each pulling ~15-20 parallel asset requests) with real headroom, well under Cloud Run's own 1000/instance ceiling, at zero extra cost unless genuinely used. **Also update `docs/DEPLOY.md`'s canonical deploy command (`:108-119`) to add `--concurrency 300` explicitly** — it's undocumented today, so a future full redeploy that doesn't know to set it risks silently reverting to the 80 default.

**Item 2 — a WebSocket heartbeat, low effort, fits an idiom already in this file.** `gateway.ts` already has a periodic `setInterval` (`sweepTimer`, `:559`) with per-tick try/catch isolation for its existing matchmaking sweeps — the standard `ws` library ping/pong pattern (tag each socket with an `isAlive` flag, set false before each ping, set true on `pong`, `socket.terminate()` — which fires the existing `close` handler, so all existing cleanup logic is reused verbatim — if it's still false next tick) fits the same shape and the same `connections` Map (`:44`, `Map<string, WsSocket>`) already iterates every live connection elsewhere in this file. Owner's own framing is right that 300 alone likely solves the reported case on its own (a handful of testers, not immediately after a deploy) — but this closes the actual gap (a connection can otherwise sit dead for up to an hour) for comparatively little code, so worth doing together rather than deferring.

---

**Ask:** item 1 is a deploy-config change (update `DEPLOY.md` + apply live via `gcloud run services update`) — Owner has explicitly approved 300, ready to apply now. Item 2 is a small, self-contained code change in one file, reusing the existing cleanup path — no new state, no protocol change. Both independent of every other open Dice ticket.
...
Confirmed both, in the same component (`DiceTrack`), for two genuinely distinct reasons — Designer's framing ("one value drives everything on a cube") is the right lens for both, but they're two separate code spots, not one shared line to fix.

## Item 1 — cubes appearing blank: our null-fallback disagrees with itself

**The cube (SVG + label) is visible — `active` is true, opacity 1, full scale — from the instant a match goes live**, `DiceHub.tsx:146-147` (`cubeOpacity`/`cubeScale`, both gated on `active` alone). That part is already correct and matches the prototype's own `diceCubeIn` (`:3422`, flips true at match-run-start, well before any roll data exists). **But the number label inside it falls back to an empty string whenever `roll` is `null`** (`DiceHub.tsx:188`, `{roll != null ? fmtRoll(roll) : ''}`) — and `roll` stays `null` for the entire wait between "cube becomes visible" and "the reveal clock's own `REVEAL_SOUND_MS` (460ms) elapses after the server's result arrives" (`revealedRoll()`, `DiceHub.tsx:378-383`). That wait includes the full server round-trip time, which is indeterminate — so the cube can sit fully visible but textually blank for a real, noticeable stretch.

**The prototype never has this gap, because it seeds the SAME state field to a real number at the exact moment the cube appears — not to null.** `this.setState({ diceMy: 0, diceOpp: 0, diceRolling: false, diceCubeIn: true })` (`:3422`) sets `diceCubeIn` true and `diceMy`/`diceOpp` to `0` in the SAME call — so `diceMyVal` (`this.state.diceMy == null ? '' : diceMy.toFixed(2)`, `:3610`) is `"0.00"` from the very first frame the cube is visible, never blank. This lines up exactly with `cubeLeft`'s own already-established resting fallback in our code (`value = roll != null ? ... : 0`, `DiceHub.tsx:143`) — the position already rests at 0% the whole time; the label is the one place that fell back to blank instead of the matching "0.00".

**Fix:** make the label's fallback match the position/fill's own existing fallback — show `fmtRoll(0)` ("0.00") whenever the cube is `active`, blank only when it isn't (idle preview, cube not shown at all). Not a fabricated value: it's the same honest "resting position" the cube's own `left`/fill width already commit to before a real roll exists, just applied to the third output of the same value.

## Item 2 — number and cube drift out of sync during the roll: a leftover CSS transition fighting a per-frame value

**During the 444ms count-up, `roll` (via `revealedRoll()`) already updates every animation frame** — the reveal clock's own `tick()` sets new state on every `requestAnimationFrame`, `DiceHub.tsx:341-344`, and `revealedRoll()` recomputes a new eased value each time, `:378-383`. The number label renders that value directly, with no CSS involved — it snaps to whatever the current frame's true value is, instantly and exactly.

**But `cubeLeft` and `fillWidth` are ALSO both wrapped in an unconditional CSS `transition`** — `CUBE_TRANSITION` (`DiceHub.tsx:82`, includes `left 420ms cubic-bezier(...)`) and `FILL_TRANSITION` (`:73`, `width 420ms cubic-bezier(...)`), both applied with no gating, every frame, `:170` and `:180`. Every ~16ms the target `left`/`width` moves to a new value, and the browser starts smoothing toward it over 420ms — but the target moves again before that catches up. The visible result is a cube/fill that perpetually lags behind a moving target, most visible right at the end: the number reaches the true final value the instant `revealElapsed` hits `REVEAL_SETTLE_MS` (`:380`), while the cube/fill are still gliding to catch up for up to another 420ms afterward — a real, visible desync between the two, exactly matching the report.

**The prototype never fights itself this way, because it never layers a CSS transition on top of its own per-frame state drive.** There is no `left` transition anywhere in the prototype's source for the live-roll path — position is pure per-frame state, full stop. The fill DOES have a transition, but it's explicitly turned OFF while a roll is in progress: `diceFillTrans: this.state.diceRolling ? 'none' : 'width 420ms cubic-bezier(0.4,0,0.2,1)'` (`:3675`) — the smoothing only applies when nothing is being driven frame-by-frame. **`CUBE_TRANSITION`'s own comment (`DiceHub.tsx:74-82`) confirms this was added under a since-superseded assumption:** it was written for ticket `2026-09-12#5`, when `roll` only ever had two states (unset, or snap-straight-to-final) — a real CSS transition made sense as "the honest substitute" for a single discrete jump. Ticket `2026-09-16#4` (D21) then added the per-frame reveal counter on top, without anyone going back to gate this transition off during that window — the same "two features individually correct, wrong once combined" shape as `2026-09-21#1`'s own root cause, and, like that one, traces back to my own earlier work rather than a fresh regression.

**Fix:** gate `left` (and `width`, which has the identical problem) off during the active per-frame count, mirroring the prototype's own `diceRolling` gate on `diceFillTrans` exactly. Concretely, something like `transition: counting ? 'none' : CUBE_TRANSITION` / `... : FILL_TRANSITION`, where `counting` is true exactly while `revealedRoll()` is actively producing a new value each frame (`revealElapsed != null && revealElapsed < REVEAL_SETTLE_MS`) — `DiceBoard` already computes everything needed for that condition, just needs to thread it down to `DiceTrack` alongside `roll`.

---

**Ask:** both fixes are local to `DiceHub.tsx`, no protocol/server changes. Independent of each other and of `2026-09-21#2` (still unimplemented, server-side) — safe to ship separately or together.
...
Confirmed real. Ruled out all 3 of Designer's own listed candidates by measurement rather than reasoning about them on paper, then found the actual mechanism — which is a genuine, if subtle, bug, but a different one than any of the three, and one I can trace to my own earlier work rather than something newly introduced.

## Ruling out candidates 1 and 2 — empirically, not by inspection

Rendered the real `MinesHubScreen` component through a full result sequence (converge → reveal → final) and read the bars' actual `transform` value out of the DOM at each phase, rather than reasoning about the measurement code on paper. **The transform value is byte-identical across all three phases** — `translateY(-71px)` / `translateY(23px)`, unchanged from the moment `converge` fires through `final`. This directly rules out candidate 1 (re-measurement) and candidate 2 (a second transform target) — confirmed our own `resultConverge` computation only edges the re-measurement effect false→true once, at the very start of the sequence, and never again.

## The real mechanism — a reflow candidate 3 doesn't quite describe, but is closely related to

**Candidate 3 was the right instinct, wrong element.** It's not the bar's own ring or fill changing its box size — the win ring is confirmed `ring-[3px]` (Tailwind's own box-shadow-based utility, no layout effect either way) and both the fill and verdict text are `position:absolute` (already out of flow, can't shift anything). The actual culprit: **the round clock**, which sits inside the board — a FLEX SIBLING of both bars in the shared column layout (`hub-section-game`, `display:flex; flex-direction:column`, matching the prototype's own `data-rc-gamewrap` structure exactly, confirmed line 431). The round clock collapses to 0 height on `phase === 'result'` (added in `2026-09-16#5` item 4) — a `GameHub`-level state that, by design, lands right around the same moment `resultPhase` reaches `'final'`. Collapsing an EARLIER sibling's height in a column-flex layout pulls every LATER sibling upward through ordinary reflow — the own bar (below the board) moves up toward the opponent bar (above it), closing the gap between them and covering VS, **without the bar's own transform ever changing** — exactly matching what the empirical capture showed, and exactly why it reads as "the bars converge twice."

## This is a timing mistake in my own earlier ticket, not a new regression — correcting it plainly

**Checked why the prototype doesn't have this problem, since it uses the identical flex-column structure.** The prototype's own clock-collapse condition (`minesClockOp`, gated on `minesPhase === 'run'`) flips the INSTANT a round ends — `minesPhase: 'done'` is set synchronously at the bust or clock-timeout, `Full Spec.html:3370`/`:3709` — which is **1500ms (or 500ms) BEFORE `converge` even starts**, not near `final`. Any reflow it causes is fully settled long before the bars ever move, so there's nothing left to shift by the time `final` lands.

**Our own `2026-09-16#5` item 4 ticket (mine) approximated this wrong.** Its own text: `phase !== 'result'` is "the cleanest local proxy `MinesBoard` already has" — a reasonable-sounding proxy at the time, checked in isolation before the bar-convergence feature existed, but it fires far too late (near `final`, not near the lock itself) once combined with the later-built convergence sequence. This is exactly the kind of "two features individually correct, wrong once combined" gap — not something introduced by any recent change, and not something to treat as someone else's mistake.

**Fix: swap the proxy for the real signal, already in scope in the same function.** `MinesBoard` already computes `myLocked` (`me?.locked ?? false`) for its own tile-rendering logic — this is the SAME signal the prototype's own `minesPhase === 'done'` tracks, firing at the exact right moment (immediately at lock, well before any bar movement begins). Swap both of the round clock's conditions (`opacity`, `maxHeight`) from `phase === 'result'` to `myLocked` — a two-token change, no new props, no new plumbing. Confirmed `myLocked` stays true for the rest of the match once set (including through `'closed'`, matching the prototype's own `minesPhase` never reverting to `'run'` mid-match) — the clock correctly stays collapsed through the whole post-lock lifecycle, not just until `final`.

---

**Ask:** small, precise, two-token fix in one file — no new state, no new props. Worth a live-page re-check once shipped given how empirical this investigation had to be to actually find it.

---
### 2026-09-18#2 — Owner/company-management feedback (not a Designer package), four distinct findings from one report: (1) "no sound when playing" is a real gap but not a bug — most games including Mines simply have zero gameplay sound effects, only PLAY/Dice/Chess do; (2) "JOIN occasionally does nothing" is a genuine, deterministic bug — the rejection notice is wired to a dead screen, never actually visible; (3) a new, deliberately minimal "rejection" sound cue, Owner-approved scope (failure-only, not success clicks, framed as a functional bug fix not a Designer-domain sound-design feature); (4) bet amount never shows for a JOINing player — a real PLAY-vs-JOIN asymmetry, same fix shape as an existing precedent            [READY TO TICKET]
From: Advisor   Re: Owner's own direct report (relayed live, no screenshots — Designer wasn't available), verified against `apps/web/src/lib/sound.ts`, `App.tsx`'s challenge/match-start flow, `GameHub.tsx`'s bet-display state, and `packages/shared/src/protocol.ts`'s `MatchStartPayload`

Four genuinely different things came in one report — kept them separate rather than folding them into one vague ticket, since they have different root causes, different owners, and (per Owner's own explicit call, made after I raised it directly) different levels of Designer involvement needed.

---

## Item 1 — "no sound when playing": not a bug, a real content gap. Not building it this round.

**Checked the whole sound system before concluding anything.** The engine itself (`lib/sound.ts`) is correctly built: mobile-autoplay unlock via the first tap/keydown, a persisted mute preference, silent-by-design failure on any missing/undecoded asset. All 4 existing sound files exist with real content — nothing corrupted or missing. **The actual finding: only 3 UI actions in the entire app ever call `play()` at all** — the shared PLAY-button click (every hub), and Dice's own roll/win chimes. Chess has a move-click. **Mines, RPS, Blackjack, Coinflip, and every other game have zero gameplay sound effects** — no tile-tap, no bust, no win/lose. Given recent testing has been almost entirely Mines, this fully explains the report with no further investigation needed — there was never sound designed for Mines beyond the one shared PLAY click.

**Not recommending building Mines-specific sound effects this round** — that's genuine sound-design content work (Designer's own domain, same category of decision as item 3 below, at a larger scale), not a bug fix. Flagging it here for the record; no action unless Owner wants to prioritize it separately.

## Item 2 — "JOIN occasionally does nothing": confirmed real, deterministic, and precisely diagnosed

**Not an intermittent glitch — this happens with 100% certainty every time a JOIN is genuinely rejected by the server.** When `takeChallenge` fails (`CHALLENGE_TAKEN`, `SELF_TAKE`, or `INSUFFICIENT_BALANCE` — confirmed these are the ONLY 3 possible rejection codes, `packages/core/src/matchmaking.ts:216`), `App.tsx`'s `onError` handler correctly sets `challengeNotice` with a real message. But `challengeNotice` is passed to exactly one place: `StakeEntryScreen` (`App.tsx:1287`, `StakeEntry.tsx:238`) — the pre-hub-migration "legacy stake-entry screen" (the code's own words, `App.tsx:864`). Checked whether this screen is still reachable for any real game: `HUB_GAMES` (`App.tsx:52`) lists all 12 currently-playable games, and `hubScreenFor()` routes every one of them to its own hub screen — `'stake-entry'` is the fallback for a game NOT in that set, which no longer exists. **The screen holding the only rendering of this notice is unreachable for every game in the app today.** The state is set correctly; nothing ever displays it.

**Fix:** redirect `challengeNotice` to the SAME mechanism `actionNotice` already correctly uses — a `position:fixed` banner pinned to the bottom of the viewport (`.ws-banner-error`, `styles.css:19`), visible regardless of scroll position or which screen is showing (exactly the property needed here, and exactly the same class of gap `2026-09-18#1` just fixed for scroll position — a JOIN attempt happens from a scrolled-down list, so anything screen-relative or scroll-relative won't reliably reach the user). Simplest version: route `setChallengeNotice(...)` through the same state `actionNotice` uses (or keep them as two states but render both through the identical fixed-banner treatment) — either way, the fix is purely making the EXISTING, correctly-computed message visible, not computing anything new.

## Item 3 — a new, deliberately minimal rejection sound, Owner-approved scope

**Owner's own proposal (a faint high-pitched click on success, low blip on rejection) is a reasonable idea in principle, but I raised a real tension in it directly rather than silently building the literal ask:** "make it imperceptible so it's low-risk" doesn't fully hold together — a cue either registers with the user (and is a genuine addition to the app's sound identity, worth Designer's eyes) or it doesn't (and doesn't solve the reported problem). **Owner's own resolution, after that discussion:** ship a minimal, genuinely subtle version now, scoped to REJECTION only (not the success-click half), framed as a functional bug fix rather than a new sound-design feature — revisit/expand once Designer is back.

**Scope, precisely:** one new sound (a short, quiet, low-frequency tone), fired on the three moments the app already recognizes as a rejected action:
- JOIN rejection (all 3 codes from item 2, now that item 2 makes them visible — sound + visible text together, matching Owner's own reasoning that a signal was needed)
- PLAY pressed with no bet armed (the existing `guideToBet()` call, `GameHub.tsx`)
- "Play a Friend" pressed (armed or not) — confirmed this is a real, live no-op today: `handlePlayFriend()`'s own comment, `/* TODO(D1): launch the friend-invite flow */` — pressing it while a stake IS armed currently does nothing observable at all, exactly matching Owner's own example

**Checked whether "PLAY with insufficient funds" (Owner's own other cited example) is actually reachable as a distinct case:** the bet-preset UI already has an `excludedStakes` prop pre-filtering stakes the player can't currently afford before they're ever selectable — so a PLAY-side insufficient-balance rejection may not be a live path today (client-side prevention already exists there). `INSUFFICIENT_BALANCE` as an explicit server error is confirmed reachable only via JOIN (a challenge's stake is set by someone else, independent of the joiner's own balance) — already covered by the item 2 fix. Flagging this precisely rather than assuming a second mechanism is needed for a path that may not exist.

**Asset generation:** there's already a clean, licensing-free precedent for exactly this — `assets/sounds/gen_move_wav.py`, a small Python script synthesizing a short tone from scratch (self-authored, CC0, no external sample). Recommend the same approach for the new reject sound: a short (~120-150ms), quiet, low-frequency tone, amplitude baked low in the generated waveform itself (the current `play()` has no separate volume/gain control — the file's own synthesis is where "quiet" actually needs to live).

**Not in scope this round, per Owner's own explicit call:** the success-click half of the original proposal. Nothing else in the app changes tone/interaction model — this is 3 specific, already-recognized rejection moments getting an added cue, not a new interaction paradigm.

## Item 4 — bet amount never shows for a JOINing player: confirmed real asymmetry, precedented fix

**Confirmed precisely.** The bet-amount display (`GameHub.tsx:1504-1508`) is gated entirely on `armedStake`, LOCAL client state populated only when a player taps a bet-preset button themselves, before pressing PLAY. A player who JOINs an already-open challenge never touches that UI at all — `handleTakeChallenge` (`App.tsx`) never sets it, so it stays `null` for the entire match, and the display renders as invisible (`opacity: armedStake == null ? 0 : 1`) — even though the real, committed stake is fully known and locked in server-side. A PLAY-initiator's `armedStake` correctly persists and displays throughout their own match; a JOIN-initiator's never does. Pure path asymmetry, not a display-logic bug.

**Fix, following an exact existing precedent rather than inventing a new mechanism:** `MatchStartPayload` (`packages/shared/src/protocol.ts:143`) already solved the identical problem once, for the opponent's name — its own doc comment: *"Lets every game show the real opponent's name on both the PLAY and JOIN paths."* Recommend the same move for stake: add a `stake` field to `MatchStartPayload` (server-confirmed, not client-echoed — consistent with this app's own standing honesty principle, and this value is safe to trust either way since a JOIN can only ever succeed at the exact stake already publicly listed), then have `GameHub.tsx` sync `armedStake` (or whatever the display ultimately reads) from it once a match starts. Touches the shared protocol package and the server's own match.start emission, not just the web client — same shape and size as the `opponentName` addition it mirrors.

---

**Ask:** items 2 and 4 are concrete bugs with precedented fixes, ready to build now. Item 3 is scoped and Owner-approved for a minimal version — ship it, don't expand it. Item 1 is a finding, not a ticket — no code this round.
...
Confirmed real, and pinned the exact mechanism down precisely rather than settling for "it's probably candidate 4." It's a genuinely different, more specific cause than any single one of the four listed — closest to #4 in spirit (it IS a server-round-trip artifact) but the actual code path is worth knowing exactly, since it changes where the fix belongs.

## The mechanism, confirmed directly in code

**It's not the board's own opacity at all — `MinesPanel`'s wrapper opacity (`args.barSlideActive ? 0.28 : 1`) is genuinely untouched by a tap, exactly as D29's own investigation already ruled out.** The dim is 24 individual TILES each independently getting Tailwind's `opacity-70` class, all at once — a different code path than the board-wide dim, which is why it doesn't match any of "board re-mounts" (#2) or "a container-level `:active` style" (#3) either.

**The trigger: `MinesBoard`'s own tile className applies `opacity-70` to any tile that's `kind === 'covered'` and NOT currently `clickable`** (`clickable = kind === 'covered' && canMove && legalSet.has(i)`, where `legalSet` is built directly from the `legalMoves` prop). The instant ANY tile is tapped, `App.tsx`'s shared `handleMakeMove` — used by every hub, not Mines-specific — calls `setLegalMoves([])` right after a successful submit ("disable buttons after a successful submit," its own comment). With `legalMoves` empty, `legalSet.has(i)` is false for every square, so `canMove`/`clickable` goes false for the WHOLE BOARD until the next WS broadcast arrives with a fresh `legalMoves` array reflecting the post-move state — and every one of the 24 other still-covered tiles picks up `opacity-70` for that entire round-trip window. This fires on EVERY tap, safe or mine, matching D29's own "happens before the tile reveals" description exactly. Worth knowing the visual character too: this tile-level opacity has no transition set (only `background 200ms ease` does), so it snaps instantly rather than fading — a sharper, more jarring flicker than a smooth 380ms board-wide dim would produce, which fits "darkens the whole board for a moment" better than any of the 4 candidates' own framing would predict.

## Confirms the caveat at the top of D29 — no optimistic reveal, just stop dimming the wait

**The tapped tile's own reveal is correctly, honestly gated on server confirmation already — `cellKind()` only ever returns `'safe'` once `myUncovered` (server-confirmed) includes the index, never optimistically.** This is exactly right per the seed-is-server-held decision the caveat itself cites, and nothing about this ticket should change it. The actual fix is precisely what the caveat predicts: leave the honest reveal alone, stop the OTHER tiles from visually reacting to a state (`legalMoves` briefly empty) that has nothing to do with them.

**Fix, scoped narrowly:** drop `opacity-70` from the covered-tile styling — keep `disabled={!clickable}` (the functional gate, correctly prevents a double-tap mid-round-trip) but stop pairing it with a visible dim. A `'covered'` tile only ever exists while the player is actively, un-locked mid-round (confirmed via `cellKind()`: once `myLocked`, the fallback branch returns `'autoSafe'`, never `'covered'`) — so there's no OTHER legitimate state a covered tile could be in that this opacity was protecting; removing it doesn't hide a real "genuinely can't play at all" signal, it only removes the false-positive one. `cursor-not-allowed` can stay if desired — it's a mouse-only cosmetic, not what D29's complaint is about.

## Scope

The shared mechanism itself (`setLegalMoves([])`) is correct and needed for other games (e.g. RPS/Chess genuinely should disable further input until the next legal-moves update lands) — not touching it. The fix is entirely local to `MinesBoard`'s own tile className in `MinesHub.tsx`; Mines is the one place this shared, correct mechanism happens to paint across 24 tiles at once instead of one or two buttons, which is presumably why it reads as "the whole board darkens" here specifically and (as far as I know) hasn't been reported for any other game.

---

**Ask:** small, precisely scoped, no architecture change — one class removed from one conditional in one file.
...
Confirmed real: today, once `resultPhase` reaches `'final'`, nothing ever moves it anywhere else except a brand new match starting — the bars stay converged, the board stays dimmed at 0.28, forever. Good news: the fix composes cleanly with what's already built rather than needing new parallel plumbing, because `resultConverge` (the prop already driving bar-shift + board-dim, and about to drive VS + the captions once `2026-09-17#3` ships) can simply be redefined to exclude the new phase — everything downstream releases for free.

---

## The fix: one new phase value, one new opt-in prop

**Add `'closed'` as a 5th `ResultPhase` value**, reachable only from `'final'`. Redefine `resultConverge`'s own computation (currently `resultPhase !== 'idle'`) to `resultPhase === 'converge' || resultPhase === 'reveal' || resultPhase === 'final'` — i.e., excluding both `'idle'` and the new `'closed'`. Traced the cascade precisely before recommending this, rather than assuming it "should" work: `resultConverge` already feeds `barSlideActive` (`GameHub.tsx:657`, `matchForming || searching || Boolean(resultConverge)`) directly, which already drives BOTH the bar-shift transform AND `MinesPanel`'s own board-dim (`args.barSlideActive ? 0.28 : 1`) — so redefining `resultConverge` alone correctly releases both, no separate wiring needed for either. Once `2026-09-17#3`'s VS-label fix ships (`matchForming || resultConverge`), it inherits the same release for free too — worth building that fix with this ticket's own phase value already in mind, since D26 hasn't shipped yet.

**One thing does NOT derive from `resultConverge` and needs its own explicit extension:** the opponent's icon-strip visibility (`oppGemRow`'s `visible={resultPhase === 'reveal' || resultPhase === 'final'}`) needs `'closed'` added to that list too — D27 is explicit that this strip stays populated after dismiss (`Full Spec.html:3765`, `oppGemRowOp` is 1 for `mGemsKept && mr === 'closed'` specifically), and without the addition it would incorrectly fade back out. The player's own row is already unconditional — no change needed there. Confirmed the ring (`ownBarVerdict`) and the win-fill animation are driven by entirely separate mechanisms (`phase==='result'`/`outcome`, not `resultPhase`/`resultConverge` at all) — neither needs any change, matching D27's own "stays exactly as it was" list precisely.

**The dismiss trigger itself: a new opt-in `GameHubProps` callback, not a full-screen overlay.** Checked the actual JSX nesting before designing this (not assumed): `OpponentSlot`, `renderGameArea(areaArgs)`, and `OwnSlot` are all direct children of the SAME `hub-section-game` element — VS label, both bars, and the board are all already inside one container. A single `onClick` on that container, gated by a new optional prop (e.g. `onSectionTap?(): void`, `undefined` for every hub but Mines — the same "byte-identical no-op elsewhere" shape as every prior Mines-only addition), covers the board, both bars, and the empty space around them in one place — Mines' own handler just checks `resultPhase === 'final'` before calling `setResultPhase('closed')`.

## A deliberate simplification vs. the prototype's own literal mechanism — flagging, not silently deviating

The prototype's own approach is a truly page-wide transparent overlay (`z-index:6`, covering the whole app including the bottom nav) with a manual `getBoundingClientRect` check against the Play button specifically, since Play sits underneath that same overlay and needs to keep working through it. **Our own layout doesn't need that check at all** — `hub-section-game` and `hub-section-play` (the Play button's own container) are separate sibling sections, so scoping the dismiss handler to just `hub-section-game` means Play is never covered in the first place; it keeps working with zero special-casing. This satisfies every concrete example in D27's own "Check" list (board, bars, empty space near them) without the added complexity and risk of a truly page-wide tap-catcher — which would also mean intercepting taps meant for the bottom nav (Menu/Games/Account/Rewards/Chat) or the wallet button while a result is showing, a real behavior change beyond what any of D27's own examples call for. Recommend the narrower scope; flagging the literal alternative in case Designer specifically wants navigation itself blocked during the reveal, which seems unlikely but isn't something to assume silently either way.

## The "same on Dice" note — flagging a scope question, not expanding silently

D27 closes with "Same on dice (the handler branches on view at line 3775)." Checked: Dice has no bar-convergence or board-dim mechanism to release at all — `mConverged = mActive && !isDice` explicitly excludes Dice from this entire mechanism (confirmed again directly, unchanged since it was first noted in `2026-09-16#7`); Dice's own reveal choreography (`2026-09-16#4`/`#622`) is a structurally different, bars-never-move design. There's nothing for a Dice dismiss tap to release. Not expanding scope there on my own read of one line in a Mines-focused report — flagging back in case Designer meant something narrower for Dice specifically (e.g., just re-enabling PLAY-button interaction after its own result lands, if that's even currently blocked) rather than the bars/dim mechanism this ticket is about.

---

**Ask:** small, self-contained, and the good news is it composes with `2026-09-17#3` (still unshipped) rather than needing to be sequenced after it — recommend building both with the `'closed'`-excludes-from-`resultConverge` design in mind from the start. One scoping call flagged (page-wide vs. game-section dismiss) with a recommendation, not a silent decision; one scope question on the Dice mention.

---
### 2026-09-17#3 — D26, the result reveal: 2 of the 3 listed gaps are real (the VS label never shows during the result sequence; the gem-count captions above/below the bars don't exist at all — this is the long-tracked `2026-09-12#2` item 3(c) finally landing), but the 3rd (opponent gem strip) is a correction, not a build — it's already fully implemented and test-covered since `2026-09-16#7`, confirmed by direct code read AND existing passing test coverage. Converge geometry and the final ring/fill are also already correct — Designer's own "Check" list will pass on those today            [READY TO TICKET]
From: Advisor   Re: Designer's D26 report (three-state result sequence: VS label, gem-count captions, opponent gem strip), verified against `GameHub.tsx`, `MinesHub.tsx`, `apps/web/src/test/MinesHub.test.tsx`'s existing passing coverage, and the prototype's own source (`Full Spec.html:433/467/690` markup, `:3419` converge geometry formula, `:3753-3754`/`:3779-3782` computed values, `:3787-3789` ring/fill)

Of the three "missing" elements D26 lists, two are genuinely missing and one is a correction to Designer's own diagnosis — it's not missing, it already shipped in `2026-09-16#7` and has a passing test proving it. The converge geometry and final ring/fill sections of D26 are both explicitly "already specified, unchanged" per Designer's own text, and I independently confirmed both are already implemented correctly — nothing to do there either. The real, actionable scope is genuinely small: extend the VS label's existing visibility condition by one clause, and build the gem-count captions, which is exactly the `2026-09-12#2` item 3(c) gap flagged as still-open in `2026-09-16#6`/`#7` and never built.

---

## Item 1 — the VS label: real gap, one-line fix

**Confirmed missing exactly as described.** `hub-match-vs` (`GameHub.tsx`) — the shared VS label used across every hub — has its opacity gated on ONE condition only: `matchForming`. It has no awareness of Mines' own post-bust result sequence at all, so it stays at `opacity:0` throughout `converge`/`reveal`/`final` — confirmed directly, not inferred.

**Fix is a single clause added to an already-correct mechanism.** `resultConverge` (`resultPhase !== 'idle'`, added in `2026-09-16#7`) is already threaded into `GameHub`'s own scope, `undefined`/false for every hub but Mines — extend the VS label's opacity condition to `matchForming || resultConverge`. This exactly mirrors the prototype's own gate (`rpsMatchVsOp: rpsMatching || mConverged ? 1 : 0`) — Mines-only, byte-identical no-op for every other hub. No change needed to the VS label's POSITION — it's already centered on the section's own vertical midpoint (a deliberate, already-reasoned divergence from the prototype's literal fixed pixel value, explained in the component's own comment, since our board sizes are responsive not fixed-width) — this already coincides with where the converged bars land, since that's what the convergence math targets in the first place.

## Item 2 — gem-count captions: real gap, the long-tracked `2026-09-12#2` item 3(c)

**Confirmed genuinely absent — `oppGemText`/`myGemText` don't exist anywhere in this codebase.** This is exactly the SEPARATE mechanism flagged as still-open back in `2026-09-12#2` item 3(c) and re-flagged in both `2026-09-16#6` and `2026-09-16#7` ("a genuinely separate, still-open piece... plain text spans positioned outside the bar box... needs either a new prop or an extension of the existing slot API") — D26 is the first package to actually ask for it directly. `gemRow` (the icon strip) renders content INSIDE the bar row; these captions need to sit OUTSIDE it, above the opponent's bar and below the player's own — a genuinely different mechanism, not an extension of `gemRow`.

**A precise asymmetry in the source, worth preserving exactly rather than "fixing" into consistency:** re-checked both text-generation lines directly (`Full Spec.html:3779/3781`). The opponent's caption is ALWAYS `"{n} gems"` — no singular case at all, even at 1. The player's own caption DOES singularize: `"{n} gem"` at exactly 1, `"{n} gems"` otherwise. This isn't a typo to correct — it's what the source actually does on both sides; port it as-is, including the asymmetry.

**A second, easy-to-miss distinction from the already-shipped icon strip:** the icon strip's OWN row (`ownGemRow`) is live and continuous — visible from the very start of a round, well before any bust (`2026-09-17#1` item 3, already shipped). The CAPTION is NOT live the same way — its own gating (`myGemTextOp: (!isDice && mGrown) ? 1 : 0`, where `mGrown = mActive` requires the result sequence to have actually started) means it only ever appears once `resultConverge` is true, same as the opponent's caption timing, not from the start of the round. Don't reuse the icon row's "live from the start" pattern for the caption — they have genuinely different visibility windows despite both being "the player's own gem info."

**Mechanically:** both bar wrappers (`data-rc-oppbar`/`data-rc-playerbar`) are already `position:relative` — no structural change needed there, just two new absolutely-positioned children: opponent's `bottom:calc(100% + 6px); right:4px`, own `top:calc(100% + 6px); right:4px`, both `color:var(--rc-green)` (already a token, confirmed in `index.css`), Space Grotesk 700 13px, `white-space:nowrap; pointer-events:none; transition:opacity 320ms ease`. Being DOM descendants of the bar itself, they'll move with it during the converge slide for free, no extra wiring needed for that part. Recommend the same prop-slot split already established for `gemRow`: two new optional slot props on `GameHubProps` (rendered as the absolutely-positioned wrapper, owned by `GameHub`), Mines supplying just the text + visibility.

## Item 3 — opponent gem strip: **correction — this is already built, not missing**

**Checked directly against both the code and existing test coverage before writing this: the opponent's icon-strip gem row is already fully implemented, exactly matching D26's own cited behavior.** `oppGemRow` (`MinesHub.tsx`) is already `undefined` until the result sequence starts, then a `<GemCountRow>` bound to `oppGemCount = opp?.score ?? 0`, capped at 22, with `visible={resultPhase === 'reveal' || resultPhase === 'final'}` — hidden during `converge`, fading in at `reveal`. This shipped in `2026-09-16#7` item 5. There's already a passing test (`MinesHub.test.tsx`, "item 5: gem-count rows appear on the sequence's own beats") that explicitly asserts opacity `'0'` at `converge` and `'1'` at `reveal`, with the count capped at 22. Nothing to build here — recommend not touching this, and flagging back that the "jumps straight to the ring" perception D26 opens with is fully explained by items 1+2 being missing (no VS pop, no caption text calling attention to the beat), not by this element being absent.

## Converge geometry and final ring/fill — also already correct, independently re-confirmed

Both sections D26 itself marks "already specified, unchanged" — checked anyway rather than taking that at face value. The live-measurement formula (`mid` = midpoint between opponent-bar-top and player-bar-bottom, opponent shifts to `mid−71`, player to `mid+23`) matches `GameHub.tsx`'s own `useLayoutEffect` computation exactly, already re-used for the result-phase convergence (no new code needed there per `2026-09-16#7`'s own doc comment). The ring/fill colors and `final`-only gating were confirmed correct in `2026-09-16#7`. Nothing to do in either area.

---

**Ask:** items 1-2 are the real, ready-to-build scope — item 1 is genuinely a one-line change; item 2 is a small, well-specified new mechanism with two precise asymmetries (singular/plural, and the caption-vs-icon-row visibility timing) worth getting right the first time. Item 3 needs no code — just confirming back that it already works.

---
### 2026-09-17#2 — D25, both items real: item 1 is our own D24 fix recurring ONE LEVEL DEEPER — I fixed the wrapper-vs-tile relationship but missed that the reveal-pop `motion.div` itself is still a bare, percentage-width-SVG-containing flex item, the exact same bug class reappearing inside the fix; item 2 is the exact `flex:1`/`shrink-0` tradeoff I flagged in D24 and left for reconsideration — now confirmed, by direct measurement, to be the actual cause. Both empirically proven against the LIVE deployed page, not reasoned from source — item 1 needed real investigation since my own repro initially (and misleadingly) measured it as already-fixed            [READY TO TICKET]
From: Advisor   Re: Designer's D25 report (icon/halo drift persists, gem strip right-aligned), verified against the LIVE deployed page (`rapidclash-65hoiaulbq-uc.a.run.app`) via injected-DOM measurement — not a synthetic repro this time, since a synthetic repro of the fix I'd already shipped gave a false negative — and `GameHub.tsx`/`MinesHub.tsx`'s current source

Both items are real. Item 1 took real digging: my own hand-built repro of the current (shipped) structure measured as CORRECTLY CENTERED — contradicting Designer's screenshot — until I stopped trusting a synthetic approximation and injected the EXACT structure into the actual live page instead, which reproduced the bug immediately. The discrepancy turned out to be genuinely informative, not a fluke: it's a second occurrence of the exact CSS mechanic D24 fixed, recurring one nesting level deeper than where I fixed it.

---

## Item 1 — icon/halo drift persists: D24's own fix has the same bug one level down

**Designer's new diagnosis (halo as a second flex item) doesn't match our code either — but the underlying CSS mechanic Designer is circling is real, and it's the SAME one from D24, not a new one.** Our halo is not, and was never, a plain sibling `<svg>` with no position — it's already wrapped in `position:absolute` divs. What's actually happening:

D24's fix wrapped the icon's existing `motion.div` (used for the reveal-pop animation) in a NEW `<div className="absolute inset-0 flex items-center justify-center">`. This correctly solved the problem AT THAT LEVEL — the wrapper itself measures dead-center against the tile, confirmed. But **the `motion.div` INSIDE that wrapper is itself a bare, non-absolutely-positioned flex item whose only content is a percentage-width `<svg>`** — exactly the same ambiguous-sizing pattern D24 diagnosed and fixed at the outer level, now reappearing one level deeper. Measured directly on the live page (injecting the real markup and reading `getComputedStyle`): the inner `motion.div` gets stretched to the wrapper's full width (94px, matching the tile) instead of shrinking to its content, and the SVG inside it — having nothing to be centered against anymore — sits flush at that box's own start. This is confirmed as the live, current behavior via direct injection into the deployed page (not a synthetic approximation) and screenshotted side by side against a corrected structure for a definitive before/after.

**The fix has to eliminate the intermediate flex-item layer entirely, not add another wrapper around it.** The most robust option, verified working across repeated fresh page loads: make the icon a `motion.svg` directly (Framer Motion supports animating SVG elements, not just divs) with `position:absolute` on the SVG itself and no offsets — collapsing "wrapper div → motion.div → svg" down to just "motion.svg, absolute". Apply the same collapse to the halos (currently `absolute inset-0 flex-centered wrapper div → plain svg` — drop the wrapper, `position:absolute` straight on the halo SVG, matching Designer's own literal recommendation and the prototype's own markup exactly). Verified via side-by-side injection into the live page: the current structure reproduces the exact drift from both Designer's screenshots; the collapsed structure (position:absolute directly on both SVGs, no wrapper divs at all) measures pixel-identical centers for both layers, consistently across repeated loads.

**Scope:** applies everywhere `GemIcon`/`MineIcon`/`GemHalo`/`MineHalo` render inside a tile — `'safe'`, `'autoSafe'`, `'mine'`, and `'bustedOn'` `CellKind`s all currently go through the wrapper-div-around-motion.div pattern and are all affected identically; none of them were actually fixed by D24, they just hadn't been screenshotted again yet. Preserve the DOM-order-determines-paint-order rule from `2026-09-16#6`: whichever layer needs to paint on top (the icon, over its halo) must still come after it in DOM order, since both will now be direct `position:absolute` siblings with `z-index:auto`.

## Item 2 — gem strip right-aligned: exactly the tradeoff flagged in D24, now confirmed as the actual cause

**This is not a new bug — it's the consequence of the `flex-1`/`shrink-0` choice `2026-09-16#7`/`D24` made deliberately, which I flagged for reconsideration and which wasn't changed.** Measured directly: the bar's name span (`GameHub.tsx`, both `OpponentSlot` and `OwnSlot`) carries `flex-1` — it grows to fill essentially the entire row's available width, regardless of how short the actual name text is. The gem-row span placed after it (`shrink-0`, content-sized) has nowhere to go but immediately after that now-enormous box — which visually lands it at the bar's far right edge, exactly matching both screenshots (D24's own and this one).

**Fix, safely scoped to Mines only, exactly as flagged last time:** since `gemRow` is `undefined` for every hub but Mines, make the name span's `flex-1` conditional on `gemRow`'s absence — when a gem row IS present, drop `flex-1` from the name (matching Designer's own citation: "no `flex:1`... takes only its own width") and give the gem-row's own wrapper `flex-1 min-w-0` instead of `shrink-0`, so it soaks up the freed space and left-aligns within it (`GemCountRow`'s own inner flex already has no explicit `justify-content` override, which defaults to flex-start — no change needed there). Every other hub keeps the exact byte-identical `flex-1` name behavior, since `gemRow` is always `undefined` for them.

**Two small, low-priority spacing notes, not blocking:** Designer's citation specifies a 12px row gap + 6px strip margin (18px total) between name and first gem; our shared row gap is `gap-2.5` (10px, used identically across every game's bar, not Mines-specific) with no additional margin on the gem row today. Bringing this fully in line would mean either changing the shared 10px gap (touches every other game, not recommended) or adding a small Mines-only `margin-left` on the gem-row wrapper to close the difference — a minor polish item, not the cause of the reported bug. Separately, `GemCountRow`'s own `maxWidth:90` inline cap should likely be dropped now that the wrapper will actually have room to grow via `flex-1` — Designer's citation for the strip has no max-width at all, just `flex:1 1 auto`.

---

**Ask:** item 1 is a real, if narrow, structural fix (motion.svg not motion.div, no wrapper divs) — recommend verifying centering on the actual deployed page after shipping, not just in tests, given how a synthetic repro of the current code gave a false negative for me this round. Item 2 is a small, already-understood, safely-scoped fix.

---
### 2026-09-17#1 — D24, three more on Mines: icon/halo drift is a real but different bug than Designer's own diagnosis (empirically proven — a CSS percentage-width-in-an-auto-flex-item quirk, not "corner vs centre" positioning), the post-bust sequence is gated on the WHOLE MATCH settling instead of just MY OWN round ending (a real regression in how `2026-09-16#7` shipped, needs a genuine hold-state-machine fix plus a `holdResultMs` consequence Designer's own brief doesn't mention), and the live own-gem-bar is a straightforward un-gating of an already-built component. One open question (a lock reason Designer's own source has no case for), one already-considered tradeoff flagged for reconsideration            [READY TO TICKET]
From: Advisor   Re: Designer's D24 report (icon/halo drift, result-runs-on-your-own-clock, live own-gem-bar), verified against `MinesHub.tsx`, `GameHub.tsx`, `packages/games/mines/src/mines.ts`, a live empirical repro of the exact tile structure, and the prototype's own source (`Full Spec.html:474` tile flex-centering, `:3370/3705-3711` the two result-sequence trigger origins, `:674-675`/`:449` gem-strip placement+binding)

All three items are real. Item 1 needed the same kind of empirical proof D23's stacking bug did — Designer's stated mechanism doesn't match what's actually happening in our code, though the FIX ends up being effectively the technique Designer describes. Item 2 is the most consequential: last night's `2026-09-16#7` shipped a real, working reveal sequence, but over-applied the honesty principle — it gated the entire visual sequence (including the purely-cosmetic board-dim/bar-slide, which reveals no data at all) on full match settlement, when only the number-revealing beats need that. Item 3 is a clean, low-risk un-gating of a component that's already fully built.

---

## Item 1 — icon/halo drift: confirmed real, root cause empirically proven, and it's narrower than Designer's own framing suggests

**Designer's stated cause doesn't match our code, but isn't wrong about the fix.** Designer's diagnosis: "the halo is being positioned from the tile's corner rather than its centre," implying `top:0;left:0` or `inset:0` or a `translate(-50%,-50%)` literal is present and needs removing. Checked directly: `GemHalo` (and `MineHalo` in the `bustedOn` case) are wrapped in `<div className="absolute inset-0 flex items-center justify-center">` — `inset:0` IS present, but on the HALO's wrapper, and this technique is not actually the bug — I proved this empirically (below).

**What's actually happening, proven with an isolated repro of the real tile structure (not reasoned out on paper):** `GemHalo`'s wrapper (`absolute; inset:0` + internal flex-centering) measures dead-center — confirmed via `getBoundingClientRect()`, its center lands exactly on the tile's own center. The problem is the OTHER layer: `GemIcon` (for a `'safe'` tile) is NOT wrapped in anything special — it's a bare `motion.div`, an ordinary normal-flow child of the tile's own flex container. Measuring it directly: its wrapping `div`'s computed `width` was STRETCHED to the tile's FULL width (not shrink-to-fit, which is what a centered flex item needs) — a real, if obscure, CSS interaction: a normal-flow flex item with `flex-basis:auto` whose content is a REPLACED element (the SVG) using a PERCENTAGE `width` creates an ambiguous sizing dependency (the item's size depends on the percentage, the percentage's resolution depends on the item's size) that Chromium resolves by giving the flex item the full container width — at which point `justify-content:center` has zero free space left to center anything, and the SVG itself (a block element, no auto margins) just sits flush at the START of that now-full-width box. Screenshotted both the current (buggy) structure and Designer's own suggested fix side by side: the buggy version reproduces the exact upper-left-icon/lower-right-halo drift from the screenshot; giving the icon `position:absolute` (Designer's own suggested technique — no wrapper, no offsets, relying on the flex container's static-position alignment) measures pixel-identical to the tile's own halo, dead center. Absolutely-positioned elements resolve percentage widths against their CONTAINING BLOCK (the tile, a definite size) rather than against an ambiguous auto-sized flex-item box — that's why it sidesteps the bug entirely.

**Scope, precisely — only 2 of the 4 `CellKind` branches are affected.** `'mine'` and `'bustedOn'` (the mine-hit tile) already wrap their icon in the SAME `absolute inset-0 flex items-center justify-center` div the halo uses (added for `2026-09-16#6`'s stacking-order fix) — that technique is proven correct above (it's exactly how `GemHalo` already measures dead-center), so those two are NOT affected. Only `'safe'` (a tapped-open gem — the screenshot's own drifting gems) and `'autoSafe'` (a ghosted, never-tapped tile revealed on bust — same bare-motion.div pattern, no halo present there to make it as visually obvious, but the icon itself is equally off-center) need the fix.

**Fix:** wrap `GemIcon` (in the `'safe'` branch) and the `'autoSafe'` branch's icon in the same `<div className="absolute inset-0 flex items-center justify-center">` already used 3 other places in this file — the minimal, most-consistent-with-existing-code option (Designer's own literal "bare SVG, position:absolute, no wrapper" also measured pixel-identical in my repro, so either technique works if a smaller diff is preferred).

## Item 2 — result runs on your own clock: real, and more consequential than a timing tweak

**Confirmed exactly as described, and traced to a specific over-application of last night's own honesty-principle fix.** `2026-09-16#7`'s controlling effect (`MinesHubScreen`) gates ALL THREE result beats — including `'converge'`, which is purely a board-dim + bar-slide, revealing no number or outcome at all — behind `props.lastOutcome && props.lastSettlement`, i.e. the WHOLE MATCH being settled (both players locked). That's stricter than the honesty principle actually requires: only beats that show a NUMBER (`'reveal'`, the opponent's count) or an OUTCOME (`'final'`, the ring) need confirmed data. `'converge'` and your OWN gem count (item 3) are never at risk of being wrong — they don't depend on the opponent at all.

**The corrected mechanism, precisely:**
- **Trigger `'converge'` off MY OWN lock, not match settlement.** `MinesBoardView.locked` (already in the view shape) flips true the instant MY round ends, for any of the 3 real ways it can (`packages/games/mines/src/mines.ts:238-266`/`382-398`): bust, a perfect 22-tile clear, or my own clock timeout — independent of whether the opponent is still playing. Use this (generalizing the current `didBust`/`bustedOn !== undefined` check, which only catches the bust case) as the arm condition.
- **The delay before `'converge'` depends on WHY I locked** — Designer's own two cited origins, confirmed against the prototype's source: a mine hit is 1500ms (`:3711`), the clock running out is 500ms (`:3370`). The reason is derivable ENTIRELY from already-stable view fields, no new server/protocol work needed: `bustedOn !== undefined` → bust (1500ms); `uncovered.length === 22` (the ruleset's own safe-tile count, `packages/games/mines/src/board.ts`'s `SAFE_COUNT`, mirroring how this file already keeps its own local `ROUND_SECONDS` mirror) → cleared; otherwise (locked, not busted, not cleared) → timeout (500ms).
- **Open question, not guessed:** the prototype has NO "cleared all 22" code path at all (checked directly — its own `reveal()` handler has no early-win branch; the simulation just waits for the clock even on a perfect clear), so there's no source to confirm a timing for it against. Recommend defaulting a clear to the same 500ms as a timeout (both are non-bust endings) but flagging this precisely rather than silently picking, since Designer's own material simply doesn't cover this case.
- **The hold, exactly as Designer describes:** once `'converge'` fires (on my own schedule, always), check `opp.locked`. If already true → schedule `'reveal'` at +820ms and `'final'` at +700ms after that, running straight through (today's already-correct relative spacing, unchanged). If not yet true → stay at `'converge'` (dimmed board, bars together, my gem row showing, opponent's hidden — exactly the existing `oppGemRow`/`ownGemRow` visibility gating, unchanged) until an effect observes `opp.locked` flip true, THEN schedule `'reveal'`/`'final'` at the same +820/+700 relative to THAT release moment, not the original converge timestamp.
- **A real consequence Designer's own brief doesn't mention, found by checking how the existing pieces actually connect: `holdResultMs` (the delay before the win/lose RESULT OVERLAY POPUP appears) needs to become dynamic, not the fixed `RESULT_FINAL_MS` constant it is today.** `GameHub`'s hold-and-show-overlay effect only fires once `currentMatchId` clears — i.e. once the match is TRULY over (both players locked) — and reads whatever `holdResultMs` equals at that exact moment. Today's fixed 3020ms constant is only correct for the "opponent already done" case (where match-end and my own lock are the same instant). In the hold case, match-end happens LATER (when the opponent finishes) — using the fixed 3020ms there would hold the in-match view for 3020ms MORE after that, well past when `'final'` has already visually landed. The correct value is `max(0, reasonDelay + 1520 − elapsed-since-my-own-lock)`, recomputed on every render — this formula also correctly reduces to exactly `RESULT_FINAL_MS` (3020) in the straight-through case, so it's a strict generalization, not a parallel mechanism.
- **A pre-existing limitation, unchanged, worth one line rather than silently inheriting it unremarked:** on a fresh mount/reconnect where `me.locked` is already true, there's no server-provided "when did I lock" timestamp to compute elapsed time from — this file's own existing comment already accepts this same imprecision for the current bust-only case (arms the sequence fresh from "now" rather than the true lock instant); the fix above doesn't solve this further, just carries the same accepted approximation forward.

## Item 3 — live own-gem-bar: real, and mostly just removing an over-restrictive gate

**Confirmed exactly as described.** `ownGemRow` today is gated behind `didBust && resultPhase !== 'idle'` — meaning it only ever appears AFTER the round has already ended, never live. Designer's own citation (`minesGems` bound directly to opened-safe-tile count, no result-state involvement) is correct — `myGemCount` (`me?.uncovered?.length`) is already computed live in this file, just not passed through unconditionally. **Fix:** drop the `didBust &&` gate entirely for `ownGemRow` — always pass `<GemCountRow count={myGemCount} visible />`; `GemCountRow`'s own existing `count <= 0 → null` check already produces the "fades in with the first gem" behavior Designer describes, no new logic needed. The opponent's row keeps its existing `'reveal'`/`'final'`-only gating (now driven by the corrected phase machine above) — nothing changes there.

**One already-considered tradeoff worth flagging for reconsideration, not silently overriding.** Designer's own citation asks for `flex:1 1 auto; min-width:0` on the gem strip. `GameHubProps.oppGemRow`'s own doc comment records that this was already considered and deliberately rejected last night — giving the gem row `flex-1` would mean taking `flex-1` off the shared name span, which would break long-name truncation on every OTHER hub using this same bar component (`shrink-0` was the safer call at the time). Since `gemRow` is Mines-only (`undefined` → byte-identical no-op everywhere else), this could be done safely by moving `flex-1` conditionally — only when `gemRow` is actually present — without touching any other hub's behavior. Recommend this rather than either blindly following Designer's literal citation (which would risk the name-truncation regression PM already flagged) or silently keeping `shrink-0` against Designer's explicit ask.

Two small, mechanical corrections while matching Designer's citation precisely: `GemCountRow`'s transition is `320ms` today; Designer's citation says `280ms` — a one-value fix. `oppGemRow`'s own visibility check can drop its now-redundant `didBust &&` (once `resultPhase` itself is correctly gated on the fixed state machine above, checking `resultPhase` alone is sufficient).

---

**Ask:** items 1 and 3 are small and independently shippable. Item 2 is the real work here — a genuine state-machine fix plus the `holdResultMs` consequence — recommend it as its own, careful PR given the reconnect/timing edge cases involved, with the 'cleared' timing question confirmed (or accepted as a reasonable default) before or alongside shipping.

---
### 2026-09-16#7 — D23 items 4-5, scoped as their own ticket: the post-match bar-convergence sequence + ring/fill colours, and the gem-count icon rows — finally closing out `2026-09-12#2`'s long-open items 3(a)/3(c). Includes a self-caught correction to a colour I misquoted in my own `2026-09-16#6` dispatch — Mines' win colour is `#22C55E`, not Dice's `#16A34A`               [READY TO TICKET]
From: Advisor   Re: `2026-09-16#6` items 4-5, re-verified fresh against `GameHub.tsx`'s current (post-#624/#625) state and the prototype's own source (`Full Spec.html:3378-3385/3520-3530/3690-3711` result-sequence timing, `:3765-3789` gem-row + ring/fill values, `:436-437`/`:660-661` the existing 620ms bar-transform transition, `apps/web/src/index.css:297` `--rc-loss`)

**Correction to my own `2026-09-16#6` dispatch, caught before anything was built on it:** I cited the post-match ring's win colour as `#16A34A`, matching Dice's already-shipped `winRingColor` fix (`2026-09-16#4`/`#622`). That's wrong for Mines. `#16A34A` is `DiceHub.tsx`'s own `DICE_WIN_GREEN` — a value Designer explicitly asked for on Dice's cube-number recolor and its bar ring, deliberately diverging from the prototype's own shared ring mechanism (already documented, `2026-09-16#4`'s own ticket text). Mines has no per-number recolor at all — its only win/lose/draw tell is the shared `playerBarRing`/`winFillAnim` mechanism, whose own literal (`Full Spec.html:3787/3789`, re-read directly just now) is unambiguously **`#22C55E` win / `#FF3E5E` loss / `#FF8A1E` draw** — not Dice's pair. Below uses the corrected values throughout.

---

## Item 4 — post-match bar convergence + correct ring/fill colours (closes `2026-09-12#2` item 3(a))

**Confirmed still fully unbuilt.** `GameHub.tsx`'s `barSlideActive` (line 632) is unchanged: `matchForming || searching` only — no post-match state exists yet.

**Exact sequence, re-derived directly from the prototype's own trigger chain (`Full Spec.html:3705-3711`, `:3378-3385`), scoped to the BUST path specifically** (matching D23's own literal scope — "what happens when you hit a mine"; the separate clock-runs-out/no-bust round-end path uses a different, shorter trigger, `500ms` not `1500ms`, at `:3370` — not audited here, flagging rather than assuming it needs the identical fix):
- **t=0** — mine hit (the already-shipped `rcMineJump` bounce starts here, independently timed, no dependency on the below).
- **t=+1500ms** — `startMinesResult()` fires: `minesResult` flips to `'converge'`, gem counts are captured. Bars begin sliding via `transform:translateY()`, `transition:transform 620ms cubic-bezier(0.3,0.9,0.32,1)`.
- **t=+2320ms** (1500+820) — `minesResult` flips to `'reveal'`: the gem-count elements (item 5) become visible.
- **t=+3020ms** (1500+1520) — `minesResult` flips to `'final'`: the ring + fill land (`mOutcome` computed from the captured gem counts).

**The good news: the visual mechanism itself is already fully built and reusable, exact transition included.** `GameHub.tsx`'s existing `barShiftY` prop already applies `transform:translateY(Npx)` with `transition:transform 620ms cubic-bezier(0.3,0.9,0.32,1)` (confirmed at `GameHub.tsx:998`/`:1080`) — character-for-character the same transition the prototype uses for this exact convergence (`Full Spec.html:436-437`/`:660-661`). **What's missing is purely a second trigger + a second magnitude source, not a new animation mechanism:**
- **Trigger:** a new, Mines-scoped local state (NOT extending the generic `barSlideActive`, which every game shares and specifically means "pre-match slide") that flips true at t=+1500ms post-bust and clears once the result closes — recommend building this the same way D21's own reveal state machine went in (`2026-09-16#4`/`#622`): a locally-timed sequence inside the Mines game area, signalling the hub per beat, not a new fixed-timing prop on `GameHub` itself.
- **Magnitude:** the prototype uses a DIFFERENT fallback constant for this state (`mRShift` `{o:100,p:-100}`, `:3529`) than the pre-match slide (`mShift` `{o:123,p:-123}`, `:3520`) — since Mines already uses `matchBarSlide="measured"` (live DOM measurement, not a hardcoded number) for the pre-match slide, recommend re-measuring live a second time at the point of convergence rather than hand-tuning a second fallback constant — consistent with why 'measured' mode exists at all, and naturally produces whatever the real post-match layout difference is instead of guessing at one.
- **Board dim:** `MinesPanel`'s existing `args.barSlideActive ? 0.28 : 1` (`MinesHub.tsx:443`) needs to also dim during this new post-bust state — same `0.28`, matching the prototype's own `minesBoardOp: rpsMatching || mConverged ? 0.28 : 1` (`:3756`) exactly.

**Ring/fill colours — genuinely missing for Mines today, same class of gap Dice had before `2026-09-16#4`.** Confirmed: `lossRingColor`/`winRingColor`/`winFillColor` (`GameHub.tsx:791/797-798`) are ALL `gameId === 'dice' ? ... : undefined` — Mines gets none of the three, falling back to the generic shared `outlineClasses()` (`--rc-green` #34d399/#0b8f5a win, `--destructive` loss, `ring-amber-400` draw, `hub-shared/slotReveal.tsx:44-49`) — none of which match the prototype's own values for this mechanism. **Fix:** extend the same 3 props to Mines, but with MINES' OWN correct literals — `#22C55E` win, `#FF3E5E` loss (already exists as `--rc-loss`, `index.css:297` — reuse the token, don't re-literal it), `#FF8A1E` draw (new, doesn't exist as a token yet). **Do not reuse Dice's `#16A34A`/`#DC2626` pair — that divergence is deliberately Dice-only.** Note also: `outlineClasses()` has no per-verdict color override for `'draw'` today (only win/lose have opt-in props) — this needs a small extension, same shape as the existing two.

## Item 5 — gem-count icon rows in both bars (closes `2026-09-12#2` item 3(c)'s icon-row half)

Re-confirmed matches `minesOppGemList`/`minesGems` exactly (`:3785`/`:3718`) — 17px gems, wrapped, capped `Math.min(count,22)`. **Depends on item 4's state machine, not independent:** the mount conditions (`oppGemRowOp`/`myGemTextOp`, `:3765`/`:3778`) key off `mReveal`/`mGrown`, both derived from the SAME `minesResult` phase item 4 introduces — build item 4's state machine first, then hang item 5's mount/fade off its `'reveal'`/`'converge'` beats, not as a fully parallel effort.

**Asymmetry to preserve (re-confirmed, not new):** the opponent's row genuinely fades in (`oppGemRowOp`, a real toggle). The player's own row's opacity (`playGemStripOp: 1`, `:3784`) is a flat constant that never changes despite carrying a `transition:opacity` — match this asymmetry, don't make both sides fade identically.

**Still flagged, not built here (unchanged from `2026-09-16#6`):** `oppGemText`/`myGemText` (`:462`/`:681`) — a separate, plain-text mechanism positioned outside the bar box entirely, from the same original `2026-09-12#2` item 3(c) finding, that D23 never mentions. Worth a Designer confirm on whether it's still wanted before anyone builds a second absolutely-positioned slot mechanism for it.

---

**Ask:** ready to build as its own ticket, as agreed with PM. Recommend item 4's state machine + ring colours first (the two colour fixes especially are cheap, isolated, and independently valuable even before the convergence animation lands), then item 5 hanging off the same phase state once it exists.

---
### 2026-09-16#6 — Mines bust reveal: all 3 of Designer's items confirmed real, root cause on item 1 empirically verified (not "wrong order," a genuine CSS stacking gotcha — proved it with a live repro, not reasoned out on paper), item 2 traced to one missing branch in the existing tile-kind logic, item 3 is a clean missing-keyframe port. Items 4-5 (the sequence + gem rows) turn out to be the SAME two features already scoped-but-never-built back in `2026-09-12#2` — D23 finally supplies the exact numbers needed to build them            [READY TO TICKET — items 1-3 are one small, contained PR; items 4-5 are a real, separate, larger follow-up finishing 2026-09-12#2's own long-open items 3(a)/3(c)]
From: Advisor   Re: Designer's D23 report (mine-hit visuals: glow layering, board reveal, bounce, sequence, gem rows), verified against `MinesHub.tsx`, `GameHub.tsx`, a live empirical repro of the exact SVG structure, and the prototype's own source (`Full Spec.html:474-514` tile markup, `:3690-3703` state values, `:3376/3710` sequence, `:72-86` `rcMineJump` keyframe, `:445-462`/`:660-681`/`:3765-3785` gem-row markup+values)

All 3 of Designer's numbered items are confirmed real. Item 1 needed more than reading the markup — I built a live, isolated repro of the exact 4-SVG structure to empirically settle WHY it's broken, since the naive "swap the render order" fix wouldn't actually have worked (the current code's DOM order is already halo-then-icon; the real cause is a CSS mechanic that ignores DOM order between positioned and static siblings). Items 4 and 5, once traced, turn out to be the exact same two features flagged as genuinely new, unbuilt GameHub-level work all the way back in `2026-09-12#2` — D23 is effectively the missing spec for that ticket's own items 3(a) and 3(c), now with the precise numbers needed to actually build them.

---

## Item 1 — the glow-on-top bug: empirically confirmed, and the real cause isn't what a quick look suggests

**The current code's own DOM order already IS halo-first-icon-second** — `MineTileContent`'s `bustedOn` branch renders `<MineHalo/>` (wrapped in its own `position:absolute` div) BEFORE `<MineIcon opacity={1}/>`, and the `safe` branch does the same for `GemHalo`/`GemIcon`. So "swap the JSX order" — the obvious first guess — would be a no-op; that's not the bug.

**The actual cause, confirmed by building and screenshotting an isolated repro of the exact structure:** `MineIcon`/`GemIcon` (`:83/53`) have no `position` set at all (`className="block"` only) — but their halo siblings ARE `position:absolute`. Per CSS's own stacking/paint-order rules, an element with `position:absolute` (even at the default `z-index:auto`) always paints ABOVE any static (non-positioned) sibling in the same stacking context, REGARDLESS of DOM order — this is a genuine, if obscure, CSS mechanic, not a simple ordering mistake. I built a standalone repro of the tile's real markup (both structures side by side — current vs. `position:absolute` added to the icon) and screenshotted it: the current structure produces exactly the screenshot's own red smear (the halo's opaque, blurred circle painting over the whole bomb icon, leaving only the spike-rects poking out at the corners); adding `position:absolute` to the icon reproduces the target exactly — a crisp icon with the glow correctly behind it.

**Cross-checked against the prototype's own markup, which explains why this only breaks the MINE case as badly as it does:** the prototype's own bomb icon SVG (`:481`) DOES have `position:absolute` explicitly set — but its gem icon SVG (`:491`) does NOT. Our port correctly copied the gem icon's own lack of positioning (harmless there — a low-opacity, shape-matching halo painted on top of a full-opacity icon just reads as a soft glow blend, confirmed in my own repro) but ALSO left the mine icon unpositioned, which the prototype never does — that's the actual, specific gap. **Fix:** add `position:absolute` (with the same centering the halo wrapper already uses) to `MineIcon` specifically; `GemIcon` can stay as-is, though matching the prototype's own bomb-icon treatment exactly is the safer, more literal port if preferred.

**Layer order, opacities, and filters — all independently confirmed correct already, no other changes needed:** bomb halo (`#FF3E5E`, `blur(3.5px) drop-shadow(...)`, opacity 1 hit / 0 otherwise), bomb icon (`filter:none`, confirmed — our `MineIcon` never applies a filter), gem halo (`#3CF06E`, `blur(3.5px)`, opacity 0.55 picked / 0 otherwise), gem icon (no filter) — every color/opacity/blur value matches the cited source exactly.

## Item 2 — the whole-board reveal: confirmed real, traced to one missing branch

`cellKind()` (`:287-292`) only returns a non-`'covered'` kind for the busted-on tile, tiles YOU uncovered, or tiles in `myMines` (revealed once locked) — **any tile that's neither picked nor an actual mine stays `'covered'` forever, even after a bust.** This matches Designer's own diagnosis exactly ("the current build is only opening the tiles you touched"). Confirmed against the prototype's own `open`/`auto` logic (`:3690-3694`): `open = picked || busted` — ALL 25 tiles satisfy `open` the instant `busted` is true, not just the picked/mine ones. **Fix:** extend `CellKind` with a new state for "revealed-but-was-never-picked-and-isn't-a-mine" (a ghosted gem, `#0A0A12`/`#E2E2EA` background, gem icon at `opacity 0.26`, no halo) — `cellKind()`'s fallback branch should return this new kind once `myLocked` is true, instead of `'covered'`.

## Item 3 — the bounce: confirmed genuinely missing, a clean keyframe port

Grepped for `rcMineJump` across the whole codebase — zero hits, anywhere. Confirmed the exact keyframe values Designer cites against the prototype's own source (`:72-86`) character-for-character — matches precisely. Confirmed `bombAnim`'s own gating (`:3702`, `hit ? 'rcMineJump 4000ms cubic-bezier(0.32,0.72,0.4,1) 1 both' : 'none'`) — applies to BOTH the bomb halo and bomb icon on the hit tile only. **Fix:** add the keyframe to `styles.css` alongside the other ported ones (`rcNavPop`, `rcNavBarPop`, and the `rcPlayShake` just added for Dice in `2026-09-16#4`/`#622` — same file, same pattern), apply to both SVGs on the `bustedOn` tile specifically.

---

## Items 4-5 — not new asks: these are `2026-09-12#2`'s own items 3(a) and 3(c), still open since that ticket, now with the exact numbers needed to finally build them

**Item 4 (bar convergence + result sequence) is `2026-09-12#2` item 3(a), word for word.** That ticket's own text: *"Bars converge to the center after match end — genuinely new state, doesn't exist in `GameHub` today... `GameHub.tsx`'s current `barSlideActive` is `matchForming || searching` only — there's no post-match 'converged' phase at all yet."* Confirmed this is STILL true today — `barSlideActive`'s own definition (`GameHub.tsx:632`) is unchanged: `matchForming || searching`, no post-match state. D23 supplies exactly what that ticket was missing: the precise timing (bust → +1500ms bars converge over 620ms → +820ms counts revealed → +1520ms from result start, the ring lands) and the exact ring colors (`#16A34A`/`#FF3E5E`/`#FF8A1E`). **This needs real, new GameHub-level plumbing** — a second bar-shift trigger keyed off the result phase (not just pre-match search), scoped to Mines only (the original ticket's own note stands: Dice never converges, it just recolors its number in-bar). Recommend building this the same way D21's own reveal state machine just went in for Dice (`2026-09-16#4`/`#622`) — a locally-timed sequence in the Mines game area, signaling the hub once each beat lands, rather than a new fixed-timing prop on `GameHub` itself.

**Item 5 (gem rows in bars) is `2026-09-12#2` item 3(c), also still open — and D23's own citation confirms a real, separate, STILL-uncovered piece worth flagging precisely.** The icon-row Designer describes (17px gems, wrapped, capped at 22) is real and exactly matches `minesOppGemList`/`minesGems` (`Full Spec.html:3785/3718` — both cited, both confirmed `Math.min(count, 22)`). **But there's a SECOND, separate element in the same markup D23 doesn't mention: `oppGemText`/`myGemText`** (`:462/681`) — plain TEXT spans positioned OUTSIDE the bar box entirely (`bottom:calc(100% + 6px)` above the opponent's bar, `top:calc(100% + 6px)` below the player's own), always colored `var(--rc-green)`. This is the exact element `2026-09-12#2` item 3(c) originally flagged ("needs either a new prop or an extension of the existing slot API to place content outside the bar box") — it's a genuinely different, additional piece of UI from the gem-icon row, still fully unbuilt, and D23's own reference screenshot doesn't appear to show it either (may only ever be visible for a brief transition beat). **Recommend building the icon row D23 asks for now, and flagging the separate text-label element as its own still-open question** — worth a quick confirm with Designer on whether it's wanted at all before spending time on a second, absolutely-positioned slot mechanism nobody's explicitly asked for yet in this round.

**One real asymmetry worth knowing before building item 5:** the prototype's OWN two gem rows aren't symmetric. The opponent's row (`oppGemRowOp`, `:3765`) genuinely fades in (`mReveal || (mGemsKept && mr==='closed') ? 1 : 0` — a real state toggle). The player's OWN row (`playGemStripOp`, `:3784`) is a flat, ALWAYS-`1` constant — it never actually fades, despite the `transition:opacity 280ms ease` sitting on that div (inert, since the value it would transition FROM/TO never changes). Match this asymmetry rather than making both sides fade identically.

---

**Ask:** items 1-3 are safe, contained, ready now — one PR, all inside `MinesHub.tsx` plus one keyframe in `styles.css`. Items 4-5 are real, separate, larger work — recommend scoping as their own follow-up (finally closing out `2026-09-12#2`'s two long-open sub-items) rather than folding into the same PR as 1-3, given the new GameHub-level plumbing item 4 specifically needs.

---
### 2026-09-16#5 — Mines board: confirmed real clutter to strip (a whole duplicate status row + explainer paragraph + a Resign button that doesn't exist in the design), every cited colour and layout value already matches exactly, and three real extra gaps found while verifying the transitions D22 asked me to check — a missing timer-collapse, a missing tile-reveal pop animation, and a shared (not Mines-only) "Playing…" text bug            [READY TO TICKET — item 1 is safe deletion with one careful exception (don't touch shared forfeit infra); items 2-3 confirm nothing needs fixing; items 4-6 are small, real, additional fixes]
From: Advisor   Re: Designer's D22 report (Mines board has clutter the design doesn't have, plus a colour check), verified against `MinesHub.tsx`, `GameHub.tsx`, `packages/games/mines/src/mines.ts`, `packages/core/src/matchmaking.ts`, and the prototype's own source (`Full Spec.html:470-529` markup, `:3690-3703` tile computed values, `:3720-3759` card/timer computed values)

Confirmed exactly what D22 describes: `MinesBoard` renders a whole duplicate status row, an explainer paragraph, and a Resign button that have no equivalent anywhere in the prototype's own `isMines` block. Every single colour and layout value D22 asked me to verify already matches exactly — this file's earlier T8 visual rebuild was already pixel-perfect on that front. Digging into the exact transition citations D22 pointed me at (rather than eyeballing them) surfaced three more real, confirmable gaps beyond D22's own explicit list.

---

## Item 1 — the clutter: confirmed real, safe to delete, with one important exception

`MinesBoard`'s own internal "You (username) · N safe" / "Opponent · 🙈 locked" row (`:299-318`), the "Your move"/"Busted"/"Board cleared" status line (`:321-331`), the "Tap a tile..." paragraph (`:372-378`), and the Resign button (`:380-388`) — confirmed zero equivalent anywhere in the prototype's `isMines` markup (`:470-529`). Designer's framing is exactly right: the shared `GameHub` bars already show the real avatar/name (above and below the card), and `OpponentSlot`'s own existing "Playing…" fallback (`Full Spec.html:462-463`, matching text confirmed) already covers the right-side status — `MinesHubScreen` doesn't pass `renderSlotAside`, so this already works for free once the duplicate row inside the card is gone. Gem count staying hidden until the result is also already true (`oppScore` is server-redacted the whole round, confirmed in `cellKind`'s own comment). **Fix:** delete all four pieces of markup; `myScore`/`myLocked`/`oppScore`/`oppLocked`/`myStatus` become unused once the row referencing them is gone.

**Confirmed via the exact grep D22 asks for:** `grep -rn -i resign apps/web/src/screens/MinesHub.tsx packages/games/mines/src` → exactly one hit, the button's own literal text (`:386`). There is no other "resign"-named anything anywhere in Mines' own code — the underlying mechanism is generically named `forfeit` throughout.

**The important exception — don't delete the mechanism behind the button, only Mines' own use of it:** `onForfeit`/`handleForfeit` (`App.tsx:1095`) is fully generic, shared, cross-game infrastructure — confirmed it's a plain "send a `forfeit` WS message for the current match" call with zero game-specific logic, and it's still legitimately wired to RPS's, Chess's, and Blackjack's own real Resign buttons today. Deleting THIS would break three other games' working resign feature — not what D22 is asking for. **What Mines-specific "code path" genuinely exists:** `minesModule.forfeit()` (`packages/games/mines/src/mines.ts:335`) — its own comment confirms Mines routes disconnects through a SEPARATE lock-on-timeout mechanism, never through `forfeit()`, so this handler is reachable ONLY via an explicit client forfeit call — meaning it becomes fully, permanently unreachable in production the moment the button is gone (confirmed no other UI surface anywhere calls `onForfeit` for Mines). **Recommend leaving it in place anyway:** it's a required member of the shared `GameModule` interface every game module implements (`minesModule: GameModule`), it's small and self-contained, and deleting it would mean either breaking that interface contract or reintroducing a stub later for no benefit — dead-but-harmless is the right state here, not a half-finished interface.

**A separate, pre-existing, unrelated dead-code cluster worth knowing about but NOT touched here:** `MinesPlay.tsx` (a different file entirely, App.tsx's old `'play'` screen route) has its OWN separate Resign button and its own `onForfeit` wiring — but that whole screen is already-known dead code (App.tsx's own comment: "Coinflip is a hub game... so the old CoinflipPlayScreen was dead — removed. The 'play' route now only serves the RPS fallback..." — Mines/Chess/Blackjack are now ALSO hub games, routed through their own Hub screens, so this branch is very likely unreachable too). This is the SAME already-flagged "lobby/stake-entry/play/result 4-screen cleanup" backlog item noted much earlier in this migration — a separate, pre-existing housekeeping opportunity, not something D22 is asking about and not touched in this ticket.

---

## Item 2 — colours: confirmed every single cited value already matches exactly, nothing to fix

Checked all 7 rows in D22's table directly against `MinesHub.tsx`'s own already-defined constants — every one matches exactly: `MINES_BOARD_BG` (`#12121F`/`#DEDEE8`), `MINES_TILE_COVERED` (`#232338`/`#CBCBD8`), `MINES_TILE_OPEN` (`#0E0E18`/`#F2F2F7`), `MINES_TILE_AUTO` (`#0A0A12`/`#E2E2EA`), `MINES_TIMER_FILL` (`#8B45F0`, both themes), and the card/timer-track colour via `--rc-surface`/`MINES_BOARD_BG` respectively (both already confirmed exact matches earlier this migration). Nothing needs changing here — this file's earlier rebuild already got every value right.

## Item 3 — structure: confirmed every cited layout value already matches exactly

Card padding (`px-3 pt-3 pb-4` = `12px 12px 16px 12px` exactly), card radius (`rounded-[22px]`), board radius (`rounded-2xl` = 16px), board padding (`p-2.5` = 10px), board grid gap (`gap-2` = 8px), tile radius (`rounded-[9px]`) — all confirmed exact matches to D22's own citations. The card's `opacity:0.28` during matchmaking is also already correct and wired (`barSlideActive`); the separate "result-converge" dim D22 also mentions is a real, already-known, already-ticketed gap from `2026-09-12#2` item 3(a) (the bar-convergence mechanism itself doesn't exist in this codebase yet) — not re-opened here, already tracked.

---

## Item 4 — the timer's collapse-on-result is missing, confirmed by deriving the exact gating condition

D22 asked me to "check the linear 1000ms drain and the collapsing wrapper." The 1000ms linear drain is already exactly correct (`transition: 'width 1000ms linear'`, confirmed verbatim). **The collapsing wrapper is not implemented at all.** Checked the prototype's own gating (`minesClockOp`/`minesClockH`, `Full Spec.html:3758-3759`): both are `1`/`46px` only during `rpsMatch === 'split' || minesPhase === 'run'` — everywhere else (including the resolved/result phase) they're `0`/`0px`, confirming D22's own "collapses on result" claim precisely. Our `<RoundClock>` is rendered unconditionally inside `MinesBoard` (`:370`), which itself stays mounted through BOTH `'in-match'` and `'result'` phase (`MinesPanel`'s own `live` gate) — so the timer today stays visible, un-collapsed, for the entire result-hold window. **Fix:** gate `RoundClock`'s own mount/height on the same phase check `MinesBoard` already has access to (`phase !== 'result'`, or equivalently whatever locked/resolved signal is cleanest), wrapped in the cited `max-height 0→46px / opacity 0→1` collapse, `620ms cubic-bezier(0.3,0.9,0.32,1)` for height and `420ms ease` for opacity.

## Item 5 — beyond D22's own list: the tile-reveal has zero animation today; the prototype's own is a real, deliberate pop

Checking the exact tile transition D22 cited (`transition:background 200ms ease, transform 160ms cubic-bezier(0.34,1.5,0.5,1)`, `:474`) led to two further findings, not in D22's own text:

- **The tile's own `transform` half of that citation is very likely inert in the prototype itself** — `t.scale` (`:3696`, the value driving `transform:scale({{t.scale}})` on the tile div) is `scale: open ? 1 : 1` — a constant, never-changing `1`. Porting this specific sub-clause literally would produce a no-op, matching the prototype's own accidental inertness — not something worth chasing.
- **The REAL reveal animation lives on the gem/bomb icon itself, and it's completely missing from our code.** The gem SVG (`:508`) and bomb SVG (`:488`) both have their own `opacity`/`transform:scale` driven by `gemOp`/`gemScale`/`bombOp`/`bombScale` (0→1 opacity, 0.4→1 scale on reveal), each with `transition:opacity 180ms ease, transform 300ms cubic-bezier(0.34,1.7,0.5,1)` — a deliberate pop-in, confirmed as an established convention used identically for RPS's own rock/paper/scissors icon reveals elsewhere in the same file (`:611-613/634-636`, byte-identical transition string). **Our `MineTileContent` just conditionally renders `<GemIcon/>`/`<MineIcon/>` with no transition at all** — the icon appears instantly the moment `kind` changes, no fade, no scale. **Fix:** give the icon itself (not the tile) the opacity/scale pop — mount it at `opacity:0, scale:0.4` and animate to `opacity:1, scale:1` on reveal (Framer Motion's `initial`/`animate` is the natural fit here, matching this codebase's own established pattern for exactly this shape of "reveal pop" elsewhere).

## Item 6 — small, low-priority: the tile's own background-color transition timing doesn't quite match

The cited `transition:background 200ms ease` (`:474`) is real and functional in the prototype (unlike the inert transform half above) — our code uses Tailwind's generic `transition-colors` utility instead, which defaults to this project's unmodified Tailwind duration (150ms, `cubic-bezier(0.4,0,0.2,1)`), confirmed via `tailwind.config.js` (no custom override). Cheap to fix (an explicit `transitionDuration`/inline `transition: 'background 200ms ease'`) — low priority given how close the two values already are.

## Item 7 — found while verifying D22's own "Playing..." claim: real, but NOT Mines-specific — it's the shared `OpponentSlot` fallback, affecting every game that uses it

D22 states the opponent bar should read "avatar, name, 'Playing...' right-aligned" — checking this precisely against the prototype's own citation (`Full Spec.html:463`: `font-family:Arial, Helvetica, sans-serif; font-size:13px; font-weight:bold; letter-spacing:0.3px; color:var(--rc-muted)`, literal text `Playing...`, no text-transform) against `GameHub.tsx`'s own current implementation (`:1022`, `text-xs font-black uppercase tracking-wide text-foreground/70`) turned up real, if small, mismatches: `text-xs` (12px, cited 13px), `font-black` (900, cited "bold"/700), `uppercase` (the cited text is mixed-case "Playing…", no transform in the source at all), `tracking-wide` (Tailwind's generic value vs. the cited specific `0.3px`). **This is shared `OpponentSlot` chrome, not Mines-specific** — it affects every game using the fallback (confirmed Dice also has no `renderSlotAside`, so it's affected too). The colour half (`text-foreground/70`, wrong token entirely — should be `var(--rc-muted)`) is part of the ALREADY-TRACKED, deferred `GameHub.tsx` light-theme backlog flagged back in `2026-09-15#9` ("17 occurrences of this exact pattern... flagged as its own open follow-up, no ticket number yet") — not re-opened here. **The weight/case/tracking half is separate, small, and wrong in BOTH themes today** — cheap to fix now regardless of when the broader token pass happens: `font-bold` (not `font-black`), no `uppercase`, `text-[13px]` and `tracking-[0.3px]` instead of the generic Tailwind utilities. Recommend fixing this alongside the Mines ticket since it's exactly what verifying D22's own claim surfaced, even though the fix itself lands in shared code.

---

**Ask:** item 1 is safe deletion, with the one exception called out precisely (leave the shared forfeit plumbing and the server-side interface member alone). Items 2-3 confirm nothing needs to change. Items 4-6 are small, real, additional fixes found by actually deriving the cited transitions rather than eyeballing them. Item 7 is a small, cheap fix in shared code, worth including here since checking D22's own claim is what surfaced it.
...
Item 1 is a clean, confirmed fix — the current code's mistake is exactly what Designer describes, one scope correction plus one genuinely missing animation. Item 2 needs more care: tracing where `#FF3E5E` would actually apply surfaced that Designer's own "add the loss fill" instruction directly reverses the answer Owner gave on this exact question earlier today (`2026-09-15#10`/D17: "keep ring-only for loss, no new fill feature"). That may well be an intentional, informed change of mind now that Designer's looked at it again — but it's a real reversal of a same-day decision, not new information landing on an open question, so I'm flagging it explicitly rather than quietly implementing it as if nothing changed.

---

## Item 1 — the bet-warning ring: confirmed real, scoped to the wrong element

`GameHub.tsx:1258`: `needsBet && 'ring-2 ring-destructive'` applied to the OUTER `hub-section-bet` wrapper — the currency row, the bet-track pill, AND the hint text all sit inside it, so the ring wraps the whole rectangle. Confirmed exact match to the screenshot. Prototype's own ring (`:3736`, `betWarnRing`) is a `box-shadow` on the bet-track pill itself (`data-rc-bettrack`, `GameHub.tsx:1299`) only. **Fix:** move the ring to the bet-track element, and use an actual `boxShadow` style (not the Tailwind `ring-*` utility) so it can animate the exact way the prototype does — `0 0 0 2px #FF3E5E` on, `0 0 0 0 rgba(255,62,94,0)` off (same hue at zero alpha/zero spread, so the transition is a clean fade rather than a hard cut), `transition: box-shadow 260ms ease`. Nothing else (currency row, panel, PLAY button) gets a ring — confirmed already true today, no change needed there.

**The hint text needs three changes, not one:**
1. **Copy:** currently "Select a bet amount to play" (`:1365`) — needs to become exactly **"Choose your bet"** (three words, confirmed against the prototype's own citation).
2. **Color:** currently `text-destructive` — needs the new `#FF3E5E` token (see item 2's token discussion below), not the existing `--destructive`/`--rc-danger`.
3. **Mechanism:** currently an instant `sr-only` toggle (`:1363`, `!needsBet && 'sr-only'`) — the prototype slides it open (`max-height` 0→26px + `opacity` 0→1, `overflow:hidden`, two different transition durations for the two properties). Confirmed genuinely different from an instant show/hide, not just a cosmetic nit — needs a real collapsible wrapper.

## Item 1, continued — the shake animation doesn't exist in this codebase at all

Checked both CSS files (`index.css`, `styles.css`) for `rcPlayShake` — zero hits. This app already has 4 other prototype keyframes ported verbatim into `styles.css` (`rcNavPop`, `rcNavBarPop`, plus `coin-spin`) at the same "global keyframes block" location — `rcPlayShake` was never added there. Exact values, confirmed against the prototype (`Full Spec.html:71`): `0%,100%: translateX(0); 12%: -7px; 28%: 6px; 44%: -5px; 60%: 4px; 78%: -2px`, 540ms total. **Fix:** add the keyframe to `styles.css` alongside the other three, apply `animation: 'rcPlayShake 540ms ...'` to the PLAY button on the same press that triggers `guideToBet()`.

## Item 1 — one thing already correct, one thing deliberately NOT worth porting

- **`BET_TRACK_BG`** (`GameHub.tsx:58`, `{ light: '#D7D7E2', dark: '#12121A' }`) already matches the prototype's citation exactly — no change needed.
- **The scroll offset math** (`bottom + 33 - viewportHeight + 152`, Designer's own citation): this exists in the prototype because the prototype computes its scroll target from a manually-tracked formula, and has to special-case "only add the 33 the first time" because a *second* call would otherwise double-count the now-already-open text's height. Our code already uses the browser's native `betRef.current.scrollIntoView({block:'center'})` (`:1189`), which re-measures the ACTUAL current layout every time it's called — it has no stale-offset problem to compensate for in the first place, first press or second. Recommend leaving this as-is rather than porting the prototype's manual arithmetic; it already satisfies the "no extra jump on the second tap" check for a different, arguably more robust reason.
- **Confirmed "one handler, all three games" already holds today** — `guideToBet()`/`handlePlayPress()` live in the shared `GameHub.tsx`, not per-game — nothing to fix there.

---

## Item 2 — the red-token correction, and what tracing "reference it from all four places" actually touches

**`#FF3E5E` doesn't exist as a token anywhere in this codebase today — confirmed via grep.** Two DIFFERENT existing reds must not be confused with it: `--rc-danger` (`#f0556b` dark / `#c42b41` light — theme-varying, used today in 4 unrelated screens: `RpsHub`, `RewardsHub`, `AffiliateHub`, `ProfileHub`) and Tailwind's `--destructive` (`hsl(350 69% 61%)`, theme-invariant-ish pink-red — currently backing BOTH `ring-destructive`/`text-destructive` on the bet-warning UI in item 1 AND the shared `outlineClasses('lose')` ring every hidden-commit game uses). **Neither should be repurposed or overwritten** — `#FF3E5E` needs to be its OWN new token, added once, referenced by name everywhere Designer wants it, exactly as instructed.

**"Funds warning" is cited as context, not a build ask — confirmed nothing like it exists in this app today.** The prototype's own `fundsWarn`/"Insufficient funds" (`:698-699`) is a genuinely separate feature (a low-balance warning on the bet panel) this app has never built — Designer's message cites it only as one of several places the prototype ALREADY uses `#FF3E5E`, to justify the value, not as a fourth thing to implement here. Flagging so nobody reads "funds warning" as part of this ticket's scope by mistake.

**The Dice loss ring/fill: same shared-component problem D17 already identified, now with different colors, plus a real decision reversal to flag.** Dice's own `DICE_LOSE_RED` constant (`DiceHub.tsx:40`) is a one-line change, `#DC2626` → `#FF3E5E` — trivial. The player-bar ring/fill is the harder part, because it's still the SAME shared `OwnSlot` (`GameHub.tsx:1023`) used by RPS/Mines/Coinflip/Blackjack/Chess, exactly as flagged in `2026-09-15#10`. **What's different this time: Designer is now also asking for a loss FILL** ("Loss fill overlay: #FF3E5E, same `rcWinFill` timing as the win") — but Owner's answer to this exact question, same day, was "keep ring-only for loss, no new fill feature" (`2026-09-15#10`, since the prototype's own source has no loss fill at all). **This ticket's instruction directly contradicts that decision.** It may be entirely deliberate — Designer may have decided a loss fill IS wanted now, prototype precedent notwithstanding — but a same-day reversal of an explicit Owner answer is worth a clear, direct heads-up rather than silently implementing it as if D17's question was never actually settled. Recommend confirming with Owner before building the loss-fill overlay specifically (the ring-color and cube-number changes don't have this conflict — ship those regardless).

**One scope assumption worth stating plainly, not guessing past:** Designer's framing ("#FF3E5E is the red for every warning and loss state... everywhere") could mean either "everywhere on Dice" (matching D17's original explicit scope, "one green, one red, everywhere on THIS SCREEN") or "everywhere in the app, every game's own loss ring too." Treating it as Dice-only, consistent with D17's own explicit scoping and the fact this ticket's title is specifically about "the dice result" — but flagging this reading rather than assuming it silently, since the difference is a one-game color tweak vs. a five-game one.

---

**Ask:** item 1 is safe and clean — ring rescope, copy/color/mechanism fix on the hint text, add the missing shake keyframe. Item 2's mechanical parts (new token, Dice's own cube-number constant, the same Dice-scoped `OwnSlot` prop already recommended in D17) are equally safe to ship. **The one thing that needs an explicit yes before building: the loss-fill overlay**, since it reverses today's own Owner decision rather than answering a question that was still open.

---
### 2026-09-15#12 — Related Games section: confirmed a full rebuild (nothing in the current version matches), but almost everything it needs already exists somewhere else in this codebase — the bolt icon, the GRID order data, the safe nav-pop pattern, the tile art — this is mostly wiring, one genuinely new piece (the selection algorithm)            [READY TO TICKET — one shared component + one new pure function, real but bounded scope]
From: Advisor   Re: Designer's D18 report (Related Games section needs a full rebuild), verified against `GameHub.tsx`'s `RelatedRail`/`related` memo, `gameSort.ts`, `categoryIcons.tsx`, `tiles.ts`, and the prototype's own source (`Full Spec.html:912-933`, `:3237-3243` `nudgeRel`, `:3842-3852`, `:3971-3972`)

Confirmed: today's `RelatedRail` (`GameHub.tsx:1470-1511`) shares nothing with the prototype's spec — plain `overflow-x-auto` (no snap), no arrows, no bolt icon, `border-border` cards with a "Related games" title-case heading, cards carrying a separate "Soon" badge overlay. Designer's "nothing carries over" is accurate. The good news: most of what needs to replace it is already sitting in this codebase, built for other screens — this is real work, but it's mostly assembly, not invention.

---

## Heading + arrows — every piece already exists somewhere else in the app

- **The bolt icon:** `categoryIcons.tsx`'s `OriginalsIcon` — `viewBox 0 0 351 374`, confirmed byte-identical path data to the prototype's citation. Already used at 26px for the Games-page title (`HomeHub.tsx:223`, `<TitleIcon className="h-[26px] w-[26px] text-[#8B45F0]" />`) — same component, just `h-[22px] w-[22px]` here.
- **The two arrow buttons' press-pop:** the exact mechanism Designer cites (`onPointerDown="navPress"`, `rcNavPop 420ms`) is already built and already proven SAFE in this codebase — `HomeHub.tsx`'s category-rail tiles use a `popId` state set on `onPointerUp` (not `onPointerDown` — this app's own established variant) plus `onAnimationEnd` to clear it, applied as an inline `animation` style. This is deliberately NOT the `key`-remount approach that caused the real nav-bar regression fixed in `2026-09-13#9` — reuse this exact pattern, not a new one.
- **RELATED GAMES headline styling:** matches this app's already-established "standard section headline" convention (uppercase literal text, Arial bold, `text-[var(--rc-text)]`) used for YOUR REWARDS/VIP TIER BENEFITS elsewhere — no new pattern needed.

## The rail + cards — new markup, but every visual value is a direct citation, and the art already exists

Confirmed against the prototype's exact CSS (`:929-933`): `scroll-snap-type:x mandatory`, `scroll-padding-left:16px`, cards at `flex:0 0 calc((100% - 18px) / 3)`, `aspect-ratio:112/158`, `14px` radius, two `16px`-wide `var(--rc-bg)` edge-mask strips. None of this exists in today's `RelatedRail` (plain scroll, no snap, `aspect-[2/3]`, `rounded-xl`, `w-36` fixed width instead of 3-per-row calc). **The card art itself needs no new assets** — `RelatedArt`'s existing helper already reads `TILE_ART[id]` (the same map every other tile in the app uses, all 12 games covered) — just needs to drop the current border/"Soon"-badge treatment for the plain, badge-free `background-image` fill the prototype actually uses (confirmed: the prototype's own card markup has ZERO overlay/badge HTML — the "RAPIDCLASH ORIGINALS" pill visible in every card is baked into the tile art image itself, not separate markup, confirmed via the harness capture below).

**Arrow scroll behavior (`nudgeRel`, `:3237-3243`):** `step = firstCard.getBoundingClientRect().width + 9`, then `rail.scrollBy({ left: dir * step * 3, behavior: 'smooth' })` — scrolls exactly 3 cards' worth, confirmed matching Designer's citation exactly. Arrow mute state: left muted at `scrollLeft <= 20` (confirmed: prototype's actual condition is `> 20` for the ACTIVE state, so `<= 20` is muted — matches Designer's citation precisely), right muted when `scrollLeft >= scrollWidth - clientWidth - 4`.

## The selection algorithm — the one genuinely new piece, and `GRID_ORDER` already exists for the "remaining games" half

**Designer's rule, precisely:** first 3 = Crash, Blackjack, Dice (fixed order) — except the current game, if it's one of those 3, is replaced in-place by Mines (not appended); then every other game from the Originals list in GRID order, excluding the current game and whatever's already shown. **`gameSort.ts`'s existing `GRID_ORDER`** (`coinflip:0, blackjack:1, chess:2, mines:3, rps:4, crash:5, dice:6, roulette:7, hilo:8, keno:9, baccarat:10, limbo:11`) is confirmed to be exactly the "Originals list in GRID order" Designer's spec refers to — reuse it directly for the tail, don't re-derive it. **What's actually new:** a small, pure function — something like `relatedGamesFor(currentGameId): string[]` — that (1) builds the fixed 3-slot head with the Mines-substitution rule, (2) sorts the remaining 9 ids by `GRID_ORDER`, excluding whatever's already used. Recommend one function, unit-testable directly against Designer's own three worked examples (Mines/Dice/Chess screens) rather than three separate assertions on the rendered UI.

**One thing the algorithm needs that today's `related` memo doesn't carry cleanly:** which games are actually playable vs. coming-soon, to render the fixed Crash slot correctly dimmed (Crash itself isn't playable yet — confirmed via `gameSort.ts`'s own `RANDOM_PLAYABLE_GAME_IDS`, which excludes it). The existing playable/non-playable split (`RelatedRail`'s own `t.playable`/`t.meta` distinction, non-interactive dimmed rendering for coming-soon) is the right existing mechanism to reuse for this — worth confirming the new algorithm's output type carries that flag through cleanly, since it needs to survive the fixed-slot substitution logic too (Crash is always shown, always in the fixed slot, but never clickable until it ships).

**Note, not a question:** `tiles.ts`'s own `COMING_SOON` array still lists `'dice'` — stale, since Dice has been fully playable and actively worked on all day this session. Harmless today (the actual filter in `GameHub.tsx`'s `related` memo checks against the real live roster, not this list, so the stale entry never surfaces), but worth a one-line cleanup whenever someone's next in this file — not blocking this ticket.

---

## Closing ask — harness capture for the Related Games section: built, run, and verified in both themes

Designer's own note: "Capture the mines/dice screen from the prototype for the harness — this section is in it." Added a `mines-related` screen to `tools/design-fidelity/src/screens.ts`. One real wrinkle worth recording: my first attempt scrolled the section into view manually inside `driveProto`, but the harness's own capture pipeline (`prototype.ts`) deliberately resets scroll to `(0,0)` right after every `driveProto` runs (a scroll a driveProto step causes is treated as a side effect to undo, not a state to capture) — the section sits ~1868px down a ~3159px-tall page, well below the single-viewport capture this reset assumes. **The actual fix was `scrollFrames: true`**, the harness's own already-established mechanism for below-the-fold content (Rewards/Account already use it) — not a driveProto-side workaround. Ran the real capture: `mines-related.frame2.png` (both themes) shows the full section exactly as specified — bolt icon, headline, both arrows (left muted, right active), three edge-to-edge cards with baked-in tile art, matching Designer's own reference screenshot closely. (The prototype's own hardcoded order — Crash/Dice/Blackjack — shows in the capture, not the app's new dynamic order, since the prototype itself never implements Designer's per-screen algorithm; expected, not a discrepancy.)

---

**Ask:** one shared component rebuild (`RelatedRail` + its heading/arrows, `GameHub.tsx`) plus one new, independently-testable pure function for the selection order. Real scope — new markup, new interaction (snap-scroll + arrow-driven scrollBy), one new algorithm — but every visual value and interaction pattern is a direct, already-verified citation or an already-proven-safe existing pattern reused, not invented from scratch.
...
Both items are confirmed real, but neither's mechanism is quite what the ticket describes. Item 1's "text covering the belt" is a genuine geometry problem (the fixed-height box runs out of room), not a stacking/overlay bug — deleting the text fixes both halves of item 1 as one change, with the arithmetic to show it. Item 2 needs a real correction: I checked the prototype's own cited lines for the loss overlay Designer describes, and it doesn't exist there — only a win fill is defined in the source. Also built and verified the harness capture for the closing ask, same as D16's.

---

## Item 1 — the result text and the "no numbers" pill complaint are the SAME bug, confirmed by the box's own arithmetic

`DiceHub.tsx:316-318` — `<p data-testid="dice-status">{resolved ? (meWon ? 'You rolled higher!' : ...) : 'Rolling…'}</p>` — confirmed, exact string match, zero occurrences in the prototype (`grep -c "rolled higher"` → 0), exactly as Designer states. **Fix:** delete this `<p>` entirely.

**The "pills render as plain bars with no numbers" complaint is real too, but not because the label is missing from the DOM** — `DiceHistoryBelt` (`:198-244`) already renders `{fmtRoll(h.value)}` (`:231`) centered inside each pill, in the correct color (`#FFFFFF` win, theme-correct dark/light for loss). **The actual mechanism is a geometry overflow**, confirmed by adding up `DiceBoard`'s own declared values: the box is a fixed `h-[266px]` with `paddingTop:47` (inline, overrides the `py-5` class's own top value) + the class's own 20px bottom padding = 199px of available content height. The 5 flex children (two `h-[46px]` tracks, the scale row, the belt, and the status paragraph), separated by 4 `gap-3.5` (14px) gaps, need roughly 209px — **about 10px more than the box has**, and nothing in this flex column has a `shrink-0` guard, so ALL children absorb that deficit together, the belt included. **Recompute the same box WITHOUT the status paragraph:** the remaining 4 children + 3 gaps need only ~179px — 20px of genuine slack, comfortably inside the 199px available. **Deleting the status text doesn't just satisfy Designer's literal ask — it removes the overflow that's very likely clipping the belt's own pills**, which would explain "plain bars with no numbers" as a side effect of the SAME cause, not a second bug needing a separate fix. Recommend a quick live visual check once this ships (a resolved 2-round match) to confirm the pills render at their full 28px with the label visible — I couldn't stage a live 2-player resolved match myself to see the before/after directly, so this is the strongest explanation the numbers support, not a live-confirmed certainty.

## Item 2 — colors: `DICE_WIN_GREEN`/`DICE_LOSE_RED` already exist and already match; the bar's ring/fill goes through a SHARED component used by 5 other games, so this needs new (small) plumbing, not a token swap

**Half of "a single pair of tokens" already exists, confirmed:** `DiceHub.tsx:39-40` already defines `DICE_WIN_GREEN = '#16A34A'` / `DICE_LOSE_RED = '#DC2626'`, already used for both the cube numbers (`:299-300`) AND the history pill's win color (`:207`) — exactly Designer's target values, already shared within this file. Nothing to add there.

**The player bar's ring/fill is a different story — it's the shared `OwnSlot` (`GameHub.tsx:1023`), used by RPS, Mines, Coinflip, Blackjack, and Chess too**, via `outlineClasses()` (`hub-shared/slotReveal.tsx:44-49`): `ring-success` (→ `--rc-success` → `--rc-green`, `#34d399` dark / `#0b8f5a` light) plus a hardcoded glow-shadow literal `rgba(34,197,94,0.5)` (`#22C55E`) for win; `ring-destructive` (→ `--destructive`, `hsl(350 69% 61%)` ≈ a pink-red) plus `rgba(239,68,68,0.45)` (`#EF4444`) for loss. **None of these three color sources agree with each other today, and none of them are the prototype's own `#22C55E`/`#FF3E5E` either** — this is a separate, already-diverged app-specific token pair from an earlier ticket (`#485`/T3a), not a straightforward "restore the prototype value." **Since this component is shared across 5 other games, overriding it to Dice's `#16A34A`/`#DC2626` requires a small NEW prop** (e.g. a `verdictColors` override on `OwnSlot`, threaded from `GameHub`'s own props, defaulting to today's shared classes for every game that doesn't opt in) — genuinely more than "swap two hex values," and scoped carefully so RPS/Mines/Coinflip/Blackjack/Chess are provably unaffected.

**A precise correction to Designer's own citation: the prototype has no loss fill at all.** Checked the exact lines cited (`Full Spec.html:662`, `:3787-3789`): the overlay div is `background:#22C55E` (hardcoded, win-only) with `animation:{{ winFillAnim }}`, and `winFillAnim`/`winTextAnim` are BOTH `mOutcome === 'win' ? 'rcWinFill...' : 'none'` — there is no loss-colored fill anywhere in the source, only a ring (`playerBarRing`, `:3787`: lose→`#FF3E5E`, draw→`#FF8A1E`, win→`#22C55E`, confirmed matching Designer's own citation exactly for the RING). Designer's ticket describes "Loss: ring ... plus the same overlay in #DC2626" — **this doesn't match the prototype's own cited source, which has never had a loss fill.** Our current `OwnSlot` code already matches the prototype's real structure (fill only on win, ring-only for loss/draw) — confirmed via direct read of `useWinReveal`'s gating (`win = barVerdict === 'win'`, `GameHub.tsx:1024`). **Flagging rather than guessing:** is a loss fill a genuinely new feature Designer wants added (beyond the prototype), or was this a description slip and only the ring should change color for loss? Recommend confirming before building a fill mechanism the source doesn't have.

**Also flagging, not assuming:** the draw ring is cited as `#FF8A1E` but Designer's own framing only names win/loss as "the one place we're deliberately overriding it" — draw isn't explicitly called out. Current draw ring is `ring-amber-400` (`#FBBF24`), which doesn't match `#FF8A1E` either. Worth a quick confirm: override the draw ring too for full consistency, or leave it exactly as the shared amber since it wasn't named?

**The opponent bar getting no ring** (`oppBarRing: 'none'`, `Full Spec.html:3786`) is already correct today — confirmed, `OpponentSlot` never receives a `barVerdict` prop at all.

---

## Closing ask — harness capture for the result state: built, run, and verified in both themes

Same approach as D16's `rps-searching` capture. Added a `dice-result` screen definition to `tools/design-fidelity/src/screens.ts`, reusing the existing `signIn`/`openGameTile`/`armStakeAndPlay` helpers, then waiting past the full PLAY→result chain (`680+1700+760+660+460+444+900 = 5604ms`, traced through `startDice`'s nested timeouts + `runDiceRoll`'s animation + `startDiceResult`'s own delay). Ran the actual capture and verified both theme outputs directly: colored cube numbers (green/red), a colored ring on the losing player's bar, and a single history pill already showing its number label correctly in the prototype — confirming the belt's OWN rendering is fine at full size, consistent with item 1's "it's a room problem, not a missing-label problem" finding above.

---

**Ask:** item 1 is a one-line deletion, safe immediately. Item 2's color fix needs a small new prop on the shared `OwnSlot` (not a one-line tweak) plus two open confirms (the loss-fill question, the draw-ring question) before it's fully scoped — recommend shipping item 1 now and item 2 once those two are answered, rather than guessing at either.
