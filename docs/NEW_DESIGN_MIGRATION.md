...
## Status snapshot — 2026-09-12 (RPS round closed out; Mines investigation now open) — supersedes all earlier snapshots in this section
...
**Advisor next:** available, no open thread — 2026-09-12#2 is the current ask. **PM next:** dispatch items 1+2+3(b) as one ticket (cheap, matches #551's code shapes); scope 3(a)+3(c) (bar convergence + gem badges) separately once agreed — real feature work, not a same-night fix.
