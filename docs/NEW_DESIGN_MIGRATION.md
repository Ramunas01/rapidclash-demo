...
## Status snapshot — 2026-09-11 (Owner's live post-deploy pass) — supersedes all earlier snapshots in this section

**Deployed and live.** Owner directed deploying #541/#542 (the two platform-wide gap fixes) rather than waiting for the RPS-specific items, on Advisor's recommendation: they're already merged, they affect all 12 games, and the RPS-only items have no dependency on deploy timing. Owner then tested the live site directly (anonymous browser, ruling out caching) and is actively finding more real gaps this way — this snapshot reflects that live-testing loop, not a paused state.

**Standing project stance, set explicitly by Owner tonight, applies to all of the below:** at this stage, visual/experiential fidelity to the Designer's prototype outweighs correctness. This demo exists to persuade investors the team can execute a compelling vision — the production build, with real correctness constraints, happens later on a different platform. Where matching the prototype means accepting a gap the code was deliberately built to avoid, **accept the gap and document why in a code comment** rather than silently keeping the safer behavior.

**Merged and live:** the 3 gap tickets from Owner's first RPS pass (`ADVISOR_TO_PM.md` 2026-09-11#8) — (A) `GameHub.tsx`'s bet/play panel, 4 missing elements, all 12 games (#542); (B) `GamesCarousel.tsx`'s doubled-`@@` bug + `$`-skin gap, all 12 game hubs + HomeHub + ProfileHub (#541); RPS's search-theater dwell restore (#543) and per-round tie reveal (#544) from the Owner-directed reversal (2026-09-11#9).

**Still open from 2026-09-11#8:** item C (RpsHub emoji → real vector art) — deliberately held by PM to avoid file conflicts with #543/#544, now unblocked since both landed.

**Owner's live testing found 2 more real gaps beyond what's shipped — `ADVISOR_TO_PM.md` 2026-09-11#10, both traced to exact root cause, neither guessed at:**
1. **The search-theater fix (#543) only restored duration, not motion.** The prototype's opponent/player bars actually slide toward the vertical center during search, hold, then slide back (`rpsOppBarY`/`rpsPlayerBarY`, `Full Spec.html:3753-3754`) — confirmed **shared across RPS/Mines/Dice** in the prototype's own state derivation (`rpsMatching`, `:3517`), so this should be scoped as a `GameHub.tsx`-level addition alongside T5's VS-label mechanism, not RPS-only. T5's own PR (#508) never mentioned this motion — an honest miss, not a deliberate cut. One real implementation detail: RPS's shift is a flat fallback (`123px`); Mines/Dice use a value measured live off the actual rendered DOM — don't hardcode RPS's number for all three.
2. **The reveal genuinely happens in-place in the prototype (no separate popup) — and this codebase already has the exact working mechanism for that, RPS just never adopted it.** `suppressResultOverlay` + `holdResultMs` + `ownBarResult` (`GameHub.tsx:248/238/260`) is already used by Coinflip/Chess/Blackjack and matches the prototype's own win-fill/"you won" bar treatment byte-for-byte. `RpsHub.tsx` currently wires neither (`:476-477`, only the default-overlay path). This significantly de-risks the item — it's "adopt an existing, proven pattern" (follow `CoinflipHub.tsx:339-341` as the reference), not new UI to invent. Distinct from and additive to item C above (C = what renders unrevealed; this = where the reveal transition happens) — both needed, not overlapping.

**Advisor next:** available, no open thread — 2026-09-11#10 is the current ask. **PM next:** item C + both new items from 2026-09-11#10 whenever convenient, none urgent per Owner's own framing; races/leaderboards and lobby-collapse-cleanup still unscoped.
