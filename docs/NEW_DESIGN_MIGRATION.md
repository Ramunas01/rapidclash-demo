...
## Status snapshot — 2026-09-12 (RPS + Mines closed out and shipped; Dice dispatched) — supersedes all earlier snapshots in this section

**2026-09-12#3 — Dice vs. prototype, Owner's own analysis pass (first time playing it) — 2 real gaps, 2 already correct as-is. Full detail: `ADVISOR_TO_PM.md` 2026-09-12#3. Dispatched, PM building.**

1. **Audio — real, confirmed gap (zero sound wired for Dice/RPS/Mines today, only Chess has one click sound), but smaller/differently-shaped than "rich audio in all phases."** The prototype ships exactly 3 clips (`play`/`dice-roll`/`dice-win.mp3`, real files already in the repo at `design/prototype/assets/audio/`), at exactly 3 trigger moments — `dice-roll.mp3` looped during the roll (Dice-only), `dice-win.mp3` **win-only** — there is no loss sound in the prototype's own asset set. **Correction from PM's own check of `playMines()`, not just its name:** `play.mp3` firing at the shared `GameHub.tsx` PLAY button isn't "RPS/Mines/Dice get it free as a side effect" — `playMines()` genuinely is the ONE handler behind the prototype's ONE PLAY button across all three games, `this.sfx('play')` fires unconditionally before any per-game branch, so wiring it at the shared button is the actually-correct prototype-faithful scope, not a bonus. Mute toggle already fully wired (`PreferencesHub.tsx` #418) — nothing to do there. `sound.ts`'s `play()` has no loop/stop primitive (built for Chess's one-shot click) — PM judged building one not worth it here and hooked `dice-roll`/`dice-win` into `DiceHub`'s existing dedup'd result effect instead.
...
3. **Reveal color animation on the own pill — real gap, same shape as Mines' shipped fix, simpler.** `mOutcome`'s gate (`gameV = view === 'mines' || isDice`, `:3519`) includes Dice — the win-fill/ring mechanism (`playerBarRing`/`winFillAnim`) Owner described is exactly `ownBarResult`, same as Mines. Fixed the same way: `suppressResultOverlay`+`ownBarResult` wired, `DiceReveal` popup deleted as dead code.
...
**2026-09-12#4 — addendum to #3, 3 more Dice items Owner asked about while the ticket was still in flight, all caught pre-merge and folded into the same PR. Full detail: `ADVISOR_TO_PM.md` 2026-09-12#4.**

5. **Idle gauges wrongly dimmed to 50%, pixel-verified.** `DiceIdle` wraps both roll gauges in `opacity-50` (an invented "dimmed anchor" convention shared with RpsIdle/MinesIdle that doesn't actually exist in the prototype for any of the three). Sampled actual pixels from the design-fidelity harness's own committed reference/capture PNGs: prototype green fill = RGB(34,197,94) = exactly `#22C55E` full-strength; our capture = RGB(30,112,70), roughly half-brightness. Source confirms: the prototype's idle-state opacity for this panel is `1` outside matching/converged (`minesBoardOp`, `:3756`) — no dimmed-idle state exists in the design. Fix: drop `opacity-50`. Same pattern flagged (not fixed) in RpsIdle/MinesIdle — own follow-up ticket, not urgent since both already shipped.
6. **"Place your bet and roll" / "Finding a rival…" — zero prototype equivalent**, same removal pattern as the already-fixed RPS/Mines redundant idle paragraphs.
7. **Panel height doesn't match the prototype's fixed 266px box.** The belt itself is already safe from clipping as history accumulates (`overflow-hidden` + 5-slot cap, matching the prototype's own `.slice(0,6)`) — but the panel's own proportions are a real, separate mismatch: prototype fixes Dice's panel at `height:266px` (`:532`), ours uses `min-h-[200px]` (a floor, not fixed, and lower). Fix: match the prototype's literal box model. RPS has the identical `height:266px` pattern (`:606`) — likely the same gap there, flagged not fixed.

**Advisor next:** available, no open thread — #3+#4 are dispatched together (PM folded #4 into the same in-flight PR pre-merge). **PM next:** merge once clean; hold the deploy until Owner is back. RpsIdle/MinesIdle's own opacity-50 echo (item 5) is a small standalone follow-up whenever convenient, not urgent.
