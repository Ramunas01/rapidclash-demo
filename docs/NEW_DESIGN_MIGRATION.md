...
## Status snapshot — 2026-09-11 (post-deploy, verified live) — supersedes all earlier snapshots in this section

**DEPLOYED and verified.** `gcloud run deploy` succeeded, revision `rapidclash-00100-fjw`, 100% traffic, live at https://rapidclash-847070222251.us-central1.run.app. demo-taker's bot-crowd restarted per the runbook. This is the first deploy since the design migration began.

**Fully DONE, merged, and deployed — everything through this line is live, not just on `main`:** Shared-chrome T1–T3b (#478/#489/#487/#494/#504). rps/mines/dice reskin: T5, T6a, T6b + follow-up #512 (#508/#509/#510/#514). T7 Mines engine rewrite in two Owner-reviewed PRs per ADR-012 (#516/#517 — a real redaction bug caught and fixed by the PM's review before merge). T8 Mines visual rebuild (#521). T9 GameHub `$` skin fix (#522). #497 (#519), #501 (#520), T4 + reconciliation sweep (#523/#524). Mines idle-state polish (#529). Currency-picker dropdown (#530/#531) — full search/sections/mock-balances/toggles feature, deep-scoped per component boundaries in `ADVISOR_TO_PM.md` 2026-09-11#5, implemented and merged. Plus #133 and #77 (small non-migration backlog). Harness bug fix + coverage (#513). Asset triage (#505). Designer handoff backup (#506).

**Post-deploy live-testing turned up a real, separate operational bug — found, fixed, and merged same-night (#532/#534):** Owner tested the live demo and saw "0 LIVE" games. Investigated directly against the running server (Cloud Run logs, `demo-taker`'s bot-crowd logs) rather than guessing — root-caused it precisely: `tools/bot-crowd/src/bot.ts`'s top-up idempotency key used a per-process in-memory counter that reset to 0 on every restart (i.e. every deploy), so the ledger correctly treated the reused key as a duplicate and no-op'd the credit while still returning `200 OK` — bot-crowd's client-side balance tracker credited itself anyway (locally), so bots that had ever needed a top-up before got permanently stuck believing they had money they didn't, unable to post/take again. Explained exactly why Mines (only 1 rester + 1 taker in the base roster) could show zero live activity even while the service was otherwise healthy. **Confirmed unrelated to any of tonight's T5–T9/T7 work.** Fixed (`crypto.randomUUID()` per attempt, PR #534) — PM restarting bot-crowd afterward to clear the already-stuck bots' drifted local state, not just ship the code fix.

**#497 corrected, not just closed.** The coder's fix (PR #519) was real and verified (light fidelity on the affected nav capture went 7.98%→83.25%), but the ORIGINAL diagnosis in this ticket (written by Advisor, earlier tonight) was wrong — it wasn't a HubToolbar light-mode bug at all, it was `AuthModal`'s backdrop z-index sitting above the persistent nav, with a much bigger visual impact in light theme than dark due to luminance math (not a "doubling" glitch). Closed with the correction spelled out on the issue rather than silently — a diff-image-level diagnosis, even one two people cross-checked, can still point at the wrong component until someone measures the actual DOM.

**Chat — fully scoped and split into two ready-to-dispatch tickets, `ADVISOR_TO_PM.md` 2026-09-11#7.** PM decided both open questions from 2026-09-11#6 (both without Owner escalation — infra-shape calls, not fairness/money/contract): **general-only room** for V1, **kill switch = a plain boot-time env var** enforced server-side, matching existing `gateway.ts` precedent exactly. Two tickets follow directly:
- **(a) Server — `ChatTransport` + new WS message types** (`chat.subscribe`/`chat.send`/`chat.message`/`chat.history`). New surface: genuine trust-boundary care needed (sender name/tier resolved server-side from the authenticated connection, never trusted from the client payload — reuses `resolveUsername` and the already-frontend-facing `tierForXp`). Fan-out follows the existing `pushChallengesUpdate` pattern exactly, no new mechanism invented. Flagged a real prototype fact worth implementing precisely, not guessing: only Gold/Emerald/Diamond get colored chat names in the source (`Full Spec.html:4134`) — Wood/Bronze/Silver/Unranked all render plain. **Treat like T7** — one agent, not parallelized with anything else touching `gateway.ts`/`protocol.ts`.
- **(b) Client — the chat sheet UI.** Standalone once (a)'s payload shapes are agreed; can build against a mock transport in parallel. Full exact citations for the sheet/scrim/transform values, the composer's transparent-textarea-over-styled-overlay trick (two different @mention regexes for typing vs. rendering sent messages — don't collapse them), and the client-side char-limit UX that should stop a user before hitting the server's hard 160-char limit, not just show a warning ring and let the send fail.

**Advisor next:** available, no open thread. **PM next:** the two chat decisions above whenever there's roadmap bandwidth for it (it's big — don't start it casually); races/leaderboards and lobby-collapse still unscoped, pick whichever's next in priority.

**Still queued, no scoping started:** races/leaderboards; lobby/stake-entry/play/result → one screen with internal phases. Multi-day, unscoped — not started deliberately, don't assume timeline.
