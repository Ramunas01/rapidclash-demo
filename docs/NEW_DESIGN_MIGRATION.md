...
## Status snapshot — 2026-09-12 (Owner's rapidclash-00102-km6 pass — a real correction to shipped work) — supersedes all earlier snapshots in this section

**Deployed and live: `rapidclash-00102-km6`**, which included #546 (GameHub bar slide-to-center-and-back), #547 (RPS in-place win/lose reveal via `suppressResultOverlay`+`ownBarResult`), and #548 (RPS real hand-icon/redaction art). Owner tested this revision live and found 4 more gaps, relayed via PM — full detail in `ADVISOR_TO_PM.md` 2026-09-12#1.
...
**Merged and live (recap):** 2026-09-11#8's 3 tickets (#541/#542, plus C via #548), 2026-09-11#9's reversal (#543/#544), and 2026-09-11#10's 2 items (#546/#547).

**2026-09-12#1 — 4 new findings, all traced to exact root cause in the prototype's `renderVals()`, not guessed at:**

1. **A real correction to #547, already shipped and already reviewed — highest priority.** `ownBarResult` (the Coinflip-style bar win-fill mechanism, `mOutcome`/`winFillAnim`/`winTextAnim`) is gated in the prototype by `gameV = view === 'mines' || isDice` (`Full Spec.html:3519`) — **RPS is structurally excluded, unconditionally.** This isn't an edge case the earlier review missed by omission; the prototype's own source cannot produce this treatment for RPS. RPS's actual, already-correct win indication (confirmed correct since T6b/#509, untouched by this) is the card frame color (`rpsLeftFrame`/`rpsRightFrame`, `:3810-3811`) combined with the card enlarging (item 2 below). **Fix:** remove `ownBarResult` from `RpsHub.tsx`'s `<GameHub>` call, keep `suppressResultOverlay` (that part's correct), add a code comment at the removal site citing `Full Spec.html:3519` so it isn't re-added later assuming parity with Coinflip.
2. **Card enlargement at reveal — real, precisely quantified, confirmed missing entirely.** `rpsExpanded()` (`:3263-3265`, true during `reveal`/`flip`/`done`) drives hardcoded transitions: card width 92→**124px**, height 130→**176px**, gap 36→**12px**, VS column 56→**38px**, picker area height 180→**0px** (collapses), picker opacity →**0**, icon scale 1→**1.6** (all `:3804-3814`, icon scale `:3252`). Current app hardcodes only the small values as fixed constants (`RpsHub.tsx:29/31/33`) and only dims the picker to `opacity-70`+`disabled` (`:459/469`) — it never collapses or hides. **Fix:** derive the dimensions from `terminal` (`:299`, already computed) the same way the prototype derives them from `rpsExpanded()`, replace the picker's dim-only treatment with a genuine height-collapse + fade-to-0.
3. **Z-index bug (real, confirmed via CSS stacking rules) + missing table-dim, confirmed absent.** `OpponentSlot`/`OwnSlot` (`GameHub.tsx:911/959`) apply `barShiftY` via a bare `transform`, no `z-index` — so during the slide-to-center motion they paint behind the RPS card block (mounted later in DOM at the same auto-z level, arena JSX `:722-733`). The prototype avoids this with explicit `z-index:3` on both bars (`data-rc-oppbar`/`data-rc-playerbar`, `:436`/`:660`), above the VS label's `z-index:2` (`:432`). Separately, the prototype fades the RPS card block to **28% opacity** during the slide (`rpsBoardOp`, `:3795`) — grepped the codebase, no implementation exists, only a stale comment (`RpsHub.tsx:193`). **Fix:** give both bars `z-[3]` whenever `barShiftY != null`; apply `opacity:0.28` to RPS's card wrapper while `barSlideActive` (`GameHub.tsx:584`) is true.
4. **Three UI text strings, three different verdicts (not one blanket call):** "Waiting for an opponent…" (`RpsHub.tsx:239`) — genuine duplicate of the shared PLAY-button countdown label, prototype has exactly one "waiting" text (`:462`) — **remove**. "Picked {choice}…" (`:487`) — no prototype equivalent, but explains real interactive behavior (freely-changeable pick) a static mock never needed to — **keep, flagged**. "Forfeit" button (`:492-493`) — prototype computes a handler (`rpsForfeit`, `:3821`) but never binds it to any visible element (dead code, same category as `DEFAULT_BOMBS`); removing the visible affordance without confirming another give-up path exists would cut real capability — **needs an explicit Owner call**, not a silent removal.

**Process note, this round:** caught myself investigating on a stale local branch (predating #546/#547/#548) before writing anything down — would have falsely reported `ownBarResult`/`suppressResultOverlay` as never wired, which is false on real `main`. Re-verified against `origin/main`@`770914f` before publishing. Same standing lesson as below, still recurring.

**Ticket merged: #549** (this analysis, docs-only). **Owner's calls on item 4, all three now final:** Forfeit — restyle to match the aesthetic (smaller/subtler), **keep the function**, no capability lost; "Waiting for an opponent…" — drop the duplicate; "Picked {choice}…" — keep, it's functional copy. **PM dispatched the actual code fix (items 1-3 + item 4's now-resolved text/Forfeit changes) as one consolidated ticket to a single agent**, deliberately not parallelized — `RpsHub.tsx` has had 5 PRs land against it in one night (#543/#544/#546/#547/#548), one careful pass beats another round of near-conflicting edits. **Awaiting PM's report once that lands and is reviewed/merged — this is the only open thread right now.**
