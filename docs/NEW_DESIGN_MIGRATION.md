...
## Status snapshot — 2026-09-13 (D01-D06 shipped+deployed; D07 dispatched; D08 Recent Games new/current top) — supersedes all earlier snapshots in this section

**2026-09-13#8 — NEW, current top item. Designer sent a spec for Account's Recent Games list — 5 real, cheap gaps confirmed (empty state, RC-coin→currency-icon, opponent avatar removal, VIEW MORE 3D + a deeper transition bug, locale-dependent date format, game display names), plus one reported "bug" that's already correct in current code. Full detail: `ADVISOR_TO_PM.md` 2026-09-13#8.**

- **Empty state, amount styling, opponent-row avatar, VIEW MORE ledge, date format, game names** — all confirmed real via direct code read, all small independent fixes, safe as one PR. One extra catch beyond Designer's own ask: the collapsed-list transition is 320ms in code vs. the prototype's 520ms, and the bottom fade is conditionally mounted rather than opacity-transitioned — **the third occurrence this week of the same "no state to animate from" architecture gap** (Menu overlay, Auth sheet, now this), worth naming as a standing pattern if a fourth shows up.
- **NOT confirmed as a bug, despite the report:** Designer's message says lost-match amounts show near-white and should be muted. Read `MatchRow` directly — the color logic already matches the prototype's own formula byte-for-byte (win→green, else→muted/text), shipped back on 2026-09-07, well before today. Flagged rather than silently "fixed" — recommending a live-page check before writing any color-logic code, since a fix against already-correct source could mask a real live/deploy discrepancy if one exists.

**Advisor next:** available, no open thread — #8 is the current ask, all real items are small and independent. **PM next:** one PR for the 5 confirmed items; check the actual live Account page for a real loss row's color before touching the color logic.

**2026-09-13#7 — dispatched, PM scoping (items 1+2), item 3 taken to Owner directly.** Designer's avatar-picker rebuild (floating popup → inline strip) — confirmed real, assets already in the repo. Owner's call on the default-avatar conflict: keep the per-user disc color everywhere it's actually rendered, but the picker's own reset-to-default tile can show the prototype's literal fixed-purple+glyph look (a control affordance, not a rendered identity) — PM dispatched with that as a firm constraint. Full detail: `ADVISOR_TO_PM.md` 2026-09-13#7.
