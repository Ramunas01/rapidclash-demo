...
**Advisor next:** available, no open thread — 2026-09-12#1 is the current ask, item 3 (the `ownBarResult` correction) first per PM's own framing. **PM next:** dispatch item 3 (correction to #547) first, items 1-2 (z-index/dim, card enlargement) next in either order/combined, item 4's three strings need three separate decisions — one clean removal, one keep-with-flag, one genuine Owner decision (forfeit affordance).
