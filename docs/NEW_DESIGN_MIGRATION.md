...
## Status snapshot — 2026-09-13 (RPS/Mines/Dice reveal bug shipped; two Designer handoffs in flight — Games hero dispatched, Menu page new/current top) — supersedes all earlier snapshots in this section

**2026-09-13#2 — NEW, current top item. Designer sent a direct spec for the Menu page (missing Dark/Light control + an opening-animation bug) — both confirmed real, the animation bug traced to its exact mechanism (more specific than either of Designer's own two guesses). Full detail: `ADVISOR_TO_PM.md` 2026-09-13#2.**

1. **Dark/Light control — genuinely missing, every value confirmed against the prototype's own computed props.** `MenuOverlay.tsx`'s groups end at SUPPORT, nothing follows. All colors (theme-swapped bg/ledge/text per button) match the prototype's `renderVals()` byte-for-byte. The state to wire already exists and needs no changes: `theme.ts`'s `useTheme().setChoice('dark'|'light')` is exactly `pickDark`/`pickLight`, same persisted key Preferences' own 3-way control already uses. The 3 new colors (`#2F2F49`/`#5F27B8`/`#C9C9D6`) are confirmed genuinely new — zero hits anywhere in `index.css` — add as tokens per Designer's own ask, not hardcoded.
2. **Opening animation — real bug, but the actual mechanism is a third thing, more specific than Designer's two guesses (overlay mounted late / route-change paint).** The overlay IS always mounted (SPA, no route paint). Root cause: `useMenuOverlay.ts`'s `anchorRect` starts `null` and is only ever set at click time (`HubToolbar.tsx`'s `handleMenuClick`), never on mount — so the FIRST open's clip-path transition changes both the circle's center (0,0 → real button position) AND its radius in the same CSS transition, since both values change in the same React batch. `clip-path: circle()` interpolates both together, producing exactly Designer's "top-left-downward first, then correct reveal." Recurs per-screen (not just once ever) because each hub screen owns its own `useMenuOverlay()` instance/state — matches Designer's own check instruction to test Games/Rewards/a game screen separately. **Fix:** measure the button's rect on mount (`useLayoutEffect` in `HubToolbar.tsx`) via a new rect-only reporting path, not only at click time, so the origin is already correct before the first open ever fires.

**Advisor next:** available, no open thread — #2 is the current ask, both items cheap, one PR. **PM next:** dispatch as one PR (new control + the mount-time rect fix), then Designer's own check (open Menu from Games/Rewards/a game screen after a fresh load) + the harness run in both themes.
