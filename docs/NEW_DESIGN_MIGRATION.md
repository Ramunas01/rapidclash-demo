...
## Status snapshot — 2026-09-15 (deploy gap found and closed same-day — Pepe image confirmed retired, live; footer light-mode ticket now queued) — supersedes all earlier snapshots in this section

**2026-09-15#3 — RESOLVED, same day. Designer sent "D12," byte-identical to D09/2026-09-14#1 (confirmed via file hash — same screenshots, same text, word-for-word). Investigating why Designer would still be seeing the already-fixed title-row bug surfaced a real deploy gap: `#582` and `#585` were both merged but never deployed — production was still on `rapidclash-00109-9hg`, the revision from the nav double-tap fix, with nothing shipped since.** PM deployed immediately on the finding (`rapidclash-00110-tb4`, confirmed live), restarted the bot-crowd, and independently verified the Pepe asset file is fully deleted with zero remaining references anywhere in the codebase — not just visually replaced. Owner was flagged the moment the gap was found and has since seen it resolved. **Item 4's "above" spacing question (2026-09-14#1) is the only piece from this stretch still genuinely open** — needs an actual answer from Designer, not something a deploy or more code-reading fixes.

**2026-09-15#2 — NEW, current top item, not yet dispatched. Designer sent a spec for the footer's light-mode theming — all 6 numbered items confirmed real, no corrections needed to Designer's own diagnosis anywhere (the cleanest ticket of the batch so far). Full detail: `ADVISOR_TO_PM.md` 2026-09-15#2.**

- **Items 1, 4, 5, 6 (gradient band, heading, link text, disclaimer/copyright):** all the same shape — a hardcoded literal (`#1A1A2E`/`#FFFFFF`/`text-foreground`/`#83838F`) where a theme token belongs. Item 5 traced precisely: `text-foreground` resolves to a legacy shadcn token (`--foreground`) with **no light-mode override anywhere in `index.css`** — the same bug class already fixed on `HubToolbar`/`HubRibbon` this week. Fix locally (swap to `var(--rc-text)`/`var(--rc-muted)` in this one file), not by re-theming the shared `--foreground` token globally, matching established precedent.
- **Item 3 (logo swap):** confirmed, and the dark-mode asset **already exists in the repo, already used correctly in the header** (`HubRibbon.tsx`) — pure copy-the-pattern reuse, zero new assets or logic.
- **Item 2 (footer background):** confirmed already correct, no change needed — Designer's own note said so, verified true.
- **"Also" (Tournaments→Events):** the label fix is cheap and ships with the rest; making it an actually-functional link needs new plumbing across 6 different call sites (no callback for it exists anywhere today) — split into its own follow-up, not bundled in.
- **The closing ask (grep the tree for hardcoded hex) is real, and genuinely bigger than this ticket:** ran it — 41 real hits (after excluding legitimate icon-fill false positives) across **15 files**. Some are real bugs, some are deliberate theme-invariants already reasoned about elsewhere (e.g. Dice's own die-cube face colors) — this needs the same file-by-file human triage as the Footer, not a mechanical sweep. Scoped as its own dedicated follow-up ticket, not silently absorbed into today's fix.

**Advisor next:** available, no open thread — #2 (footer) is the current ask, all 6 items + the cheap Events label fix ready to dispatch as one PR now; "make Events real" and the repo-wide hex sweep are both real, separately-sized follow-ups, neither urgent. **PM next:** dispatch the footer PR; item 4's "above" spacing question (2026-09-14#1) still needs an actual Designer confirm whenever there's a channel back.
