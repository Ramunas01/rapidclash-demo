...
## Status snapshot — 2026-09-15 (HIGH PRIORITY: a live Pepe image found on Rewards, needs Owner's eyes; Games-title-row mostly shipped, one open Designer question) — supersedes all earlier snapshots in this section

**2026-09-15#1 — NEW, current top item. Designer sent 3 fixes for Rewards (avatar sync, RC→currency-icon, claim-button shift) — all 3 confirmed real, but investigating item 1 surfaced something outside the ticket entirely: a hardcoded, live image of Pepe the Frog on the Rewards VIP card. Full detail: `ADVISOR_TO_PM.md` 2026-09-15#1.**

- **HIGH PRIORITY, not part of Designer's ask: the Rewards VIP card avatar is a hardcoded Pepe the Frog image, live in production right now.** Not "stale state" as diagnosed — there was never any per-user logic there at all, just a static image import (`avatar-placeholder.jpg`, bulk-imported months ago from an unrelated design-tool export, `#308`). Confirmed via direct file inspection it's genuinely Pepe, matching Designer's own circled screenshot exactly. This matters specifically because this codebase already has a standing, reasoned Owner decision to **exclude** Pepe from the platform (`assets/avatars/CREDITS.md`, from the avatar-picker work) — this specific image slipped past that review because it came in through an unrelated asset dump, not the avatar-preset pipeline. Fixing item 1 properly retires it automatically (confirmed the sole consumer via grep), but this needs to reach Owner directly today, not wait for the routine ticket to land.
- **Item 1 (avatar not propagating): confirmed, simpler cause than diagnosed.** `App.tsx`'s render call for the Rewards screen never passes `avatarId` at all (unlike Profile's own call, which does) — `RewardsHub.tsx`'s `Props` has no field to receive it either. Fix: add the prop, thread the existing App-level `avatarId` state through, swap the hardcoded image for the shared `<Avatar>` component (same call shape `ProfileHub.tsx` already uses).
- **Item 2 (RC coin → currency icon + $): confirmed, third occurrence of an already-built pattern this week** (Open Games `#572`, Recent Games `#576`, now Rewards) — direct reuse of `CurrencyIcon`+`useCurSel()`, no new plumbing. Flagged that a 4th occurrence would be worth extracting into a shared helper.
- **Item 3 (claim button shift): confirmed real, Designer's own diagnosed mechanism is exactly right this time.** The locked state already uses a shared fixed-24px-height wrapper (`CardStatusRow`); the unlocked state duplicates the layout values in a separate div with no enforced height, letting it drift. Fix: make `CardStatusRow` accept children and route both states through the same component, rather than two copies of the same fixed-size intent.

**Advisor next:** available, no open thread — items 1-3 ready to dispatch as one PR; item 0 (the Pepe image) needs Owner's attention directly, today, separate from ticket flow. **PM next:** dispatch items 1-3; flag item 0 up the chain immediately regardless of when the routine fix ships.

**2026-09-14#1 — items 1-3 + item 4's "below" half dispatched by PM (own re-verification, independently confirmed all citations, PR in review); item 4's "above" half remains genuinely open.** Designer's Games-page title-row spec — 2 of 4 claims confirmed already-correct via live measurement (font-size, icon-text gap), item 3's real cause found precisely by PM (a leftover legacy global `h2` CSS rule at `styles.css:57`, not Tailwind's fault), item 4's "below" gap fixed by moving margin from the row onto the grid/empty-states. **Item 4's "above" gap remains an open question for Designer** — PM independently re-ran the same citation check and reached the identical conclusion: the prototype's own source produces exactly 26px here, with no combination of nearby values summing to Designer's stated 35px target. This isn't resolvable by re-reading the code again; it needs an actual answer from Designer on which element should carry the extra 9px, or confirmation that 35 was itself a screenshot misread (same as items 1+2 turned out to be). Full detail: `ADVISOR_TO_PM.md` 2026-09-14#1.
