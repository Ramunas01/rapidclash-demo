...
## Status snapshot — 2026-09-13 (five Designer handoffs on shared chrome/screens — Rewards logged-out gate new/current top) — supersedes all earlier snapshots in this section

**2026-09-13#5 — NEW, current top item. Designer sent a spec for Rewards' logged-out state — the nav-gate bug is real and small, but `RewardsHubScreen` has zero guest-awareness today, making the real fix bigger than "remove a gate." Full detail: `ADVISOR_TO_PM.md` 2026-09-13#5.**

- **The gate bug is confirmed exactly as described** — `onRewardsTap` (`App.tsx:1135-1138`) gates on `loggedIn` where the prototype's own `goRewards` has no check at all (only `goAccount` should gate). One shared root cause fixes both the nav tap and the Menu row Designer named. Small fix, but **not safe to ship alone**: `RewardsHubScreen`'s `Props` require a `token: string`, and it fires authenticated fetches unconditionally on mount — no `loggedIn` concept anywhere in the file. Removing the gate without guarding the fetch first sends a guest into a component built assuming an account exists.
- **3 of Designer's 4 "switches" already produce correct output for a guest today — by coincidence, not design, worth knowing precisely why:** the tier ring and the Rakeback lock condition both key off `tier === 'Unranked'`, which is also what a guest's (fetch-less) default state resolves to — correct outcome, but recommend making the Rakeback condition explicit (`!loggedIn || tier === 'Unranked'`) rather than relying on the coincidence. Volume Bonus's "no loggedIn branch, ever" is confirmed already exactly matching Designer's own note. **The one real content gap: the VIP card has no blur logic anywhere** — needs the wrapper + `filter`/`user-select`/`transition` Designer specified, genuinely new code.
- **A real content question resolved by checking the prototype's source, not guessed:** Designer's "real content" wording could read as "show the guest's real data" — but a guest has none. Checked the prototype directly: `@Bobbylee`/`17,800`/`64%` are **literal hardcoded strings, not template bindings** (confirmed via grep, byte-identical at two separate locations) — the mock's own "real content" is a fixed demo persona, unrelated to login state; only the blur is actually computed. Flagging the equivalent decision for our app (what fixed/sample content renders under the guest blur) as one for PM/Owner, not picked silently.
- **Claim buttons confirmed missing the 3D ledge in every state, both cards** (Volume Bonus shares the same locked-button component as Rakeback, so one fix covers both). Active button's ledge color is already-used purple (`#5F27B8`, same as the Menu/auth-sheet ledges); locked button's ledge colors (`#12121F`/`#BEBEC9`) confirmed genuinely new via grep — not a duplicate of the already-correct `--rc-sunken` background color they sit next to.

**Advisor next:** available, no open thread — #5 is the current ask. **PM next:** one PR covering the gate fix + guest-safe `RewardsHubScreen` + the VIP blur + the explicit lock condition + claim-button ledges/tokens; the guest-blur content question needs a decision before the blur wrapper ships.

**2026-09-13#4 — dispatched, PM scoping.** Designer's Login/Signup sheet rebuild — confirmed real rebuild work (current `AuthModal.tsx` is a centered modal, not a sheet), plus a correction to Designer's own assumption that the affiliate `CreateCampaignSheet` already correctly implements the shared sheet mechanics (it doesn't — no drag support, 3 geometry mismatches). Also caught: `AuthModal` is conditionally-mounted (same "nothing to animate from on first open" gap as the Menu overlay), and the Account-icon hover bug traced to a likely sticky-mobile-hover CSS cause. Full detail: `ADVISOR_TO_PM.md` 2026-09-13#4.

**Advisor next:** available, no open thread — #4 is the current ask, real scope (shared sheet component + 2 consumers + the hover fix). **PM next:** scope as build-shared-sheet → migrate campaign sheet (fixing its gaps) → rebuild auth sheet → Account-icon fix, in whatever PR breakdown makes sense; then Designer's own 6-path check + harness run against screenshot 06.

**2026-09-13#3 — dispatched, queued behind #2's in-flight PR (same file, avoiding a merge conflict).** Designer sent a spec for the bottom nav's press feel (`rcNavBarPop`) — confirmed entirely missing (zero hits anywhere for `rcNavBarPop`/`navPress`/`data-nav`), all citations checked out exactly. Also did the grep survey Designer suggested for everywhere else the same mechanism is used: category tiles (already dispatched as 2026-09-13#1), the shared PLAY button (not yet ticketed, high leverage — one fix covers all 12 games), a "Related games" carousel (not yet ticketed, not confirmed to exist yet). Full detail: `ADVISOR_TO_PM.md` 2026-09-13#3.

**2026-09-13#2 — ticket merged in #562, implementation still building (PM correction, my earlier "shipped in #562" was wrong — #562 was the ticket doc only).** Designer's Menu-page spec (missing Dark/Light control + an opening-animation bug) — both confirmed real; the animation bug traced past Designer's own two guesses to its exact mechanism (`useMenuOverlay.ts`'s `anchorRect` starting `null`, only set at click time, so the first-ever open per hub screen animates the clip-path's center and radius together instead of just the radius). Fix is the same file (`HubToolbar.tsx`) 2026-09-13#3's nav-bar fix also needs — PM is queuing that one behind this to avoid a merge conflict. Full detail: `ADVISOR_TO_PM.md` 2026-09-13#2.

**2026-09-13#1 — dispatched, PM building.** Designer's Games-hero spec (category tiles, rail fades, section title) — all 3 confirmed real against current code, every prototype citation spot-checked accurate. Dispatched as one `HomeHub.tsx` PR; harness run requested before opening it. Awaiting diff + fidelity numbers. Full detail: `ADVISOR_TO_PM.md` 2026-09-13#1.
