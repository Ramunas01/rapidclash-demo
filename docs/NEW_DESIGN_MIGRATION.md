...
## Status snapshot — 2026-09-15 (D01-D13 shipped/deployed/closed; D14 shipped+deployed; D15 postponed; D16 shipped+deployed; D17 item 1 shipped+deployed, item 2 needed zero code; D18 — Related Games full rebuild — investigated, ready to ticket: real scope, but almost every piece already exists elsewhere in the app; harness capture built and verified) — supersedes all earlier snapshots in this section

**2026-09-15#12 — NEW, top item, ready to ticket. Designer's D18: the Related Games section needs a full rebuild — nothing in today's version matches the prototype. Full detail: `ADVISOR_TO_PM.md` 2026-09-15#12.**

- **Confirmed a genuine full rebuild** — today's `RelatedRail` (`GameHub.tsx`) shares nothing with the spec: plain scroll (no snap), no arrows, no bolt icon, wrong card shape/radius, a "Soon" badge overlay the prototype doesn't have. Designer's "nothing carries over" checks out.
- **Almost everything the rebuild needs already exists somewhere else in the app** — the bolt icon (`categoryIcons.tsx`'s `OriginalsIcon`, already used at 26px for the Games-page title, just needs 22px here), the arrow press-pop (the exact safe `popId`/`onPointerUp`/`onAnimationEnd` pattern `HomeHub.tsx`'s category rail already uses — deliberately NOT the `key`-remount approach that caused the real nav-bar regression in `2026-09-13#9`), the tile art (`TILE_ART`, already covers all 12 games, already used by this same component today), and the "remaining games in GRID order" data (`gameSort.ts`'s existing `GRID_ORDER` map, confirmed to be exactly what Designer's spec means by "Originals list in GRID order").
- **The one genuinely new piece: the selection algorithm.** A small pure function is needed — fixed 3-slot head (Crash/Blackjack/Dice, with an in-place Mines substitution when the current game is one of those 3) plus the remaining 9 games sorted by the existing `GRID_ORDER`, excluding what's already shown. Recommended as one function, independently unit-testable against Designer's own three worked examples, not three separate UI assertions.
- **One implementation note, not a question:** the algorithm's output needs to carry the existing playable/coming-soon distinction through the fixed-slot substitution too (Crash itself isn't playable yet, but always occupies a fixed, dimmed slot) — the existing dimmed/non-interactive treatment already handles this, just needs to survive the new selection logic.
- **One stale-data note, not blocking:** `tiles.ts`'s own `COMING_SOON` list still includes `'dice'`, which has been fully playable and actively worked on all day — harmless today (the real roster check overrides it), worth a one-line cleanup sometime, not part of this ticket.
- **Closing ask (harness capture) — done, not just requested.** Added a `mines-related` screen to the design-fidelity tool. Real technical wrinkle worth recording: the section sits well below the fold, and the harness's own capture pipeline deliberately resets scroll after every `driveProto` run — a manual scroll-into-view inside `driveProto` gets silently undone. The fix was `scrollFrames: true` (the harness's own established below-the-fold mechanism, already used by Rewards/Account), not a workaround. Verified both theme outputs directly — matches the target exactly.

**Advisor next:** available, no open thread. **PM next:** dispatch — real scope (new component markup + one new algorithm), but every piece is a direct citation or an already-proven-safe reused pattern.
