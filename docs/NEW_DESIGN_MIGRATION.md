...
## Status snapshot — 2026-09-15 (D01-D13, the full Designer collection, all processed — Open Games fix shipped in #591; Recent Games fade now queued; footer shipped in #587; deploy gap closed same-day) — supersedes all earlier snapshots in this section

**2026-09-15#5 — NEW, current top item, not yet dispatched. The last package in the collection (D13): Account page's Recent Games fade goes to grey instead of white in light mode. Full detail: `ADVISOR_TO_PM.md` 2026-09-15#5.**
...
**Fix is one line plus one new object key** (`RC.bg` doesn't exist yet in `ProfileHub.tsx`'s token object) — no scoping questions. **This closes out D01-D13 — no Designer packages left unprocessed.**

**Advisor next:** available, no open thread. **PM next:** dispatch the Recent Games fade fix.
...
**This is now the second cosmetic light-mode fix (after `#587`) sitting merged but not yet deployed** — flagged to Owner, waiting on their timing call. Nothing else pending on this ticket.
...
- **"Also" (Tournaments→Events):** the label fix shipped with the rest; making it an actually-functional link needs new plumbing across 6 different call sites (no callback for it exists anywhere today) — split out and held as its own follow-up, not bundled in. This one's cosmetic, so PM held the deploy pending an Owner timing check rather than auto-deploying (unlike the nav-bug/Pepe fixes, which were shipped live immediately).
...
**Advisor next:** available, no open thread. **PM next:** deploy `#587` once Owner confirms timing (cosmetic-only, no urgency); item 4's "above" spacing question (2026-09-14#1) still needs an actual Designer confirm whenever there's a channel back.
