...
## Status snapshot — 2026-09-13 (four Designer handoffs on shared chrome — Menu/Games-hero/bottom-nav in flight, Login sheet rebuild new/current top) — supersedes all earlier snapshots in this section

**2026-09-13#4 — NEW, current top item. Designer sent a spec to rebuild Login/Signup as the prototype's bottom sheet — a genuine component rebuild, not a restyle, confirmed necessary. Also a real correction to Designer's own "shared component" framing. Full detail: `ADVISOR_TO_PM.md` 2026-09-13#4.**

- **Current `AuthModal.tsx` is a centered modal, architecturally different from a sheet** — confirmed via screenshots + code, not a CSS-only fix. All 6 items Designer listed for removal (heading, close X, field icons, guest link, disclaimer) confirmed present exactly as described. Structure/geometry/behavior (scrim, 70%-height sheet with asymmetric 34/52px corners, drag-to-dismiss past half-height, mode toggle sharing the Menu's own Dark/Light color values, submit toast) all checked against the prototype's citations — everything matches.
- **Correction to Designer's own assumption:** the affiliate `CreateCampaignSheet`, cited as "the exact same scrim, sheet geometry, drag logic," is **not** already a faithful port on inspection — it has zero drag support (tap-to-close only), and 3 geometry mismatches (content-sized height vs. fixed 70%, top-only rounded corners vs. the asymmetric 34/52px, wrong shadow values). The "build once, share it" instinct is still right — but means building fresh to the verified spec and fixing the campaign sheet's real gaps too, not copying its current code as the reference.
- **`AuthModal` is also conditionally-mounted** (`{authOpen && <AuthModal/>}`) — the same "nothing to animate from on first open" category of gap 2026-09-13#2 found and fixed for the Menu overlay. Second occurrence in a week — flagged as worth a standing convention if a third turns up.
- **Account icon "turns white on tap" — 2 of Designer's 3 candidate causes ruled out directly** (confirmed neither React active-state nor a route change touches 'account' when opening the sheet), narrowing to the third: `ToolbarItem`'s own `hover:text-[var(--rc-text)]` class, a classic "sticky mobile hover" bug, consistent with 2026-09-13#3's own finding that nav-item colors should have no transition/pressed-state treatment at all.
