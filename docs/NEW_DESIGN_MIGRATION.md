...
## Status snapshot — 2026-09-13 (D01-D06 shipped+deployed; D07 avatar picker new/current top, flags a real default-avatar conflict) — supersedes all earlier snapshots in this section

**2026-09-13#7 — NEW, current top item. Designer sent a spec to rebuild the avatar picker as an inline account-card strip (currently a floating popup) — confirmed real rebuild, assets already in the repo, but a genuine conflict on what "default avatar" means needs an Owner call before that specific piece ships. Full detail: `ADVISOR_TO_PM.md` 2026-09-13#7.**

- **Assets:** the ten `rc-01`–`rc-10` PNGs Designer says to source from a zip **already exist in the repo**, committed as part of the prototype's own asset export back at the start of this migration (confirmed 480×480 PNG via direct file check) — no sourcing needed, just moving/wiring them.
- **The rebuild itself:** confirmed real (floating modal → inline-expanding strip, same architecture class as the Auth-sheet/campaign-sheet rebuilds this week), every structural/behavioral detail checked against the prototype matches exactly (grid-row expand/collapse, SELECT-replaces-XP, shared tab-rail styling/tokens, select-and-center, shrink-not-sink press). One real count mismatch: current picker has 7 options (6 named presets + default), prototype has 11 (10 numbered presets + default).
- **STOP — real conflict on the default-avatar state, needs an explicit call.** The prototype's default avatar is a fixed purple circle + white glyph, always identical for everyone. This app's CURRENT default is a different, already-shipped feature: a per-user, deterministically-hashed disc color (`Avatar.tsx`'s `discColor`/`glyphColor`) — every account without a preset gets its own distinct, stable color today, not one shared purple circle. Implementing Designer's spec literally would remove that per-user identity feature for what's likely most/all real accounts. Three options laid out for Owner, recommending the lower-risk one (keep the per-user disc, treat only the ten new presets as new) as the default unless told otherwise, so the asset/rebuild work isn't blocked on the decision. Separately, confirmed genuinely good news either way: swapping off the 6 old named presets retires a real, previously-flagged licensing risk on 2 of them, and the server already gracefully resets any orphaned old `avatarId` to default with zero migration needed.

**Advisor next:** available, no open thread — #7 is the current ask, items 1+2 ready to dispatch now, item 3 needs an Owner call but doesn't block the rest. **PM next:** one PR for the asset move + inline rebuild (defaulting to preserving the per-user disc unless Owner says otherwise); hold the "always fixed purple" question for an explicit decision.

**All six Designer packages from earlier today (D01-D06) are merged to `main`, deployed (`rapidclash-00107-lw7`), and demo-taker's bot-crowd restarted clean — live for Designer's review, per Owner's own request.** PM spot-checked `ownerTier` directly against the real `/open-challenges` feed post-deploy (a bot with real balance history reads "Wood", a fresh one reads "Unranked") — confirms the new server-side tier resolution works end-to-end in production, not just in tests. Ticket-by-ticket recap, newest first:

- **2026-09-13#6, shipped in #572.** Open Games tab rail + stake display + host tier icon. Tab rail got its shared track/ledge/press-sink/scroll-to-center, and the edge-fade technique corrected to a `mask-image` (it had been copying the category rail's overlay-div approach instead of its own). Stake display now always shows `$`, with the per-row/per-wallet currency icon reusing the existing `CurrencyIcon` component and a new `theme.ts`-shaped global singleton for the shared wallet-currency state. Host tier icon: Owner's call (taken directly to them given the ADR-010 conflict) was to follow Designer exactly — tier icon on every host, bots included — while keeping the 🤖 marker fully intact in the underlying `ownerName` data (informed-consent requirement preserved) and stripping it only at render. New server-side `ownerTier` field added via the same `lookupUsername`-injection pattern chat's `resolveTier` already used; verified wired across both the WS push and the two public/guest poll routes, plus the isolated guest world correctly defaulting every Demo Opponent to `'Unranked'`.
...
**Advisor next:** available, no open thread — waiting on Designer's next round of feedback against the live deploy. **PM next:** none pending; deploy + bot-crowd restart both confirmed done.

**Still queued, no scoping started:** RpsIdle/MinesIdle's own `opacity-50` idle-dim echo (flagged 2026-09-12#4, both already shipped, not urgent); the shared PLAY button's own missing `rcNavBarPop` (flagged 2026-09-13#3's survey, high leverage — one fix covers all 12 games — not yet ticketed); a possible "Related games" carousel's press feel (flagged same survey, not confirmed to exist in the app yet); races/leaderboards; lobby/stake-entry/play/result 4-screen cleanup. None urgent.
