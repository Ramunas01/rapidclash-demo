...
## Status snapshot — 2026-09-14 (nav double-tap regression shipped; new Games-page title-row handoff current top) — supersedes all earlier snapshots in this section

**2026-09-14#1 — NEW, current top item. Designer sent a spec for the Games-page section title row (icon+text placement) — verified against the LIVE deployed page directly (not just source), since 2 of Designer's 4 claims turned out to already be correct on measurement. Full detail: `ADVISOR_TO_PM.md` 2026-09-14#1.**

- **Items 1+2 (font-size, icon-text gap): NOT bugs.** Confirmed via `getComputedStyle()` on the live production page: font-size is exactly 21px, gap is exactly 12px — both already match spec byte-for-byte. Designer's screenshot-based measurements (inferring values from cap-height/visual estimate) don't match what's actually rendering; recommend telling Designer these are already correct rather than re-touching them.
- **Item 3 (vertical misalignment): real, confirmed by direct measurement (~4px), but a different cause than diagnosed.** The icon is already `display:block` (confirmed) — Designer's named fix is already in place and would be a no-op. Real root cause: the `<h2>` has an unreset `margin-bottom: 8px` (confirmed via computed style) that nobody set explicitly — since `align-items:center` centers a flex item's full margin box, this asymmetric margin shifts the visible text up relative to the icon by almost exactly the observed offset. Fix (one line): add `margin: 0` to the title's existing inline style — which is exactly what Designer's own spec already calls for, just via a different mechanism.
- **Item 4 (spacing above/below): real gaps, but measured numbers differ from the screenshot estimate, and the "above" fix has a genuine citation mismatch.** Measured live: 26px above (target 35, real 9px shortfall), 12px below (target 16, real 4px shortfall) — not Designer's estimated 29/22. The "below" fix is clean and confirmable: move the row's own extraneous `mb-3` off the row and onto the grid, matching the prototype's own margin placement exactly. The "above" fix is NOT confirmable from the prototype's own source as cited — Designer's "9px bottom padding" doesn't match anything on the actual adjacent element (the search/sort/random row has zero bottom padding in the prototype); the only 9px-bottom-padding value anywhere nearby belongs to the category rail, a different, non-adjacent element. Flagged as a question rather than guessed.

**Advisor next:** available, no open thread — #1 is the current ask, items 1-3 (2 no-ops + 1 confirmed one-line fix) and item 4's "below" half are ready to ship now; item 4's "above" half needs a quick Designer/PM confirm on which element carries the extra 9px before any code. **PM next:** dispatch the confirmed items; get the above-spacing question answered before touching that specific piece.
