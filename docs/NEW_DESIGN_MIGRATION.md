...
## Status snapshot — 2026-09-17 (D01-D23 all shipped, merged, and deployed, fully closed; D24 — Mines icon/halo drift, own-clock result sequencing, live own-gem-bar — investigated, ready to ticket; item 2 (result sequencing) surfaces a real, consequential gap in how `2026-09-16#7` shipped last night — the whole result sequence was gated on full match settlement instead of just the player's own round ending, which also means `holdResultMs` needs to become dynamic, not a fixed constant) — supersedes all earlier snapshots in this section

**2026-09-17#1 — NEW, top item, ready to ticket. Designer's D24: gem/mine icons drift off-centre from their halos, the post-bust result sequence waits for the opponent instead of running on the player's own clock, and the own-side gem count should update live as tiles are tapped instead of only appearing at the result. Full detail: `ADVISOR_TO_PM.md` 2026-09-17#1.**
...
**Advisor next:** available, no open thread. **PM next:** dispatch — items 1 and 3 are small and independently shippable; item 2 is real state-machine work plus the `holdResultMs` fix, recommended as its own careful PR.
