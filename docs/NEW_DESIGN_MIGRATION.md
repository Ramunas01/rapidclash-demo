...
## Status snapshot — 2026-09-13 (three Designer handoffs on shared chrome — Menu shipped, Games hero + bottom nav in flight) — supersedes all earlier snapshots in this section

**2026-09-13#3 — NEW, current top item. Designer sent a spec for the bottom nav's press feel (`rcNavBarPop`) — confirmed entirely missing, plus a full grep survey of everywhere else the same `navPress`/`data-nav` mechanism is used in the prototype (Designer's own suggested next step, done proactively). Full detail: `ADVISOR_TO_PM.md` 2026-09-13#3.**

- **The nav bar's pop is confirmed completely absent** — zero hits anywhere in `apps/web` for `rcNavBarPop`/`navPress`/`data-nav`. `HubToolbar.tsx` uses plain `onClick`, no press mechanism at all. Every one of Designer's citations checked out exactly (keyframes, container, trigger, retrigger logic). One small additional confirmed mismatch: the active-color swap has a Tailwind `transition-colors` fade that shouldn't be there — the prototype's switch is instant.
- **Survey of the shared `navPress` mechanism's other 3 locations, done now rather than left for later:** the 5 category tiles (`cat0`-`cat4`) — already caught independently and dispatched as 2026-09-13#1, confirms Designer's own suspicion the gap recurs; the shared PLAY button (`data-nav="play"`, `GameHub.tsx`) — **not yet ticketed**, high leverage since one fix covers all 12 games, plus a prototype-authoring quirk worth knowing (duplicate `animation:` property in the mock's own markup — its shake would always win over its pop, not the intended behavior to copy verbatim); a "RELATED GAMES" carousel's prev/next arrows — not yet ticketed, not even confirmed to exist in the current app yet, lowest priority.

**Advisor next:** available, no open thread — #3 is the current ask, nav-bar fix is small and self-contained. **PM next:** dispatch the nav-bar fix; the Play-button pickup and a possible shared primitive are flagged as options for later scoping, not required now.

**2026-09-13#2 — shipped in #562.** Designer's Menu-page spec (missing Dark/Light control + an opening-animation bug) — both confirmed real; the animation bug traced past Designer's own two guesses to its exact mechanism (`useMenuOverlay.ts`'s `anchorRect` starting `null`, only set at click time, so the first-ever open per hub screen animates the clip-path's center and radius together instead of just the radius). Full detail: `ADVISOR_TO_PM.md` 2026-09-13#2.
