...
## Status snapshot — 2026-09-13 (six Designer handoffs — Menu + Games-hero shipped; Open Games new/current top, flags a real ADR-010 conflict) — supersedes all earlier snapshots in this section

**2026-09-13#6 — NEW, current top item. Designer sent a spec for the Open Games tab rail, stake display, and host row — items 1+2 confirmed cheap (the currency-icon work Designer asked to "copy verbatim" already exists elsewhere in this codebase), item 3 is a real conflict with ADR-010's informed-consent requirement, needs an explicit Owner call before any code. Full detail: `ADVISOR_TO_PM.md` 2026-09-13#6.**

- **Tab rail:** free-floating pills (no track/ledge/press/scroll-to-center) confirmed exactly as described. One technique mismatch caught: the edge fades here should be a `mask-image` on the scroller (confirmed via the prototype's own `railMask()`), not the overlay-div technique the category rail uses — whoever built this rail copied the wrong rail's technique. Also one small separate bug: the 4th tab is labeled "RANK" in code, "LEADERBOARDS" in the prototype.
- **Stake display:** confirmed every row should always show `$`, never the RC coin — a real correction to 2026-09-11#8 item B.2's earlier decision, from more precise information, not a mistake. **The currency-icon sprite Designer says to "copy verbatim" already exists** — `currencyData.ts`'s `OPEN_CURS` and `CurrencyPicker.tsx`'s `CurrencyIcon` component were already built for the wallet chip; this is reuse, not new SVG work. The one real new piece: the selected wallet currency (`curSel`) is local component state today, not shared — needs promoting to a `theme.ts`-shaped global singleton so every row switches together, same size of work as that precedent.
- **Host row tier icon — STOP, real conflict, not a straightforward swap.** `GamesCarousel.tsx` already deliberately keeps a 🤖 disclosure emoji on bot-hosted rows, citing ADR-010's informed-consent requirement by name in its own comment. Replacing it with a tier icon (as asked) would remove that disclosure for bot rows. Separately, the data doesn't exist yet either — no tier field travels over the wire for this feed, already documented as an explicitly-deferred server change. Three options laid out for Owner (follow exactly and accept the gap, a human/bot hybrid needing Designer's sign-off, or flag back to Designer that ADR-010 may not have been accounted for) — not picking one.

**Advisor next:** available, no open thread — #6 is the current ask, items 1+2 ready to dispatch, item 3 needs Owner input first. **PM next:** one PR for items 1+2; hold item 3 for an Owner decision before scoping any ticket for it.

**2026-09-13#5 — dispatched, PM scoping.** Designer sent a spec for Rewards' logged-out state — the nav-gate bug is real and small, but `RewardsHubScreen` has zero guest-awareness today, making the real fix bigger than "remove a gate": needs a guest-safe prop + guarded fetch, a genuinely new VIP-card blur (3 of Designer's 4 "switches" already work by coincidence, not design), and claim-button ledges/new tokens. A real content question (what renders under the guest blur) resolved by finding the prototype's own reference content is hardcoded, not live. Full detail: `ADVISOR_TO_PM.md` 2026-09-13#5.
...
**2026-09-13#3 — dispatched, still queued behind #2's PR (#565 shipped #2 only, not #3 — checked #565's actual diff, #3 hasn't landed yet).** Designer sent a spec for the bottom nav's press feel (`rcNavBarPop`) — confirmed entirely missing, all citations checked out exactly. Also did the grep survey Designer suggested for everywhere else the same mechanism is used: category tiles (already caught, shipped as part of #566), the shared PLAY button (not yet ticketed, high leverage — one fix covers all 12 games), a "Related games" carousel (not yet ticketed, not confirmed to exist yet). Full detail: `ADVISOR_TO_PM.md` 2026-09-13#3.

**2026-09-13#2 — shipped in #565.** Designer's Menu-page spec (missing Dark/Light control + an opening-animation bug) — both confirmed real; the animation bug traced past Designer's own two guesses to its exact mechanism (`useMenuOverlay.ts`'s `anchorRect` starting `null`, only set at click time, so the first-ever open per hub screen animates the clip-path's center and radius together instead of just the radius). Full detail: `ADVISOR_TO_PM.md` 2026-09-13#2.

**2026-09-13#1 — shipped in #566.** Designer's Games-hero spec (category tiles, rail fades, section title) — all 3 confirmed real against current code, every prototype citation spot-checked accurate. Full detail: `ADVISOR_TO_PM.md` 2026-09-13#1.
