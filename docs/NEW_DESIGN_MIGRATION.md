...
## Status snapshot — 2026-09-11 (post-deploy) — supersedes all earlier snapshots in this section

**DEPLOYED.** `gcloud run deploy` succeeded, revision `rapidclash-00100-fjw`, 100% traffic, live at https://rapidclash-847070222251.us-central1.run.app (confirmed responding). demo-taker's bot-crowd restarted per the runbook. This is the first deploy since the design migration began — everything below "Fully DONE and merged" is now live, not just on `main`.

**Fully DONE, merged, and deployed:** Shared-chrome T1–T3b (#478/#489/#487/#494/#504). rps/mines/dice reskin: T5 shared VS-label, T6a Dice, T6b RPS + its follow-up #512 (#508/#509/#510/#514). T7 Mines engine rewrite to the Designer's 5×5 ruleset in two Owner-reviewed PRs per ADR-012 (#516 core contract, #517 Mines rewrite — a real redaction bug caught and fixed by the PM's review before merge). T8 Mines visual rebuild (#521, real tile SVGs + the 30s round clock). T9 GameHub `$` skin fix (#522). #497 (light-mode `HubToolbar` bug, #519), #501 (games-chance rail-scroll offset, #520), T4 + reconciliation sweep (#523/#524). Plus #133 (untracked design assets committed) and #77 (prettier config drift) — small non-migration backlog items the PM picked up alongside. Harness bug fix + coverage (#513). Asset triage (#505). Designer handoff backup (#506).

**Two more scoped, ready to dispatch — `ADVISOR_TO_PM.md` 2026-09-11#4, both explicitly NOT urgent (deploy already happened):**
- **Mines idle-state polish (small).** Confirmed the prototype has no separate idle design — its tile-computation function is the same one used live, just all-covered. This is "finish applying T8's own already-cited colors/radii to `MinesIdle`," not new investigation.
- **Currency-picker dropdown (a real feature, sized honestly — bigger than T9 was).** The wallet chip's multi-currency dropdown doesn't exist in the app yet: search input, Cash/Cryptocurrency sections, 8 currency rows with mock balances (confirmed hardcoded in the prototype's own `CUR_BAL`/`CUR_CRYPTO` objects — intentional per the existing Currency-presentation section, not something to "fix"), open/close animation, backdrop click-away. A fiat/crypto toggle is also present in the prototype, flagged as a separable judgment call for whoever scopes implementation, not decided here. Cosmetic-only (Charter #4), correctly deprioritized once already — no urgency now either.

**Advisor next:** available, no open harness thread. **PM next:** dispatch the two above whenever convenient; otherwise pick the next roadmap item.

**Still queued, no scoping started:** chat; races/leaderboards; lobby/stake-entry/play/result → one screen with internal phases. Multi-day, unscoped — not started deliberately, don't assume timeline.

**Process:** Advisor → worktree `worktree-advisor-migration`; PM → primary checkout; each coder → its own agent worktree.

**Standing lesson from this session, worth remembering:** three confident-but-wrong-or-nearly-wrong moments tonight (a tile-art misread, the "no design source exists" claim, and briefly reading `MinesHub.tsx` on a stale pre-T7 branch while scoping T8) all trace to the same root cause — trusting a check that stood in for actually looking at the current, real thing (a screenshot, a `git log`, a running page). Caught each one before it went anywhere, but the pattern is worth carrying forward: confirm you're looking at what you think you're looking at before drawing a conclusion from it.
