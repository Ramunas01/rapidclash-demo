...
## Status snapshot — 2026-09-15 (Open Games light-mode ticket now queued; footer light-mode ticket shipped in #587, merged; deploy gap found and closed same-day — Pepe image confirmed retired, live) — supersedes all earlier snapshots in this section

**2026-09-15#4 — NEW, current top item, not yet dispatched. Designer sent a spec for the Open Games section's light-mode theming — same root-cause class as the Footer ticket (hardcoded `#1A1A2E`/`#FFFFFF` literals instead of `var(--rc-surface)`/`var(--rc-text)`), all in one file (`GamesCarousel.tsx`). Full detail: `ADVISOR_TO_PM.md` 2026-09-15#4.**
...
**Advisor next:** available, no open thread — items 1-4 + item 5's handle half are one small, single-file PR, ready to dispatch now; item 5's tier-icon half needs no code, just a note back to Designer. **PM next:** dispatch the fix.
