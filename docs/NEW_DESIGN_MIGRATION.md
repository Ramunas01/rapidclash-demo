...
## Status snapshot — 2026-09-16 (D01-D20 all shipped and merged and fully closed; D21 — Dice's reveal choreography — investigated, ready to ticket; D22 — Mines board clutter + colour check — investigated, ready to ticket, colours/layout already confirmed pixel-perfect, 3 extra gaps found beyond Designer's own list) — supersedes all earlier snapshots in this section

**2026-09-16#5 — NEW, top item, ready to ticket. Designer's D22: strip a duplicate status row + explainer paragraph + Resign button (none exist in the design) from the Mines board, and verify every cited colour. Full detail: `ADVISOR_TO_PM.md` 2026-09-16#5.**
...
**Advisor next:** available, no open thread. **PM next:** dispatch — item 1 is feature-sized but low-risk (reusing Blackjack's own proven pattern); items 2-4 are small and ship alongside it.
