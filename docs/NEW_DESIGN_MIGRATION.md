...
## Status snapshot — 2026-09-11 (deploy push) — supersedes all earlier snapshots in this section

**Owner's target: something deployed and publicly visible by this evening (~6h from this snapshot).** No deploy triggered yet — this snapshot is for scoping/dispatch coordination during that window, not a deploy record.

**Fully DONE and merged, nothing left to revisit:** Shared-chrome T1–T3b (#478/#489/#487/#494/#504). rps/mines/dice reskin: T5 shared VS-label, T6a Dice visual, T6b RPS visual, and its follow-up #512 (RPS idle VS-card gap) — #508/#509/#510/#514. T7 Mines engine rewrite to the Designer's 5×5 ruleset, in two Owner-reviewed PRs per ADR-012's sequencing (#516 core contract, #517 Mines rewrite) — a real redaction bug (opponent score leaking via the broadcast event before both players locked) was caught by the PM's review and fixed before merge, not after. Harness bug fix + `mines-idle`/`rps-idle`/`dice-idle` coverage (#513). Asset triage (#505). Designer handoff backup (#506).

**In flight right now (PM dispatching):** **#497** (light-mode `HubToolbar` vertical bug) + **#501** (games-chance rail-scroll offset) in parallel, then **T4 + reconciliation sweep** (PR #503's ticket — ChessHub `bg-success` dark-pin, `--brand-purple` vs. `#8B45F0`, `HubRibbon` padding re-measurement). All three small, independent, safe — good deploy-eve polish even alone.

**Two new tickets just written, ready to dispatch — `ADVISOR_TO_PM.md` 2026-09-11#3:**
- **T8 (Mines visual rebuild).** T7 already right-sized the board (25 cells/5×5); still needed: the real prototype tile SVGs (gem/bomb, replacing generic lucide icons) and the 30s round clock UI, which needs one small safe addition first — `viewFor`'s non-terminal branch doesn't expose `roundStartedAt` to the client yet (pure timing metadata, not exploitable; a one-field Mines-only change, no ADR needed unlike T7).
- **T9 (GameHub `$` skin gap).** Found via tonight's own harness diffs: `GameHub.tsx`'s bet-preset buttons (already the correct `[1,5,10,25,50,100]` ladder) render via `<Credits>`, whose own doc comment says "never `$`" — predates the Owner's Sept 9 approval. `HubRibbon.tsx` (T2) already solved this exact conflict once with its own inline `$` treatment; T9 mirrors that same pattern for `GameHub.tsx` only. **Deliberately not a sitewide `<Credits>` sweep** — `isGuest` isn't threaded into 5 of the 9 files that use `<Credits>`, so a full sweep is real unscoped work, explicitly deferred as future follow-up rather than rushed into this window.

**Housekeeping merged:** #493 (this tracker), #505 (asset triage), #506. #502 closed unmerged (its content had already landed via a different path — merging would've just fought the append-only-log for nothing, per the PM).

**Advisor next:** available, no open harness thread. **PM next:** #497/#501 → T4+sweep → T8/T9 once ticketed (this snapshot), in whatever parallelization the ≤2-agent cap allows — none of the five compete for the same files.

**After this batch, still queued, not started, no scoping done yet:** currency skin's other half (the multi-currency wallet-picker dropdown — cosmetic only, explicitly lower priority than T9's `$` fix); chat; races/leaderboards; lobby/stake-entry/play/result → one screen with internal phases. None of these are realistic within tonight's ~6h window — flagged so nobody assumes they're in scope for the deploy push.

**Process:** Advisor → worktree `worktree-advisor-migration`; PM → primary checkout; each coder → its own agent worktree.

**Standing lesson from this session, worth remembering:** two confident-but-wrong conclusions earlier tonight (a tile-art misread, and the "no design source exists" claim) both came from a text/grep-based check standing in for actually driving the running artifact — fixed each time by stopping and looking at the real screenshot. A third near-miss happened scoping T8 tonight: started reading `MinesHub.tsx` on a stale branch (`docs/pause2`, predates T7's merge) and almost drew conclusions from pre-T7 code — caught via a plain `git log` check before writing anything down. Worth the habit: confirm which commit a file read is actually on before trusting what it says, especially right after a branch switch.
