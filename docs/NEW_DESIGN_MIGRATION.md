...
## Status snapshot — 2026-09-17 (D01-D23 all shipped, merged, and deployed, fully closed; D24 — Mines icon/halo drift, own-clock result sequencing, live own-gem-bar — ALL 3 items shipped, merged (PR #631 items 1+3, PR #632 item 2), and deployed live at `rapidclash-00118-b2t`; item 2's fix correctly generalizes to every lock reason, not just bust — a deliberate, verified behavior change, not a regression, worth knowing before any future "the win animation got slower" report) — supersedes all earlier snapshots in this section
