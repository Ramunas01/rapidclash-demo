...
**Chat — fully scoped, `ADVISOR_TO_PM.md` 2026-09-11#6.** This is the biggest remaining item — a real feature (client sheet UI + a new server-side WS chat transport + a kill switch), not a reskin. Full sheet/room-switcher/@mention/VIP-tier-color UI confirmed real in the prototype (not a stub); architecture follows the existing `EphemeralLedger extends Ledger` precedent for a `ChatTransport` (in-memory, gone on restart — this has to be genuine live cross-player messaging, not client-only, or the tracker's own "collusion-channel risk in a PvP money game" concern wouldn't make sense). **Two real decisions flagged for Owner/PM, not assumed:** (1) one global room vs. a room-per-game pattern — the mock only proves one example (`chess`) beyond the default `general`; (2) what "server-checked kill switch" needs to mean — a plain boot-time env var enforced server-side (matches existing precedent, ships faster) vs. something flippable live without a redeploy (new infra, nothing like it exists in this codebase today). Recommended answers given in the ticket; not decided there.
