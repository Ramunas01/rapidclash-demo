...
## Status snapshot — 2026-09-13 (RPS/Mines/Dice reveal bug shipped; new Designer handoff on Games hero, current top item) — supersedes all earlier snapshots in this section

**2026-09-13#1 — NEW, current top item. Designer sent a direct, line-cited spec for the Games-hero screen (category tiles, rail fades, section title) — all 3 confirmed real against current code, every one of Designer's own prototype citations spot-checked accurate. Full detail: `ADVISOR_TO_PM.md` 2026-09-13#1.**

1. **Category tiles:** background bug confirmed exactly as Designer described (`bg-brand/10` on the selected tile instead of `bg-surface` unconditionally — the prototype never varies tile background by selection at all). Ledge shadow (`0 7px 0 …`) and press feel (`translateY(4px)` + `rcNavPop` release animation) both confirmed **completely absent**, not approximated — zero hits anywhere in the codebase for either. Two minor mismatches too (icon size 26px uniform vs. 29px/25px split, icon-label gap 10px vs. 9px). Tile size/radius/tap-to-center scroll already correct, no change needed there.
2. **Category rail edge fades:** confirmed entirely absent (zero fade-overlay elements anywhere on the rail) — matches the screenshot's hard-clipped "EV…" tile. Prototype's fades are scroll-position-driven, not static (`catLeftFade`/`catRightFade` computed from `scrollLeft`/`maxScroll`) — flagging Designer's own warning explicitly: a fixed-opacity version would pass a still screenshot and look wrong the moment the rail actually scrolls.
3. **Section title:** icon confirmed wrong — a static two-tone brand-logo image (`boltMark`) renders unconditionally, never actually changing with category, when the prototype shows the *selected category's own icon* (solid `#8B45F0`, 26px, five different SVGs). The exact SVGs already exist as React components in this file's own `CATEGORY_ICON` map (used by the tiles) — just needs reuse in the title row. Font confirmed wrong on every axis: 15px/font-black/no-family vs. the prototype's 21px/bold/Arial — six points off in size alone, cross-checked against Rewards' own `YOUR REWARDS` headline (19px, same family/weight) to confirm 21px is deliberately larger, not a rounding difference.

**Advisor next:** available, no open thread — #1 is the current ask, all three sections cheap and can go in one PR. **PM next:** dispatch as one `HomeHub.tsx` PR (all three touch the same file + its existing `categories.ts` icon map), then run the design-fidelity harness on this screen in both themes per Designer's own ask.
