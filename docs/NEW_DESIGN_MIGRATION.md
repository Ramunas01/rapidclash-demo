...
## Status snapshot — 2026-09-11 (late, Owner's manual QA pass) — supersedes all earlier snapshots in this section

**Fully DONE and merged on `main`** (production redeploy still pending Owner timing — see below): the entire shared-chrome + light-theme workstream (T1–T3b), the rps/mines/dice reskin (T5/T6a/T6b/T7/T8/T9 + follow-ups #512/#519/#520), T4 + reconciliation sweep, the currency-picker dropdown, **Chat in full** (server transport #537 + client sheet #538, both reviewed with real trust-boundary verification, not just a read-through), plus #532's bot-crowd top-up bug (found, fixed, and the stuck bots' state cleared same night) and #497's honest self-correction (the original diagnosis was wrong; the eventual fix, by someone else, was right). Full narrative of all of the above is in this section's git history if anyone needs the blow-by-blow — this line is the current summary.

**Deploy status:** production is still running the single deploy from earlier tonight (`rapidclash-00100-fjw`) — everything above has landed on `main` since, meaning there's now a substantial, growing gap between what's live and what's actually built. PM is deliberately checking with the Owner on deploy timing rather than assuming — correct call, don't second-guess it.

**Investor-readiness estimate delivered** (Artifact: "Prototype Parity," side-by-side screenshots of designer intent vs. current `main` for the 7 highest-traffic screens, both themes, real fidelity scores) — headline findings: redeploying alone closes real visible gap at zero engineering cost; the lobby/stake-entry/play/result "architecture gap" is actually dead code today (every one of the 12 live games already routes through the unified `GameHub`); everything genuinely unbuilt (chat was still pending at the time, races, tournaments) already fails gracefully behind a toast or a disabled nav badge, not a broken screen.

**Owner did a manual, screen-by-screen visual QA pass on RPS against the prototype and found 9 real discrepancies — `ADVISOR_TO_PM.md` 2026-09-11#8, all verified against current `main`, all confirmed real.** Three root causes, two of them platform-wide:
- **(A) `GameHub.tsx`'s bet/play panel — affects all 12 games.** Missing the currency-name + live-bet-value row entirely; the bet-amount presets should be one shared pill/ribbon with a sliding indicator, not 6 individually-boxed buttons; `PLAY`/`Play a Friend` are missing the prototype's drop-shadow (`0 5px 0 #5F27B8`, the "pressable" look); "PROVABLY FAIR BY DESIGN" + its shield-check icon doesn't exist anywhere in the codebase.
- **(B) `GamesCarousel.tsx`'s "OPEN GAMES" ticker — affects all 12 game hubs + HomeHub + ProfileHub.** A real, precisely-root-caused bug: usernames render with a doubled `"@@"` because bot-crowd's own names already embed an `@` (`"🤖@sweeper"`) and the carousel blindly prepends another. **Not a simple reuse of `ProfileHub.tsx`'s existing `normalizeOpponentName`** — that helper also strips the 🤖 disclosure emoji, which ADR-010's informed-consent requirement says must stay on *this specific* feed. Correct fix is narrower (strip only a leading `@`, keep the emoji). Also: still shows the old green `RcIcon` in three spots instead of the `$` skin (T9 only touched `GameHub.tsx`, never this file); no VIP-tier badge next to usernames at all (real net-new addition, not a bug, lower priority within this ticket).
- **(C) RPS-specific, `RpsHub.tsx`.** Placeholder Unicode emoji (✊✋✌️, 🤫) were never swapped for the prototype's real flat-vector purple hand art and its solid-blue-fill opponent card — a T6b fidelity shortcut, not a functional bug. Plus one extra idle-state sentence the prototype doesn't have.

All three tickets are visual/asset-level only — no engine or mechanic risk, no open decisions needed from Owner or PM this time.

**Owner then compared RPS's actual match flow (not just the idle screen) and found two behavioral differences — both deliberate decisions made earlier tonight, and Owner has given an explicit, clear direction on both: `ADVISOR_TO_PM.md` 2026-09-11#9.** Standing project stance now, worth remembering for any future case like this: **at this stage, visual/experiential fidelity to the Designer's vision outweighs correctness** — this demo exists to persuade investors the team can execute a compelling vision, not to be production-correct; the actual production build happens later, on a different platform. Where matching the prototype exactly means accepting a real gap the code was deliberately built to avoid, **accept the gap and document why in a code comment** rather than silently keeping the safer behavior.
- **RPS's opponent-search animation** (T5's `searchFloorMs={0}`, chosen tonight to not waste play time) — reversed for RPS: restore the prototype's ~3.8s name-spin → found → split sequence. Open question flagged, not decided: Coinflip shares the identical mechanism and reasoning — does it get the same reversal for consistency, or stay as-is?
- **RPS's per-round opponent reveal** — currently the opponent's throw is never shown except on the match's final decisive round (deliberately, to prevent a player pattern-reading ties round-to-round). Reversed: reveal the real throw every round including ties, matching the prototype. This needs an actual protocol change, not a flag flip — `packages/games/rps/src/rps.ts`'s tie-resolution branch clears the throws from state before a client could ever read them, so the fix has to carry them in the `new_round` event itself, with a mandatory code comment at that exact spot explaining this is an intentional, Owner-approved information leak, not a bug for someone to "fix" back later without checking first.

**Advisor next:** available, no open thread — the two mailbox entries above are the current ask. **PM next:** dispatch A/B/C from 2026-09-11#8 (A and B are higher-leverage, prioritize over C) and both items from 2026-09-11#9 whenever convenient; races/leaderboards and lobby-collapse-cleanup still unscoped whenever there's bandwidth.
