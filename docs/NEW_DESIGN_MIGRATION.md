...
**Post-deploy live-testing turned up a real, separate operational bug — filed as #532, not part of the design-migration workstream:** Owner tested the live demo and saw "0 LIVE" games. Investigated directly against the running server (Cloud Run logs, `demo-taker`'s bot-crowd logs) rather than guessing — root-caused it precisely: `tools/bot-crowd/src/bot.ts`'s top-up idempotency key uses a per-process in-memory counter that resets to 0 on every restart (i.e. every deploy). The ledger correctly treats the reused key as a duplicate and no-ops the credit while still returning `200 OK` — bot-crowd's client-side balance tracker credits itself anyway (locally), so bots that have ever needed a top-up before eventually get permanently stuck believing they have money they don't, and can never post/take again. Explains exactly why Mines (only 1 rester + 1 taker in the base roster) can show zero live activity even while the service is otherwise healthy and other games look fine. **Confirmed unrelated to any of tonight's T5–T9/T7 work** — pure interaction between bot-crowd's restart cadence and the ledger's (correct) idempotency design. Fix is small and contained to `tools/bot-crowd/src/bot.ts` (swap the resettable counter for a random UUID per top-up attempt) — no server/ledger change needed.

**Advisor next:** available, no open thread. **PM next:** #532 whenever convenient (not urgent, but will keep quietly recurring after every future deploy until fixed); otherwise pick the next roadmap item.
