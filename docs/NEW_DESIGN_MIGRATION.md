...
D23 is now split cleanly: items 1-3 done (merged, awaiting deploy); items 4-5 (bar convergence + gem-count rows) remain to be scoped as their own ticket, closing out `2026-09-12#2`'s long-open 3(a)/3(c) — next Advisor action once picked up.

**Advisor next:** scope items 4-5 as their own ticket. **PM next:** ready to start on 4-5 as soon as that ticket lands (per their own message); nothing else queued.
