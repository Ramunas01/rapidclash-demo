...
## Status snapshot — 2026-09-12 (RPS/Mines/Dice all shipped; HIGH-PRIORITY shared bug found in Mines+Dice's live reveal) — supersedes all earlier snapshots in this section

**2026-09-12#5 — HIGH PRIORITY, current top item. Owner's live post-merge Dice pass (#558) found a real bug that turns out to also affect Mines' already-shipped #555 — same root cause, both games. Full detail: `ADVISOR_TO_PM.md` 2026-09-12#5.**

1. **The live board unmounts to the idle preview during `'result'` phase — confirmed for BOTH Dice and Mines, Mines is worse (zero grace period).** `DicePanel`/`MinesPanel` both gate on `phase === 'in-match'` only, reverting to the blank idle preview the instant `phase` becomes `'result'` — discarding the fully-resolved board even though `gameState` still has it. Dice at least shows it first for `HOLD_MS`'s 2200ms before vanishing (matches Owner: "briefly visible... then vanish"); Mines has **no `holdResultMs` at all**, so its board may never be visible before reverting to blank. The correct pattern already exists, unused by either fix: `CoinflipPanel`'s `const live = phase === 'in-match' || phase === 'result';` (`CoinflipHub.tsx:120`). **Fix:** apply the identical `live` gate to both `MinesPanel` and `DicePanel` — one root cause, recommend one combined PR.
2. **Missing matching-phase table dim for Dice** — same fix RPS (#551) and Mines (#555) already got, never applied to Dice. Confirmed absent (`barSlideActive` has zero references in `DiceHub.tsx`).
3. **Cube "travel" doesn't exist at all — real gap, needs a no-fabrication-safe fix.** The cube is invisible until the true roll is known, then pops directly to its final position — no slide, because `left` isn't even a transitioned property. Fix respects the standing no-fabricated-data rule: make the cube visible from match-start at its known 0% resting spot, and animate its POSITION (not any number) to the true endpoint once known — an honest slide between two always-true points, no invented intermediate value. Also flagged: the existing ~2.4s search-floor dwell likely already gives Dice a post-match-found pause, but items 1+2's missing visual cues probably make it read as "nothing happened" — worth re-checking pacing only after 1+2 ship.
4. **Mines: a full text audit of `MinesBoard`'s live-play HUD, at Owner's request for a systematic (not one-by-one) check.** Method now standard for reuse: grep every quoted string literal out of a hub's `.tsx`, cross-check each against the whole prototype file. Result: the entire live-play status block (`MinesHub.tsx:294-386` — own/opponent safe-counts, round number, status line, the rules-explaining paragraph, "Resign") has zero prototype equivalent, because the prototype shows nothing at all during live play beyond the tile grid + clock (its own gem-count text is post-match-only, `mReveal`/`mGrown`-gated). **Not a blanket removal call** — same standing rule as RPS's "Picked {choice}" text and Forfeit button: the opponent's live safe-count is the *only* place that redacted info is shown anywhere, recommend keeping it and the rest, flagged as functional; "Resign" needs an explicit Owner call same as RPS's Forfeit, with *less* prototype support (no unbound dead-code precedent at all, unlike `rpsForfeit`). This is a survey, nothing dispatched.

**Advisor next:** available, no open thread — #5 is the current ask, item 1 first (it's a correction to two already-shipped PRs). **PM next:** dispatch item 1 as its own combined `MinesPanel`+`DicePanel` fix; item 2 is cheap/Dice-only; item 3 needs an implementation decision on durations, flagged not prescribed; item 4 needs Owner's call on "Resign" before any dispatch, the rest are recommended keeps.
