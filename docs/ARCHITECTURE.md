...
| Durable state | SQLite (file) for the demo, snapshotted to GCS for durability (ADR-011) → Postgres (Cloud SQL) later | The ledger, accounts, and standings must outlive a process restart. Start file-backed + snapshot/restore, migrate to Postgres with the same schema. |
...
- **ADR-011 — Durable demo persistence via periodic SQLite→GCS snapshot; password-clear soft reset.** Accepted. The demo needs a small amount of state (~10–100 kB: accounts, hashed credentials, standings) to survive Cloud Run instance recycle/redeploy, which ADR-009's local SQLite does not. Rather than pull the scale-phase Cloud SQL forward, the server **snapshots the SQLite file to a Cloud Storage bucket** — debounced, on settlement/standings writes — and **restores it on startup** when present. Schema and queries are unchanged (consistent with the file-backed→Postgres plan), it stays GCP-native, and it is effectively free at this size; `max-instances=1` means a single writer, so no concurrency concern. SQLite-over-GCSFuse is rejected (network-FS locking/corruption); the durable path is explicit snapshot/restore, never a mounted live DB. **Passwords are stored only as salted hashes** (bcrypt/argon2), never plaintext. **Reset becomes two tiers:** a **soft reset** clears an alias's password — preserving the alias and its standings and auto-re-granting that account's wallet — so the alias is re-claimable by setting a new password (keeps leaderboard position, fresh wallet); the **hard reset** is the unchanged full DB wipe. Standings persist **independently of the wallet** so a wallet re-grant never erases leaderboard position. The soft reset reuses the single-account append-only exception (`WALLET_LEDGER.md`) and keeps the active-match/escrow guard. See `ADMIN.md`, `WALLET_LEDGER.md`, `DEMO.md`.
