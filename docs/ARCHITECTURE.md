...
- **ADR-011 — Durable persistence (GCS snapshot/restore), credentials security baseline, and password-clear soft reset.** Accepted. Three decisions taken together because they all touch the persistence and identity layer.

  1. **GCS snapshot/restore.** The Cloud Run SQLite is ephemeral (resets on recycle/redeploy — ADR-009). We add a lightweight, explicit snapshot/restore layer: on startup the server downloads the DB file from a configured Cloud Storage bucket if one is present, restoring the last known state; after each settlement or standings write it debounces an upload back. This gives durability across recycles without changing to Postgres or adding infrastructure. **Ruled out:** GCSFuse (mounting a live DB over a network filesystem is a corruption path); in-process replication; anything that touches the DB while it is open for writes. Single instance (`--max-instances=1`, ADR-009) = single writer; no locking concern. `GCS_BUCKET` env var enables it; unset (local dev) → silent no-op so the dev loop is unchanged. Requires `storage.objectAdmin` on the Cloud Run service account for the chosen bucket; see `DEPLOY.md` for the IAM grant. No schema or query changes.

  2. **Hashed credentials.** Passwords are stored only as salted bcrypt hashes (`bcryptjs`, `BCRYPT_ROUNDS = 12`); plaintext is never persisted; login uses `bcrypt.compare`. This was implemented from the start (`packages/core/src/identity.ts`). Recorded here for completeness: every account, including the pre-seeded admin, uses this path.

  3. **Password-clear soft reset.** Replaces the old remove-account convenience (issue #14) with a gentler tool that preserves match history. New admin action — `POST /admin/players/:id/clear-password` — clears the account's password hash, writes a fresh `ADMIN_CREDIT` ledger entry (the starting grant), and leaves the alias and all standings intact. The alias is then **re-claimable**: a returning player, or a new user, may register with that alias to set a fresh password, inheriting the cleared wallet and keeping the old standings. **Key correctness invariant:** `net_winnings` is derived from match-linked ledger entries (`BET_ESCROW`, `SETTLE_WIN`, `SETTLE_REFUND` — all have a non-null `match_id`); the new `ADMIN_CREDIT` entry has a null `match_id` and is therefore excluded from the leaderboard calculation by construction (ADR-007). ELO ratings are stored in a separate table and are equally unaffected by the wallet reset. The action is refused while the alias has an active match or escrowed stake (same guard as the old remove-account). The **hard reset** (full DB wipe + restart, which re-seeds admin via `ensureAdmin`) remains the canonical operator reset for demo teardown and is unchanged. See `ADMIN.md` for the operator-facing description.

  **Reset model (supersedes the remove-account section in `ADMIN.md`):**

  | Action | Alias | Standings | Wallet | Use when |
  |--------|-------|-----------|--------|----------|
  | Clear password (soft reset) | Preserved, re-claimable | Preserved | Reset to starting grant | Free an alias for re-use without losing match history |
  | Full DB wipe (hard reset) | Gone | Gone | Gone | Full demo teardown; admin re-seeds on startup |
