...
- **ADR-009 — Demo deploys as ONE Cloud Run service that serves API + WebSocket + the built PWA on a single origin.** Accepted. The Node container serves `apps/web/dist` (with SPA fallback) alongside the API and the WS, so one `*.run.app` HTTPS URL delivers everything — no CORS, no second service — and the PWA installs ("Add to Home Screen") because HTTPS is built in, retiring the dev tunnel. `--max-instances 1` is **mandatory**: match state is in-memory and the DB is a local SQLite file, so neither survives scale-out. `--min-instances 1` keeps a WebSocket-warm instance during demos (drop to `0` between them); `--session-affinity` + `--timeout 3600` support the WS / `match.resume` reconnect path; `--allow-unauthenticated` (public demo) with `ADMIN_PASSWORD` from Secret Manager. Reset = redeploy/recycle (SQLite is ephemeral; admin re-seeds via `ensureAdmin`) — the accepted reset story. **No Firebase Hosting, no second deployment.** Persisting data or scaling out is the ADR-005 graduation (Cloud SQL + Redis, then raise `--max-instances`) — deferred. See `DEPLOY.md`.
