...
4. **No real currency.** Wallet credit is granted to new accounts for demonstration only. No payment rails, no crypto, no cash-out. *(Owner-approved 2026-09-09: the registered/investor demo may present a **cosmetic** `$` and multi-currency "wallet skin" over these same integer demo credits — no FX, no rail, no real value behind it. The guest preview surface stays `¢` / play-money-framed, unchanged. Details in `NEW_DESIGN_MIGRATION.md` → "Currency presentation".)*
...
Games are grouped in the UI as **Originals** (PvP-native and PvP-redefined signature games), **Classics** (traditional skill games — Chess today, more later), and surfaced alongside **Events** (tournaments). See `SCREENS.md` for the display taxonomy.
...
- **Resolution:** if both chose the **same** side → **draw → instant replay** (fresh seed, no rake — the universal tie rule, 10-replay safety cap then refund). If they chose **different** sides → the server flips the coin (deterministic from the **match seed**, independent of both choices) and the player whose choice matches the flip result wins `pot − rake`; the other loses. Exactly one winner.
