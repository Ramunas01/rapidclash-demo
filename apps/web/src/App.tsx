...
type Screen = 'auth' | 'home' | 'profile' | 'wallet' | 'game-list' | 'stake-entry' | 'lobby' | 'play' | 'result' | 'leaderboard' | 'coinflip-hub' | 'rps-hub' | 'mines-hub';
...
const HUB_GAMES = new Set(['coinflip', 'rps', 'mines']);
const hubScreenFor = (gameId: string | null | undefined): Screen | null =>
  gameId && HUB_GAMES.has(gameId) ? (`${gameId}-hub` as Screen) : null;
const isGameHubScreen = (s: Screen): boolean => s === 'coinflip-hub' || s === 'rps-hub' || s === 'mines-hub';
...
export function App() {
  ...
  function renderScreen() {
    switch (screen) {
      ...
      case 'rps-hub':
      case 'mines-hub': {
        const HubScreen = screen === 'rps-hub' ? RpsHubScreen : screen === 'mines-hub' ? MinesHubScreen : CoinflipHubScreen;
        return <HubScreen
          token={token!}
          playerId={playerId}
          username={username}
