...
function challenge(over: Partial<OpenChallenge> = {}): OpenChallenge {
  const now = Date.now();
  return { matchId: 'm1', ownerName: 'alice', ownerTier: 'Unranked', stake: 10, openedAt: now - 6000, expiresAt: now + 48000, timeControlId: 'none', ...over };
}
...
describe('OpenChallengesList (OC2)', () => {
  it('renders a card per entry with owner, stake, and a live countdown', () => {
    ...
    expect(screen.getByTestId('stake-m1').textContent).toContain('10');
...
describe('applyChallengesUpdate', () => {
  const base: OpenChallenge[] = [
    { matchId: 'a', ownerName: 'amy', ownerTier: 'Unranked', stake: 10, openedAt: 1, expiresAt: 100, timeControlId: 'none' },
    { matchId: 'b', ownerName: 'bob', ownerTier: 'Unranked', stake: 20, openedAt: 2, expiresAt: 200, timeControlId: 'none' },
  ];

  it('appends an added challenge, keeping longest-waiting-first order', () => {
    const added: OpenChallenge = { matchId: 'c', ownerName: 'cleo', ownerTier: 'Unranked', stake: 30, openedAt: 0, expiresAt: 90, timeControlId: 'none' };
