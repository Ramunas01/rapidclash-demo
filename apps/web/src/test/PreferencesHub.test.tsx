...
describe('PreferencesHubScreen', () => {
  ...
  it('persists the marketing and sound toggles independently', () => {
    render(<PreferencesHubScreen onBack={() => {}} />);

    const sound = screen.getByTestId('preferences-game-sound');
    expect(sound).toHaveAttribute('aria-checked', 'true'); // default ON
    fireEvent.click(sound);
    expect(sound).toHaveAttribute('aria-checked', 'false');
    expect(localStorage.getItem('rc_pref_gameSound')).toBe('0');
