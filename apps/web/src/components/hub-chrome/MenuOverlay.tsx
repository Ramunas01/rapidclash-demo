...
interface Props {
  ...
  /** GAMES group's "RapidClash Originals"/"Card games"/"Chance games"/"Skill games" rows (issue
   *  #465) → open the Games view pre-filtered to the tapped category, replacing what used to be
   *  each row's own placeholder toast. Optional: a caller that hasn't wired a category-aware
   *  destination (every hub screen other than HomeHub itself, which renders this overlay directly
   *  and can just flip its own local category state) falls back to the plain `onOpenGames()`
   *  navigation below — unfiltered, but a real destination rather than a dead toast. */
  onOpenGamesCategory?(category: CategoryId): void;
}
...
type RowAction = 'games' | 'rewards' | 'affiliate' | 'games-category' | 'placeholder';
...
interface MenuRow {
  ...
  icon: ReactNode;
  action: RowAction;
  /** Only present on `action: 'games-category'` rows (issue #465). */
  category?: CategoryId;
}
 ...
 * #465) — shared with HomeHub.tsx's category rail, which renders the exact same four glyphs. */

const ICON_ORIGINALS = <OriginalsIcon className={ROW_ICON} />;
const ICON_CARD_GAMES = <CardGamesIcon className={ROW_ICON} />;
const ICON_CHANCE_GAMES = <ChanceGamesIcon className={ROW_ICON} />;
const ICON_SKILL_GAMES = <SkillGamesIcon className={ROW_ICON} />;
...
const GROUPS: MenuGroup[] = [
  {
    label: 'GAMES',
    rows: [
      { key: 'originals', label: 'RapidClash Originals', icon: ICON_ORIGINALS, action: 'games-category', category: 'originals' },
      { key: 'card-games', label: 'Card games', icon: ICON_CARD_GAMES, action: 'games-category', category: 'card' },
      { key: 'chance-games', label: 'Chance games', icon: ICON_CHANCE_GAMES, action: 'games-category', category: 'chance' },
      { key: 'skill-games', label: 'Skill games', icon: ICON_SKILL_GAMES, action: 'games-category', category: 'skill' },
    ],
  },
  {
    label: 'EARN',
...
export function MenuOverlay({ open, anchorRect, onClose, onOpenGames, onOpenRewards, onOpenAffiliate, onOpenGamesCategory }: Props) {
  ...
  function navigate(row: MenuRow) {
    ...
    else if (row.action === 'games-category') {
      // A caller that hasn't wired a category-aware destination still gets a real navigation
      // (unfiltered), not a dead toast — see the Props doc comment on `onOpenGamesCategory`.
      if (onOpenGamesCategory && row.category) onOpenGamesCategory(row.category);
      else onOpenGames();
    }
