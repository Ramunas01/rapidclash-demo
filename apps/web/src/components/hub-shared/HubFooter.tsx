...
const SOCIALS: { label: string; icon: ReactNode }[] = [
  ...
  // Deliberate logo/label mismatch (Designer): keep the X glyph, label reads "Twitter". testid → home-social-twitter.
  { label: 'Twitter', icon: (<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M18.2 2H21l-6.4 7.3L22 22h-6.2l-4.8-6.3L5.5 22H2.7l6.8-7.8L2 2h6.3l4.4 5.8L18.2 2Zm-1 18h1.6L7.4 3.7H5.7L17.2 20Z" /></svg>) },
    ...
    <footer data-testid="home-footer" className="mt-4 bg-background px-4 pb-6 pt-6">
      ...
      <div className="mb-5 flex gap-3 rounded-[14px] bg-surface p-3.5">
        ...
        <p className="text-[11.5px] leading-relaxed text-foreground">
          <b className="text-brand">Provably fair, by design.</b> Every match runs on a seeded RNG you can verify — the platform never plays against you, just a flat fee per game.{' '}
      ...
      <div className="mb-5 grid grid-cols-2 gap-x-3.5 gap-y-2.5">
        {FOOTER_LINKS.map((l) => (
          <span key={l} className="cursor-default text-[12.5px] text-foreground">{l}</span>
        ))}
      </div>
