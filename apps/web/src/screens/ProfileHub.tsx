 ...
 *  otherwise"). Icons are this codebase's own lucide-react primitives (README's own instruction:
 *  recreate with the target codebase's primitives, not a literal port of the prototype's custom
 *  inline SVGs) standing in for the design's hand-drawn glyphs — a reasonable substitution for
 *  rows that are otherwise-inert placeholders in this ticket's scope. */
const FIRST_GROUP: { key: string; label: string; icon: LucideIcon }[] = [
  { key: 'account-details', label: 'Account details', icon: IdCard },
  { key: 'verification', label: 'Verification', icon: ShieldCheck },
  { key: 'security', label: 'Security', icon: Lock },
];
const SECOND_GROUP: { key: string; label: string; icon: LucideIcon }[] = [
  { key: 'responsible-gaming', label: 'Responsible gaming', icon: HeartHandshake },
  { key: 'blocked-players', label: 'Blocked players', icon: UserX },
...
export function ProfileHubScreen({ token, username, avatarId = 'default', onAvatarChange, balance, onLogout, onHome, onOpenProfile, onOpenRewards, onOpenPreferences }: Props) {
  ...
  function renderControlRow({ key, label, icon: Icon }: { key: string; label: string; icon: LucideIcon }) {
    ...
    return (
      ...
      >
        <Icon style={{ width: 19, height: 19, flex: '0 0 19px' }} color={RC.muted} />
        <span style={{ flex: '1 1 auto', fontSize: 14, fontWeight: 600, color: RC.text }}>{label}</span>
        <ChevronRight style={{ width: 12, height: 12, flex: '0 0 12px' }} color={RC.muted} />
      </button>
    );
  }

  const controlRows = [...FIRST_GROUP, { key: 'preferences', label: 'Preferences', icon: Settings }];
