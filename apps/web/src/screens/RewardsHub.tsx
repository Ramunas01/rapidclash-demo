 ...
 * agree exactly). `xpRequired` doubles as this tier's OWN threshold (needed, alongside the
 ...
 */
const TIER_ORDER: { tier: Exclude<VipTier, 'Unranked'>; xpRequired: number }[] = [
...
function progressPercent(xpLifetime: number, tier: VipTier, nextTier?: { xpRequired: number }): number {
...
/** Per-tier badge icon — the same path data the design uses for BOTH the VIP_ROWS table's
 *  76px-wide header column icons (19px) and the progress row's current/next labels (15px), and
 *  the Monthly Volume Bonus accordion's inline Emerald/Diamond glyphs (16px). `useId` keeps the
 *  Wood clip-path unique across however many times this renders on one page. */
function TierIcon({ tier, size }: { tier: VipTier; size: number }) {
  const clipId = useId();
  switch (tier) {
    case 'Unranked':
      // No badge exists below Wood — the design's 6-tier icon set starts at Wood.
      return null;
    case 'Wood':
      return (
