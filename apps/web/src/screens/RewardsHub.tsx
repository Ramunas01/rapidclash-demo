 ...
 * (which already had a centralized `RC` object before its own light-threading pass, #491), this file
 * had NO indirection layer at all: ~96 raw hex-literal occurrences scattered directly through
 * `style={{...}}` props and SVG `fill=`/`stroke=` attributes, so it stayed dark-only even once the
 * rest of the app went light (T1, issue #472). This object is new, not a migration of an existing
 * one — introduced here as the mechanism given the sheer number of call sites (same reasoning
 * ProfileHub's own comment gives for keeping a small object rather than inlining `var(--rc-*)` at
 * every site: this file consumes color via plain inline `style={{}}` and raw SVG attributes
 * throughout, so the object matches the file's own call-site shape). `purple` aliases the fixed
 * `--brand-purple` (not a themed `--rc-*` token) — this screen's accent stays the same fixed brand
 * purple in both themes, matching the prototype's own `navXColor`/pill literal precedent used
 * elsewhere (`HubToolbar.tsx`), same reasoning `ProfileHub.tsx`'s `RC.purple` already uses.
 *
 * Two things deliberately NOT routed through this object, left as raw literal hex:
 *  - The `RakebackIcon`/`VolumeBonusIcon`/`RakebackFormulaIcon`/`VolumeBonusFormulaIcon` decorative
 *    illustration shading ramp (`#A870F0`/`#D8D3E8`/`#BFB8D6`, and the small `#0B0B0B` depth-shadow
 *    outlines paired with them) — these are secondary/tertiary shading facets on a purple-brand
 *    illustration, not surface/text roles, and (per the reference fidelity captures,
 *    `tools/design-fidelity/references/{dark,light}/rewards.frame0.png`) they don't visibly change
 *    between themes, matching `AffiliateHub.tsx`'s own explicit precedent for the same class of
 *    color ("Secondary/tertiary shading facets keep the handoff's own literal ramp hex... the
 *    README's 'Fixed brand values'"). The small white sparkle accents drawn OUTSIDE that shading
 *    ramp in the same two icons DO re-theme in the reference captures (white in dark, near-black in
 *    light) — those route through `RC.text` like every other text/glyph color in this file, not
 *    left literal.
 *  - `CheckIcon`'s contrast stroke and the VIP-tier-highlight border/progress-bar-track/locked-CLAIM
 *    background all shared the literal `#0B0B0B` in the dark-only source, but split into two
 *    semantic roles here: the two panel backgrounds route through `RC.sunken` (confirmed against the
 *    light reference frames — both render a light recessed grey, not white, in light mode); the
 *    `CheckIcon` stroke also routes through `RC.sunken` (not verified directly against a captured
 *    frame — the VIP-tier-benefits row isn't in a committed reference frame — chosen because it
 *    gives a legible light-on-dark-green mark in light mode, same as the other two; flagged in the
 *    issue #498 PR description for a design pass to confirm). The `RakebackFormulaIcon`/
 *    `VolumeBonusFormulaIcon` depth-shadow strokes above are a third, unrelated use of the same
 *    literal value and stay raw hex per the decorative-ramp reasoning above — not the same call.
 */
const RC = {
  text: 'var(--rc-text)',
      ...
      <main data-testid="rewards-hub">
        <div className="mx-auto flex max-w-md flex-col">
          <div style={{ margin: '6px 16px 0 16px', padding: '16px 0 14px 0' }}>
            <span style={{ fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '19px', fontWeight: 'bold', letterSpacing: '0.6px', color: RC.text }}>
              VIP PROGRAM
            </span>
            ...
            <div style={{ marginTop: '14px', background: RC.surface, borderRadius: '22px', padding: '18px 16px 20px 16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', paddingBottom: '16px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px', flex: '0 0 auto' }}>
                  <span style={{ fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '22px', fontWeight: 'bold', letterSpacing: '1.2px', color: RC.text }}>XP:</span>
                  <span data-testid="rewards-xp" style={{ fontFamily: "'Space Grotesk', Arial, Helvetica, sans-serif", fontSize: '22px', fontWeight: 700, color: RC.green }}>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: '12px' }}>
                <span style={{ fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '12px', fontWeight: 'bold', letterSpacing: '0.6px', color: RC.text }}>YOUR VIP PROGRESS</span>
                <span data-testid="rewards-progress-pct" style={{ fontFamily: "'Space Grotesk', Arial, Helvetica, sans-serif", fontSize: '15px', fontWeight: 700, color: RC.green }}>{pct}%</span>
              </div>
              <div style={{ marginTop: '7px', height: '8px', borderRadius: '999px', background: RC.sunken }}>
                <div
                  data-testid="rewards-progress-bar"
                  style={{ width: `${pct}%`, height: '8px', borderRadius: '999px', background: RC.purple, boxShadow: '0 0 12px 2px rgba(139,69,240,0.55)' }}
                />
              </div>
              <div style={{ marginTop: '9px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span data-testid="rewards-tier-current" style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
                  <span style={{ fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '12px', fontWeight: 'bold', letterSpacing: '1.2px', color: RC.text }}>{tier.toUpperCase()}</span>
                  <TierIcon tier={tier} size={15} />
                </span>
                <span data-testid="rewards-tier-next" style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
                  <span style={{ fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '12px', fontWeight: 'bold', letterSpacing: '1.2px', color: RC.text }}>
            ...
            {claimError && (
              <p role="alert" data-testid="rewards-claim-error" style={{ marginTop: '8px', fontSize: '12px', color: RC.danger }}>{claimError}</p>
            )}
            ...
            <p style={{ margin: '18px 0 0 0', fontSize: '15px', lineHeight: '22px', color: RC.text, textWrap: 'pretty', fontWeight: 600, textAlign: 'justify' }}>
...
function RakebackCard({
  ...
  return (
    <div style={{ background: RC.surface, borderRadius: '22px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
      <div>
        <div style={{ fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '12px', fontWeight: 'bold', letterSpacing: '1.2px', color: RC.text, textAlign: 'center' }}>RAKEBACK</div>
...
function VolumeBonusCard({ tier, xpMonthly }: { tier: VipTier; xpMonthly: number }) {
  ...
  return (
    <div style={{ background: RC.surface, borderRadius: '22px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
      <div>
        <div style={{ fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '12px', fontWeight: 'bold', letterSpacing: '1.2px', color: RC.text, textAlign: 'center' }}>VOLUME BONUS</div>
        <div style={{ display: 'flex', justifyContent: 'center', margin: '10px 0 4px 0' }}>
          <VolumeBonusIcon />
        </div>
...
function LockedClaimRow() {
  return (
    <div data-testid="rewards-locked-claim" style={{ position: 'relative', background: RC.sunken, borderRadius: '999px', padding: '11px 0', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
      ...
      <span style={{ fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '13px', lineHeight: '13px', fontWeight: 'bold', letterSpacing: '0.8px', color: RC.muted }}>CLAIM</span>
    </div>
  );
}
      ...
      <div style={{ overflowX: 'auto', overflowY: 'hidden', scrollbarWidth: 'none', WebkitOverflowScrolling: 'touch' }}>
        <div style={{ position: 'relative', display: 'inline-flex', flexDirection: 'column', gap: '2px', minWidth: '100%', padding: '11px 0' }}>
          {tierMarkLeft && (
            <div
              ...
              style={{
                ...
                border: `3px solid ${RC.purple}`, borderRadius: '38px',
          ...
          <div style={{ height: '62px', display: 'flex', alignItems: 'flex-end', paddingBottom: '8px' }}>
            ...
            {TIER_ORDER.map(({ tier }) => (
              <span key={tier} style={{ width: '76px', flex: '0 0 76px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '5px' }}>
                ...
                <span style={{ fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '11px', fontWeight: 'bold', letterSpacing: '1px', color: RC.text, textAlign: 'center' }}>
                  {tier.toUpperCase()}
                </span>
              </span>
            ))}
          ...
          {VIP_ROWS.map(([label, cells], i) => (
            <div key={label} style={{ height: '54px', display: 'flex', alignItems: 'center', background: i % 2 === 0 ? RC.surface : 'transparent', borderRadius: i === 0 ? '26px 26px 0 0' : i === VIP_ROWS.length - 1 ? '0 0 26px 26px' : '0px', boxSizing: 'border-box' }}>
              <span style={{ width: '150px', flex: '0 0 150px', padding: '0 10px 0 18px', boxSizing: 'border-box', fontSize: '12.5px', fontWeight: 600, color: RC.text, whiteSpace: 'nowrap' }}>{label}</span>
              {cells.map((c, j) => {
                const isCheck = c === '✓';
                const isText = c !== '✓' && c !== '—';
                const color = i === 0 ? RC.green : i === 1 ? RC.purple : RC.text;
...
function Chevron({ open }: { open: boolean }) {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" style={{ display: 'block', flex: '0 0 20px', marginLeft: '2px', transform: open ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 200ms ease' }}>
      <path d="m6 9.5 6 6 6-6" fill="none" stroke={RC.text} strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
  ...
  return (
    <>
      <div style={{ marginTop: '22px', padding: '0 2px' }}>
        ...
        >
          {icon}
          <span style={{ fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '19px', fontWeight: 'bold', letterSpacing: '0.6px', color: RC.text }}>{title}</span>
          <Chevron open={open} />
        </span>
      </div>
...
function XpEngineAccordion() {
  ...
  return (
    <AccordionShell testid="rewards-xp-engine-toggle" icon={<XpEngineIcon />} title="THE XP ENGINE" open={open} onToggle={() => setOpen((o) => !o)}>
      <p style={{ margin: '14px 0 0 0', fontSize: '14px', lineHeight: '22px', fontWeight: 600, color: RC.text, textAlign: 'justify', textWrap: 'pretty' }}>
      ...
      <div style={{ margin: '16px 0 0 0', display: 'flex', flexDirection: 'column', gap: '2px', padding: '11px 0' }}>
        <div style={{ height: '44px', display: 'flex', alignItems: 'flex-end', paddingBottom: '8px' }}>
          <span style={{ flex: '1 1 40%', fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '10px', fontWeight: 'bold', letterSpacing: '1.2px', color: RC.text }}>RAKE</span>
          <span style={{ flex: '1 1 30%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '10px', fontWeight: 'bold', letterSpacing: '1.2px', color: RC.text }}>XP / 1 <RcIcon size={13} /></span>
          <span style={{ flex: '1 1 30%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '10px', fontWeight: 'bold', letterSpacing: '1.2px', color: RC.text }}>100 <RcIcon size={13} /> WAGERED</span>
        </div>
        <div style={{ borderRadius: '26px', overflow: 'hidden', display: 'flex', flexDirection: 'column', gap: '2px' }}>
          {RAKE_XP_ROWS.map(([rake, xp1, xp100], i) => (
            <div key={rake} style={{ height: '54px', display: 'flex', alignItems: 'center', background: i % 2 === 0 ? RC.surface : undefined }}>
              <span style={{ flex: '1 1 40%', padding: '0 8px 0 18px', boxSizing: 'border-box', fontFamily: "'Space Grotesk', Arial, Helvetica, sans-serif", fontSize: '14px', fontWeight: 700, color: RC.text }}>{rake}</span>
              <span style={{ flex: '1 1 30%', textAlign: 'center', fontFamily: "'Space Grotesk', Arial, Helvetica, sans-serif", fontSize: '14px', fontWeight: 700, color: RC.green }}>{xp1}</span>
              <span style={{ flex: '1 1 30%', textAlign: 'center', fontFamily: "'Space Grotesk', Arial, Helvetica, sans-serif", fontSize: '14px', fontWeight: 700, color: RC.green }}>{xp100}</span>
            </div>
          ))}
        </div>
      </div>
...
function RakebackAccordion() {
  ...
  return (
    <AccordionShell testid="rewards-rakeback-toggle" icon={<RakebackFormulaIcon />} title="HOW RAKEBACK WORKS" open={open} onToggle={() => setOpen((o) => !o)}>
      ...
      <div style={{ marginTop: '14px', padding: '15px 18px', borderRadius: '20px', background: RC.surface, textAlign: 'center', fontFamily: "'Space Grotesk', Arial, Helvetica, sans-serif", fontSize: '15px', fontWeight: 700, color: RC.green }}>
...
function VolumeBonusAccordion() {
  ...
  return (
    <AccordionShell testid="rewards-volume-bonus-toggle" icon={<VolumeBonusFormulaIcon />} title="MONTHLY VOLUME BONUS" open={open} onToggle={() => setOpen((o) => !o)}>
      ...
      <div style={{ margin: '16px 0 0 0', display: 'flex', flexDirection: 'column', gap: '2px', padding: '11px 0' }}>
        <div style={{ borderRadius: '26px', overflow: 'hidden', display: 'flex', flexDirection: 'column', gap: '2px' }}>
          <div style={{ height: '46px', display: 'flex', alignItems: 'center', background: RC.surface }}>
            <span style={{ flex: '1 1 40%', padding: '0 8px 0 18px', boxSizing: 'border-box', fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '10px', fontWeight: 'bold', letterSpacing: '1.2px', color: RC.text }}>TIER</span>
            <span style={{ flex: '1 1 30%', textAlign: 'center', fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '10px', fontWeight: 'bold', letterSpacing: '1.2px', color: RC.text }}>MONTHLY XP</span>
            <span style={{ flex: '1 1 30%', textAlign: 'center', fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '10px', fontWeight: 'bold', letterSpacing: '1.2px', color: RC.text }}>BONUS</span>
          </div>
          {VB_ROWS.map((v, i) => (
            <div key={`${v.tier}-${v.xp}`} style={{ height: '54px', display: 'flex', alignItems: 'center', background: i % 2 === 0 ? 'transparent' : RC.surface }}>
              <span style={{ flex: '1 1 40%', padding: '0 8px 0 18px', boxSizing: 'border-box', display: 'flex', alignItems: 'center', gap: '7px', fontFamily: "'Space Grotesk', Arial, Helvetica, sans-serif", fontSize: '14px', fontWeight: 700, color: RC.text }}>
                <TierIcon tier={v.tier} size={16} />
                {v.tier}
              </span>
              <span style={{ flex: '1 1 30%', textAlign: 'center', fontFamily: "'Space Grotesk', Arial, Helvetica, sans-serif", fontSize: '14px', fontWeight: 700, color: RC.green }}>{v.xp}</span>
              <span style={{ flex: '1 1 30%', textAlign: 'center', fontFamily: "'Space Grotesk', Arial, Helvetica, sans-serif", fontSize: '17px', fontWeight: 700, color: RC.purple }}>{v.bonus}</span>
            </div>
          ))}
        </div>
      </div>
      <div style={{ marginTop: '4px', padding: '15px 18px', borderRadius: '20px', background: RC.surface, textAlign: 'center', fontFamily: "'Space Grotesk', Arial, Helvetica, sans-serif", fontSize: '15px', fontWeight: 700, color: RC.green }}>
  ...
  return (
    <div style={{ marginTop: '22px', padding: '0 2px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        {icon}
        <span style={{ fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '19px', fontWeight: 'bold', letterSpacing: '0.6px', color: RC.text }}>{children}</span>
      </div>
    </div>
  );
}
...
function CheckIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" style={{ display: 'block' }}>
      <circle cx="12" cy="12" r="11" fill={RC.green} />
      <path d="m7.5 12.3 3.1 3.1 5.9-6.2" fill="none" stroke={RC.sunken} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
function RewardsShieldIcon() {
  return (
    <svg width="26" height="26" viewBox="0 0 24 24" fill={RC.purple} style={{ display: 'block', flex: '0 0 26px', overflow: 'visible' }}>
...
function VipShieldOutlineIcon() {
  return (
    <svg width="26" height="26" viewBox="0 0 24 24" fill={RC.purple} style={{ display: 'block', flex: '0 0 26px' }}>
  ...
  return (
    <svg width="52" height="52" viewBox="7 6 34 34" style={{ display: 'block' }}>
      <g transform="rotate(45 24 24)">
        <rect x="22" y="5" width="4" height="17" rx="2" fill={RC.purple} />
      ...
      <path d="M37 10.5l.9 2.4 2.4.9-2.4.9-.9 2.4-.9-2.4-2.4-.9 2.4-.9z" fill={RC.text} />
      <path d="M12 33l.7 1.8 1.8.7-1.8.7-.7 1.8-.7-1.8-1.8-.7 1.8-.7z" fill={RC.text} />
    </svg>
  );
}
  ...
  return (
    <svg width="52" height="52" viewBox="7 6 34 34" style={{ display: 'block' }}>
      <g transform="rotate(45 24 24)">
        ...
        <circle cx="24" cy="18.4" r="3.2" fill={RC.purple} />
        <path d="M18.4 21.6l-3.9 4.6a2 2 0 0 0-.5 1.3v3.9l4.4-3z" fill={RC.purple} />
        <path d="M29.6 21.6l3.9 4.6c.3.4.5.8.5 1.3v3.9l-4.4-3z" fill={RC.purple} />
      ...
      <path d="M38.3 12.4l.7 1.9 1.9.7-1.9.7-.7 1.9-.7-1.9-1.9-.7 1.9-.7z" fill={RC.text} />
      <path d="M12.2 21.3l.9 2.4 2.4.9-2.4.9-.9 2.4-.9-2.4-2.4-.9 2.4-.9z" fill={RC.text} />
      <path d="M32.2 25.9l.6 1.5 1.5.6-1.5.6-.6 1.5-.6-1.5-1.5-.6 1.5-.6z" fill={RC.text} />
    </svg>
  );
}
...
function RakebackFormulaIcon() {
  return (
    <svg width="26" height="26" viewBox="6 6 36 36" style={{ display: 'block', flex: '0 0 26px' }}>
      <g transform="rotate(45 24 24)">
        <rect x="20.9" y="5.4" width="6.2" height="17.6" rx="3.1" fill={RC.purple} />
        <path d="M12.6 27V24a3.2 3.2 0 0 1 3.2-3.2h16.4A3.2 3.2 0 0 1 35.4 24v3z" fill={RC.purple} />
        <rect x="12.6" y="26" width="4.6" height="8.8" rx="2.3" fill={RC.purple} />
        <rect x="19.3" y="26" width="4.6" height="8.8" rx="2.3" fill={RC.purple} />
        <rect x="24.1" y="26" width="4.6" height="8.8" rx="2.3" fill={RC.purple} />
        <rect x="30.8" y="26" width="4.6" height="8.8" rx="2.3" fill={RC.purple} />
...
function VolumeBonusFormulaIcon() {
  return (
    <svg width="26" height="26" viewBox="9 7 30 30" style={{ display: 'block', flex: '0 0 26px' }}>
      <g transform="rotate(45 24 24)">
        <path d="M24 5.6c4.6 4 7.2 9.2 7.2 14.8v7.2H16.8V20.4c0-5.6 2.6-10.8 7.2-14.8z" fill={RC.purple} />
        <circle cx="24" cy="18" r="3.6" fill="#0B0B0B" />
        <path d="M16.8 20.8 11.9 26a2.6 2.6 0 0 0-.7 1.8v5l5.6-3.6z" fill={RC.purple} stroke="#0B0B0B" strokeWidth="1.3" strokeLinejoin="round" />
        <path d="M31.2 20.8 36.1 26c.5.5.7 1.1.7 1.8v5l-5.6-3.6z" fill={RC.purple} stroke="#0B0B0B" strokeWidth="1.3" strokeLinejoin="round" />
        <path d="M19.4 29.4h9.2l-1.7 4.2a2 2 0 0 1-1.9 1.3h-2a2 2 0 0 1-1.9-1.3z" fill={RC.purple} stroke="#0B0B0B" strokeWidth="1.3" strokeLinejoin="round" />
        <path d="M24 35.6c1.4 1.9 2.2 3.5 2.2 4.8a2.2 2.2 0 0 1-4.4 0c0-1.3.8-2.9 2.2-4.8z" fill={RC.purple} />
      </g>
    </svg>
  );
}
