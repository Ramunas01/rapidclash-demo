...
import { selectChessMove } from './index.js';
...
//
// Ported verbatim from `apps/server/src/guest/chess-bot-heuristic.test.ts` by issue #432, which
// moved `selectChessMove` itself out of `apps/server/src/guest/index.ts` into this package so
// `tools/bot-crowd` can import the same implementation. Only the import path changed; the
// `createGuestServices().selectBotMove` describe block that used to live alongside these tests
// stayed behind in apps/server, since that wrapper (and its "thinking"-delay policy) is still
// guest-mode's own.
...
describe('selectChessMove (issue #278 §3)', () => {
  ...
  it('throws if called for a player with no legal move', () => {
    // Stalemate-free shortcut: it is Black to move, so White (the bot) has no legal move at all.
    const blackToMove: GameState = {
      players: [BOT_ID, HUMAN_ID],
      fen: '4k3/8/8/8/3p4/8/8/3QK3 b - - 0 1',
      history: [],
    };
    expect(() => selectChessMove(blackToMove, BOT_ID, 1_000)).toThrow(/no legal move/);
  });

  it('works when the bot is BLACK (players[1]) — material is diffed against the right side', () => {
    // Mirror of ONE_CAPTURE_STATE with colours swapped and Black to move: black queen d8 can take
    // the white pawn on d5. The bot is players[1] here, so botSide resolves to 'b'.
    const botIsBlack: GameState = {
      players: [HUMAN_ID, BOT_ID],
      fen: '3qk3/8/8/3P4/8/8/8/4K3 b - - 0 1',
      history: [],
    };
    let captures = 0;
    for (let i = 0; i < 200; i++) {
      const move = selectChessMove(botIsBlack, BOT_ID, 1_000, seededRandom(i + 7));
      if (move.from === 'd8' && move.to === 'd5') captures++;
    }
    // Same ~50% signature as the White case — proves the capture actually scored above 0 (a
    // botSide mix-up would make every move tie at 0 and drop this rate to ~1/legalMoves).
    expect(captures / 200).toBeGreaterThan(0.3);
    expect(captures / 200).toBeLessThan(0.7);
  });
});
